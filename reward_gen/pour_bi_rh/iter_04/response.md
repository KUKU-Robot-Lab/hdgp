# iter_04 - make receiver progress a precondition for the source lift pay

## Why the receiver abandoned its cup (measured numbers only)
- Near its cup (e2000) the receiver earned reach_rcv ~0.7 and grasp_rcv ~0.01, and paid about 0.2-0.3
  (rush -0.23 summed, cup_disturb_rcv -0.13, contact_push -0.047). Away (e2500) it kept reach ~0.2 and paid
  almost nothing: leaving cost only ~0.3 per step.
- Over the same epochs lift_src went 0.005 -> 0.73 (later 2.09), a gain of ~3.0 per step that did not depend
  on the receiver at all. rcv_near_rate fell 0.87 -> 0.19 exactly then; contact/rcv_max 0.54 -> 0.05 N, tip gap
  stayed 66-86 mm, rcv_grasped 0.000.
- align_mouths / pour_tilt were 0 throughout: both need the receiver 5 cm up, so with cups_center_dist 0.34 m
  there was no gradient pulling the source toward the receiver.
- The two hypotheses in the notes (H1, H2) are unverified; the changes below rely only on the measured
  reward balance, not on them.

## Changes vs iter_03
1. lift_src is multiplied by lift_gate = 0.15 + 0.85 * rcv_prog, rcv_prog = 0.35 * reach progress
   (reach_rcv 0.2 -> 0.7 mapped to 0..1) + 0.65 * max(grasp_rcv/0.4, rcv up_w). Receiver away: 2.09 -> ~0.31.
2. W_REACH_RCV 1.0 -> 1.5 and new away_rcv = -0.8 * tanh(4 d_funnel) (receiver only).
3. Receiver near-cup penalties rescaled: rush 0.2 -> 0.05, one-sided 0.3 -> 0.1, palm 0.2 -> 0.1,
   slide 0.5 -> 0.25; tilt penalty 0.3, fling, foreign, cup-cup unchanged (knock-over protection stays).
   Proximity-dependent receiver penalties are now bounded by 0.25 vs >= 1.5 * 0.7 earned at the grasp pose.
4. W_CLOSE_RCV 0.3 -> 0.6.
5. New carry_src (W 2.0): source held and >= 5 cm up, receiver held (no lift needed), pays for reducing
   d_align with a constant slope out to 0.45 m (covers the measured 0.34 m).
6. Source reach/grasp/lift formulation and weights unchanged. align/tilt still gated by both cups up.
7. rcv_progress returned as a diagnostic (not summed).
