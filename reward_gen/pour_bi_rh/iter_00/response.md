...
- Assumptions that the prompt does not settle: (a) `tips_pos` are fingertip frame origins up to about 1 cm off the pad surface (PAD = 0.010); (b) the order of fingers 1..F-1 is not given, so the code only distinguishes thumb (index 0) from "the others"; (c) the sign convention of the palm normal in `*_palm_axes` is not given, so no palm-facing term was written; (d) whether `success` ends the episode is not stated. The bonus is per step, which is right if the episode continues after success. If success terminates the episode, a one-time 20 is small next to the dense stream and should be raised.
