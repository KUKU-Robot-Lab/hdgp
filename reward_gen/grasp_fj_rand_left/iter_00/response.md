Seeded from `reward_gen/grasp_fj_rand/iter_01/compute_reward.py` (right arm, the round that reached far-start success
0.85 at epoch 5000) by user decision on 2026-09-20: "left arm too", start from scratch with the i01 reward.

This iteration was not generated from `prompt.md`. The left-arm environment is the exact mirror image (y -> -y) of the
right-arm one: the cup is placed uniformly in x 0.10-0.40 m, y 0.00-0.30 m, the start pose is the mirrored right start
pose, and ctx is built with the same conventions (hand_q_norm is 0 = straight, 1 = most flexed for every joint). The only
direction-dependent line in the reward is the start-orientation term, which used palm_normal along +y on the right hand;
on the left hand the start palm normal points along -y, so that line reads `-n[:, 1]`. Nothing else is changed.
...
def compute_reward(ctx: RewardContext) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    ...
    # start orientation kept (palm_normal -y on this left hand, palm_finger_dir +x); ~0.46 at 20 deg on both axes
