...
The robot has to pick up an upright cup that appears at a new place on the table in every episode. The stages must happen in order:

1. **Approach (stage 1, until `approach_done`).** The hand starts raised beside the table. It travels to the cup's -y side and keeps its start orientation (palm_normal about +y, palm_finger_dir about +x, up to about 45 deg of turn for far-corner cups). The hand stays in its open default pose, with the thumb sticking out towards the cup. The palm plane ends up about 1-2 cm from the cup surface, with the cup between the thumb and the four fingers, and nothing touches the cup yet. The env latches `approach_done` when all six conditions hold on the same step.
2. **Envelope grasp (stage 2, until `envelope_done`).** With the same orientation, the palm moves onto the cup's side. The four fingers wrap the far side of the cup body and the thumb closes on the near side. The env latches `envelope_done` once the palm, the thumb and at least 4 digits have touched the cup for 5 steps.
3. **Lift and hold (stage 3).** The hand keeps that grasp and raises the cup straight up, 0.21-0.28 m above its own starting height. It holds the cup there, upright and still, and the env counts a success on every hold that also has the fingers wrapped.

### What went wrong in round 1 (`fj_rand_i00`)

- **Staying on the table paid more than lifting.** The grasp ladder was gated by `g23`, so every rung (grasp_pose, palm_reach, palm_contact, wrap, thumb, fingers, envelope_quality and more) kept paying at full rate in stage 3 with the cup still on the table. That was about 0.70 per step. Lifting added only about 0.05 (lift_hold 0.036, of which most was also paid on the table, plus lift_clear 0.006 and goal_close 0.008). Lifting also brought on tilt, table and arm-speed penalties. The best policy was to close the hand and stay put. Nothing rewarded upward motion itself and nothing charged downward motion, so the hand drifted down and pressed the cup into the table.
- **Height income was weak and got paid too late.** lift_clear saturated at 5 cm but was multiplied by `dwell3` and `grip`. The distance terms (goal_close and goal_near) only become large near the goal, which is 20+ cm away. The first centimetres of a lift earned almost nothing, so the lift never got bootstrapped.
- **Arm speed was penalised everywhere, not in a way that shapes the lift.** `-0.02 * sum(qd^2)` taxed all motion, including slow useful motion. Violent lifts (6-7 rad/s, cup thrown) were not singled out.
- **The approach latch fell from 0.94 to about 0.6.** Nothing in stage 1 gives grasp or lift income, so no path closes on the cup without the approach and still gets paid. Two effects remain:
  - Approach income decayed with `dwell1`, so approaches that took longer paid less.
  - The approach latch reward did not care how fast the arm arrived. As overall arm speed grew (arm_vel_pen -0.003 to -0.007), overshoot into the cup became more likely. That touch blocks the latch and pushes the cup away.

## 2. Changes

1. **Stage 2 ladder is stage 2 only.** It is now gated by `g2`, which already requires the cup to rest on the table and decays with dwell. Rungs and weights are unchanged, because they produced the envelope.
2. **In stage 3 the grasp becomes a ticket, not income.**
   - One hold-quality factor `hold_q` in [0,1] measures firm thumb, at least 2 firm fingers, palm contact and finger wrap. It **multiplies** every stage-3 payment.
   - The only grasp term left, `grasp_hold`, roughly matches the stage-2 value at the latch, so latching the envelope is never a step down. While the cup rests on the table it decays with the step budget (down to 40 %). That decay is removed as soon as the cup is 3 cm up.
3. **Lift income is measured from the cup's own spawn height and rises steadily.**
   - `lift_start` pays for the first 3 cm, dense and immediate.
   - `lift_height` is linear in `dz / (goal_z - spawn_z)`, which is per-environment and so handles different cup models.
   - `lift_vel` pays cup vz while held, in [-1, 1] and capped at 0.08 m/s. Moving down is **negative**, so it earns less than staying still. It fades out at goal height, where hold_still takes over.
4. **Downward pressing is charged.** `hand_sink_pen` applies in stages 2 and 3 while the cup rests: the palm sliding below the grasp band along the cup is penalised. That is exactly what "hand sinking with the cup wrapped" does, because the cup cannot move down.
5. **Smoothness shapes the lift but never blocks it.**
   - The global arm penalty now charges only joint speed above 0.5 rad/s.
   - While the cup is held in the air, `carry_arm_pen` (joint speed above 1 rad/s) and `carry_cup_pen` (cup linear speed above 0.25 m/s, angular speed above 2 rad/s) add at most -8 raw. A 1 cm slow lift already earns about 20 raw, and a lift carried to the goal earns about 240, so a slow lift always beats no lift.
6. **Approach keeps its value.**
   - `dwell1` is removed, so reaching the approached pose pays the same rate however long it took.
   - `approach_lock` is multiplied by arm calmness (0.5 to 1), so arriving slowly without overshooting into the cup is preferred. Stage-1 max is still 10 raw, at or below stage-2 entry.
   - Grasp and lift terms stay strictly behind `approach_done`, and lift terms behind `envelope_done`.
7. **Success condition, tolerance and success bonus are unchanged.**

Per-step raw ordering (x0.05 scale):

| Situation | Raw per step |
|---|---|
| Stage 1 | at most 10 |
| Stage-2 entry | about 11-19 |
| Stage-2 full envelope | at most about 51, decaying |
| Envelope held on the table | about 32 x hold_q, decaying to 40 % |
| Cup 3 cm up | about +25 on top, no decay |
| Held at the goal, still | about 240, plus 300 per success |

Expected metric moves:
- task/lifted_frac, lift_start, lift_height and goal_close go up.
- grasp_* ladder components fall in the logs, because they are now stage 2 only.
- table_pen and palm_table_pen shrink.
- The 99th-percentile arm speed while airborne drops below about 3 rad/s.
- The approach latch should recover towards 0.9. If it does not, the cause is not reward income and needs a per-placement probe.
