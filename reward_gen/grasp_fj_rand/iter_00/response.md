Seeded from `reward_gen/grasp_fj_stage/iter_08/compute_reward.py`, unchanged, by user decision on 2026-09-17.

This iteration was not generated from `prompt.md`. The user restarted the loop as a new track (`grasp_fj_rand`) because the
approach, envelope grasp and lift were established by the earlier `grasp_fj_stage` rounds, which the user regards as the
experiments that built the reward's basic structure. Round 9 of that track (`fj_stage_i08`) is the one reward that carried
the cup from the raised start through approach, envelope grasp, lift, upright carry and holding at the goal (far-start
success about 0.76, tilt 5.4 degrees). This track keeps that reward and changes the environment: the cup is placed
uniformly at random in x 0.10-0.40 m, y -0.30-0.00 m every episode, and there is no second start pose beside the cup.
Later iterations are generated from the feedback on this one.
