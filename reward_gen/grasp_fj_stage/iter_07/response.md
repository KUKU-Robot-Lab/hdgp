# 1. What the task means, and what the last round actually tells us

## The task

One right arm + a five-finger DG-5F hand must pick an upright cup off a table in three ordered stages, and the order is enforced by two latched environment flags (`approach_done`, `envelope_done`) that the reward may not redefine:

- **Stage 1 — approach.** From the raised pose (palm ~0.25 m above the table, ~0.38 m from the cup, turned sideways with the fingers pointing +x), carry the hand to the cup's **-y side without changing its shape**: `palm_normal` stays on +y, `palm_finger_dir` stays on +x, and every movable finger joint stays within 0.3 of `hand_default_q_norm` (hand open, thumb sticking straight out of the palm). The cup must end up *between* the extended thumb and the four fingers — palm plane within about a centimetre of the cup's side, the cup axis about one radius ahead of the palm along the fingers, palm inside the graspable band — **and nothing may be touching the cup yet**. All six of those conditions must hold *on the same step* for the env to latch the flag. Half the episodes (from the second on) instead start beside the cup with the flag already set; that is a training aid, not the task.
- **Stage 2 — envelope grasp.** Only after the flag: drive the palm the last few centimetres onto the cup's side and curl the four fingers around the far side while the thumb closes on the near side. The env latches `envelope_done` when palm + thumb + ≥4 digits hold for 5 consecutive steps.
- **Stage 3 — lift and hold.** Keep that grasp, carry the cup up to the goal (0.21–0.28 m above spawn) **upright and still**, and hold it there so successes keep counting. Success also requires a *wrap* measure above a curriculum threshold that rises towards 0.85.

Hard constraints throughout: do not knock the cup over, do not drop it, and **do not put any part of the hand on the table** — including the palm, which the episode-ending floor check does *not* cover.

## What the last round got right (must not be given back)

The grasp and the lift are solved for near starts: palm against the cup, fingers wrapped, cup carried clear of the table, 0.93 envelope / 0.75 success in near-start episodes. The hand stayed off the table by the finger measure (deepest penetration 0.006 m, `done/hand_floor` never fired). The pinky joined (0.24 of steps vs 0.059). Everything that pays for those — `pos`/`press_geo`/`palm_contact`, the curl and wrap rungs, the lift ladder, `pinky_join` — I keep, with the same shapes.

## What was missed or wrong

**(a) Stage 1 was never actually paid for, so far starts were abandoned.** This is the decisive number: `approach_reach` averaged **0.0026** and `approach_keep` **0.0010**, against `lift_height` 0.394 and a total of 1.03. The cause is arithmetic, not exploration. Stage 1's raw ceiling was 1.25 (0.125 after scale) — already an order of magnitude below what a near-start episode earns per step — and inside it `approach_reach = s1 * (0.5*coarse + 0.5*fine) * (0.2 + 0.8*keep)`. With the hand turned and the fingers closed (`keep ≈ 0`) the multiplier collapses to 0.2, and at the raised start `coarse ≈ 0.26`, giving `1.0 * 0.13 * 0.2 = 0.026` raw = **0.0026 scaled**, exactly the logged value. A shared policy optimising a return dominated by near-start episodes simply cannot see a signal that small. The far-start approach peaked at 0.669 around epoch 530 and collapsed as near-start success took off — precisely the failure mode the feedback warns will repeat. **Fix: stage 1 must pay on the same order as a near-start step.** I raise its ceiling from 1.25 to 10.0 raw, keep the shape multiplier (with a 0.15 floor so there is still a gradient while the hand is mis-shaped), and give the travel term a much longer reach (`1 - tanh(1.8·d)` = 0.41 at the 0.38 m start, 0.82 at 0.1 m) so progress pays *the whole way in* rather than only on arrival. These terms are multiplied by `s1`, which is only ever non-zero in far-start episodes — so this is a reason to improve that structurally **cannot** be taken over by the easy start.

**(b) The approach reward was never shaped on the conditions that actually fail.** The env reports which of the six gates pass: orientation 0.855 and no-touch 0.735 are fine, but **gap 0.083, along 0.046, pose 0.208, height 0.363** rarely do — and all six must hold simultaneously. The old code had no term for this at all; it used the *grasp* window (`d_pose`, deliberately widened to let the palm press in) as its proxy, which is not the latch window. I add an explicit six-condition vector: each gate gets a soft window score, `approach_form` pays the mean (dense partial credit on exactly the four failing gates) and `approach_lock` pays the **product** (worth 2.0 raw only when all six hold at once — the simultaneity the latch demands).

**(c) Reaching by rotating the wrist was not actually cheaper — it just wasn't much more expensive.** The video shows the arm turning the wrist to get the hand in low and rotated, skipping the C-shaped pre-grasp. Since the orientation gate passes 0.855 of steps, the turn is happening at the *end*, during the final centimetres. `approach_form` and `approach_lock` both contain `c_ori` and `c_pose`, and `approach_travel` is multiplied by `keep = ori * open_pose`, so arriving turned or closed now forfeits most of a 10-point stage instead of most of a 1.25-point one. Orientation stays multiplicative in stages 2/3 via `ori_k` as well.

**(d) The palm-on-table blind spot is now measurable and must be charged.** `palm_low_pen` paid **exactly 0.000 for the entire round** — it used `palm_pos[:,2]`, a virtual point up to 65 mm from the palm surface that moves as the wrist turns. The env now supplies `ctx.palm_clearance` (measured lowest palm point above the table), and I use it directly: free above 8 mm, quadratic to -8 at contact, -48 when pressed 10 mm in, plus a multiplicative factor on the grasp ladder. I deliberately do **not** put the free threshold at 20 mm: for the smallest cup (`cup_half_height` = 42 mm) a correct mid-body grasp puts the palm's lowest extent only a few millimetres above the table, and a high threshold would punish the very grasp that finally works. The residual small cost at 2–3 mm also nudges the grasp slightly *up* the cup body, which is the direction the feedback asks for anyway.

**(e) The cup is carried tilted, and nothing corrects it.** Mean tilt peaked at 23.9° mid-round and the video shows ~30° while carrying. `cup_tilt_pen` alone is additive and was worth -0.009; `lift_height`, the biggest term, did not care about tilt at all. I multiply **every carrying term** by `upright_c = exp(-(relu(tilt-0.09)/0.17)²)` — 1.0 below 5°, 0.78 at 10°, 0.35 at 15°, 0.002 at 30° — and add `carry_upright`, which pays from the moment the cup leaves the table (`dz/0.03`), not only at the goal. At the round's end-state (3.7° mean) this costs nothing, so the working lift is untouched; only the tilted carries lose.

**(f) A forward-looking failure the feedback did not name.** Success also requires a wrap measure above a rising curriculum threshold: that threshold went 0.150 → 0.689 (ceiling 0.85) while **the measured wrap fraction was 0.189**. Successes are currently being produced by a grasp that will stop qualifying as the curriculum tightens — successes already fell from 2.84 to 1.71 per episode. So I raise the curl/wrap rungs (`finger_curl` 1.5→5.0, `grasp_wrap` 1.5→5.0, `thumb_curl` 0.8→2.5, `envelope_quality` 2.0→6.0) rather than leaving them at keep-alive size.

**(g) Continuity and the anti-farming rule.** Raising stage 1 to 10 creates a cliff unless stage 2 starts above it, so rung 0 (`grasp_pose`) is set to exactly 10.0: the step the flag latches, reward goes up, never down. Ordering: stage 1 ≤ 10 raw < stage 2 boundary ≈ 13 < perfect parked envelope 51 < carried to the goal ≈ 192, with a success worth +300. Every term is bounded and the stage-2 ladder is **capped** at 51 while the lift ladder alone is worth 141, so grasp progress can never substitute for lift progress — the failure of the last two rounds. `dwell` factors (stage 1 mild, stage 2 to half) make parking in a stage strictly worse than finishing it. Global `scale` drops 0.1 → 0.05 so that despite the larger raw ladder the per-step totals stay in the range the working round trained on (parked envelope ≈ 2.6, carried ≈ 9.6, success +15).
...
| metric | now | expected |
|---|---|---|
| `approach_done` (far start) | 0.0014 | rises and **holds** past epoch 530, target > 0.5 |
| approach gates gap / along / pose | 0.083 / 0.046 / 0.208 | all rise together (`approach_lock` pays only jointly) |
| `reward/approach_travel`+`_form` | 0.004 total | 0.1-0.4 in far-start episodes |
| `contact/palm_touching`, envelope, lift (near start) | 0.235 / 0.93 / 0.94 | **unchanged** — those rungs keep their shape |
| `task/tilt_deg` while carrying | up to 23.9 | < 8 (carrying terms vanish by 15 deg) |
| palm on table | unmeasured, paid 0 | `palm_table_pen` becomes non-zero and then decays toward 0 |
| measured wrap fraction | 0.189 | rises toward the 0.689 curriculum threshold |
| `reward/total` | 1.03 | similar magnitude (scale halved against a taller ladder) |

Watch for the same warning sign as the last two rounds: if `reward/total` climbs while `approach_done` (far start) or `task/lifted_frac` falls, a bounded term is still being farmed — but every term here is capped, and the stage-2 ladder can no longer exceed the lift ladder's floor.
