...
The right arm starts raised beside the table: the palm is about 0.25 m above the table top, just outside the near table edge, with `palm_normal` along +y and `palm_finger_dir` along +x. The cup is about 0.38 m away, mostly in +x, somewhat in +y and below the palm. The hand already has the orientation the grasp needs. So the task is three stages in a fixed order, each unlocked by an environment flag that stays set once raised:

1. **Approach (`~approach_done`).** Move the palm, without turning it, to the cup's **-y side** at the height of the graspable band. The palm centre sits about 1 cm outside the cup surface, at x close to the cup axis. The fingers and thumb stay in `hand_default_q_norm` and do not touch the cup. Because the palm already faces +y, the palm faces the cup axis automatically once it is on the -y side. The flag also checks that the palm is on the -y side and that the start orientation and the default finger pose were kept.
2. **Envelope grasp (`approach_done & ~envelope_done`).** Stay at that spot with that orientation. Close the fingers so the palm presses on the cup side, the four fingers wrap one side of the cup (the +x side, since the fingers point +x), and the thumb wraps the opposite side. The flag is set once the palm, the thumb and at least 4 digits have touched the cup for 5 consecutive steps.
3. **Lift and hold (`envelope_done`).** Keep the grasp and lift the cup straight up to `goal_pos`, then hold it still and upright so that successes keep being counted.

Always-on constraints: do not tip the cup, do not push it before the grasp, do not drive the hand into the table, and keep actions smooth.

## 2. What went wrong in the previous reward (from the run)

- **Wrong orientation signal.** `approach_face` paid for pointing `palm_normal` at the cup axis from wherever the hand was. From the start pose that means a wrist turn of about 68 deg. It paid 0.19 before the hand moved and up to 0.49 after a turn. The policy learned the turn, which then broke the start orientation the approach flag needs. **Fix:** remove the face term from stage 1. Reward keeping the start orientation instead (`palm_normal . +y` and `palm_finger_dir . +x`). At the -y side target, facing the axis then follows from the geometry.
- **Wrong target.** The reach target was the nearest point of the band. From the start pose that point is on a diagonal side of the cup, so the policy was pulled away from the -y side the flag requires. **Fix:** the target is now a fixed point on the cup's -y side: `lat = -(palm_y - cup_y)` should be `R + 8 mm`, `|palm_x - cup_x|` close to 0, and the band height. A small stand-off funnel keeps the palm up to 4 cm further out in -y while it is still far from the cup axis in x. The fingers point +x and reach the cup first, so this keeps them off the cup while the hand slides in.
- **Pose term always zero.** `approach_pose_keep` was 0.000 for the whole run. It used the *worst* joint of max(actual, commanded) deviation and was flat once any joint moved about 0.15. A policy action of 0 already commands the mid range (norm 0.5), so the pose was past that point from step 1 and the term gave no gradient. **Fix:** a per-joint score averaged over the 13 movable joints, with a linear part plus an exponential part. The linear part keeps a gradient at any deviation. The score uses the actual joint angle and the raw policy action (the action maps linearly to the target), so reward follows the action directly.
- **Reach paid too little.** Reach paid 0.10 to 0.16 against 0.19 to 0.37 for face. **Fix:** reach is now the largest stage 1 term, up to 2.0 of the stage 1 maximum of 3.0: 1.0 coarse from any distance plus 1.0 for arrival. The arrival part (`exp(-d/0.03)`) is multiplied by orientation kept and finger pose ready, the same conditions the flag checks. That way the last centimetres pay only in a pose that can actually set the flag.
- Stages 2 and 3 got no feedback, so their structure and flag gating stay as they were. There are only two stage 2 changes: the "stay" target is now the same -y side point, and the palm-faces-axis term is replaced by keeping the start orientation ("at that position and with that hand orientation"). Stage offsets are recomputed so that finishing a stage never lowers reward: stage 1 max 3.0 < stage 2 base 3.5, and stage 2 max 3.5 + 3.75 = 7.25 < stage 3 base 7.5.

Expected metric changes: `approach_orient_keep` should stay near its start value of 0.5 instead of the hand turning. `approach_pose_keep` should rise from about 0.1 (random actions) towards 0.5. `approach_reach` should rise steadily as the palm distance drops. `approach_arrive` becomes non-zero in the last few cm, then `approach_done` starts to be set and `stage_base` jumps to 3.5 in those environments. Contact metrics should stay near 0 until the envelope stage.
