...
Stage 1, approach: starting from the raised pose beside the table, move the hand to the side of the cup while the hand keeps its default pose (ctx.hand_default_q_norm): the fingers and the thumb must not change shape and must not touch the cup during the approach. The approach is complete when the palm centre is at the cup's graspable band with the palm facing the cup; the environment then sets ctx.approach_done for the rest of the episode.
Stage 2, envelope grasp: only after the approach is complete, and at that position, close the hand into an envelope grasp (a power grasp): the palm against the side of the cup and all five fingers wrapped around the cup body with their inner surfaces, the thumb on the opposite side of the cup from the other four fingers. The environment sets ctx.envelope_done once the palm, the thumb and enough fingers hold the cup for several consecutive steps.
