...
The hand has to form a power (envelope) grasp on an upright cylinder and carry it to a goal about 0.25 m above where it started. The stages are:

1. **Oriented approach.** Move the palm centre from the raised pose to the side of the cup, at the height of the graspable band, just outside the cup surface. The palm normal must point at the cup axis, and the fingers must lie flat and point along the tangent of the cup so they can curl around it. This is close to the start pose: the palm is already sideways and the fingers already point forward, so the hand should not roll.
2. **Wrap and contact.** With the palm on the cup, close the fingers. The palm surface presses on the cup. The links of all five fingers touch the cup inside the band. The thumb wraps around the other side of the cup from the index, middle, ring and pinky fingers.
3. **Lift.** Raise the cup while it stays upright and the envelope stays closed.
4. **Hold at the goal.** Keep the cup upright and still inside the success tolerance so successes keep being counted.

Throughout: don't push or tilt the cup while it sits on the table, don't knock it over, and don't press the hand into the table.

## 2. What went wrong in the last round and why

- **Reach paid without orientation.** Reach had weight 1 and did not care where the palm faced. Once the policy found it could reach the cup low and from the robot side with the hand rolled, that was the cheapest way to earn reach, and `palm_orient` (weight 0.5, about 0.10 per step) could not outweigh it. The reach point was also the palm link origin, so the wrist end of the hand was the part pulled onto the cup. The environment has fixed this point now.
- **Palm contact counted any palm-body contact.** It was gated only by the palm point's radius and height, never by `facing`. So the wrist end touching the cup while the palm faced away earned `palm_contact` (0.13 rose to 0.41).
- **`finger_wrap` paid for closeness alone.** Its only gate was `near_soft`. Straight fingers sliding under or behind the cup earned 0.26 per step, and no bending or contact was needed.
- **Resting paid the same every step.** Reach, palm contact and wrap could all be collected by resting against the cup, so the return grew linearly with episode length. Nothing made lifting clearly worth more.
- **Pushing and tilting were almost free.** The push penalty was about 0.09 per step, there was no tilt penalty, and `upright` barely changed at 20 degrees (0.89).

## 3. Changes

- **Orientation gate `orient_ok`.** It multiplies `palm_gate` and `finger_gate`:
  - `palm_gate`: the palm normal must point at the cup axis (1 within about 26 degrees, 0 beyond about 66 degrees).
  - `finger_gate`: the finger direction must be horizontal. The direction is measured from the finger links, projected into the palm plane, so it does not depend on the palm frame axes.
- **Reach split in two.**
  - A small ungated coarse term keeps the fast move. It stops growing within 4 cm of the grasp spot.
  - A larger fine term pays only when `orient_ok` is high.
- **Palm contact requires the palm surface on the cup.** It needs force, the palm centre within 3 cm of the cup surface and inside the band, and `palm_gate`.
- **No proximity-only finger reward.**
  - Fingers are paid only for contact force on their links, inside the band and near the surface.
  - Only when the palm is at the grasp spot and oriented (`gate_g`).
  - Thumb opposition also needs contact from both the thumb and the other fingers.
  - Commanded finger closing is paid only when the palm is at the grasp spot and correctly oriented.
- **All-five-finger envelope.** A term rewards more fingers in contact (quadratic ladder) and all five together with the palm.
- **Stage ladder.** Rough per-step maxima:
  ...
  | Lift | +4 |
  | Goal and hold | +5 |
  | Each success (bonus) | +20 |

- **Stall decay.** While the cup is not raised upright, the approach and grasp terms shrink with `episode_progress` (1.0 down to 0.25). Resting next to the cup is worth less and less, and a raised upright cup removes the decay.
- **Stronger penalties before the lift.**
  - Tilt: -1.5 at 25 degrees, 5-degree dead band.
  - Sliding the cup: -1.0 at 6 cm, 1 cm dead band.
  - `upright = exp(-(tilt/0.3)^2)` now drops to 0.26 at 20 degrees, and it gates the grasp, lift and goal terms.

Expected shifts:
- `palm_orient` / `orient_ok` go up.
- `palm_contact` drops at first, then climbs again with the palm correctly facing.
- `finger_contact`, `fingers_touching` and `envelope` go up.
- `task/tilt_deg` goes down.
- After that, `lifted_frac` should become non-zero.
