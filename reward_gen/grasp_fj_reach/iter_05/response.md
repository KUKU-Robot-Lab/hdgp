...
The right OpenArm with the DG-5F hand starts raised beside the table. It has to take an upright cup off the table with a power (envelope) grasp, bring it to a goal about 12-28 cm above where it started, and hold it there upright and still so the environment keeps counting successes. Cup sizes differ between environments.

Stages:

1. **Approach with an open hand.** Move fast from the start pose to the side of the cup. The palm faces the cup axis, the fingers are roughly horizontal and still open. The cup is not touched, tilted or pushed, and the hand stays off the table.
2. **Seat the palm.** The palm surface presses on the side of the upright cup.
3. **Wrap.** All five digits curl around the cup body inside the graspable band and touch it with their inner surfaces (the cup is on the inside of each finger's bend). The thumb is on the other side of the cup from the four fingers, and the palm stays on the cup.
4. **Lift.** Raise the whole cup off the table in that grasp, keeping it upright, along the short path from the start position to the goal.
5. **Hold at the goal.** Keep the cup at the goal, upright and still, so success is counted again and again until `max_successes`. It must not be carried past the goal.

Penalties throughout: the hand coming down onto the table, tilting or shoving the cup on the table without a grasp, carrying a lifted cup away from the goal, and jerky actions.

## 2. What was missed or wrong in the last reward (epoch 4000-5231 behaviour)

The approach, the closing of the hand and the lift now work: grasp 0.99, lift 0.89-0.99. What remains broken, and why:

1. **The back of a finger counted as a wrap.** `link_c` counted any contact between a link and the cup. Index and ring resting on the cup with their backs paid the same `wrap_fingers` (1.16/step) and `wrap_links` as a real wrap. Nothing checked which side of the finger touched the cup.
   - *Fix:* for every measured link, estimate the normal of its inner surface.
     - Four fingers: `cross(segment, flexion_axis)`. The flexion axis is `palm_normal x finger_direction`. For a straight finger this normal equals `palm_normal`, and it rotates with the curl.
     - Thumb: the normal points from the thumb link toward the object centre in front of the palm, perpendicular to the thumb segment.
     - A link counts for wrapping only if the direction from the link to the cup axis points along its inner normal. Contact with the back of a finger earns nothing in the wrap terms.
   - The count of digits holding the cup with any surface is still used to decide that the cup is held (for lift, the goal terms and penalty relief), so the working lift is not lost while the grasp is being corrected.
2. **The palm had no reason to join the grasp.** `palm_credit` gave half palm credit whenever `grasp_q` was high without palm contact, so palm contact in a grasp added almost nothing (palm 1% of steps).
   - *Fix:* remove that credit and add `palm_in_grasp`: the palm pressing and facing the cup while at least 1-3 digits wrap with their inner surfaces.
3. **Grasp quality did not affect lifting and holding.**
   - *Fix:* lift and every goal term are multiplied by `q_mult` = 0.4-1.0, computed from the palm plus the inner-wrapped digits with the thumb opposed. A full envelope is worth about 1.7x the current thumb-and-middle grasp at every later stage.
4. **Lifting paid the same anywhere, and the goal terms were too weak to steer.**
   - `lift` (weight 6) saturated at goal height but stayed fully paid wherever the cup was carried.
   - `goal_coarse`/`goal_fine` had weight 2 and were further scaled by a `grasp_q` of only 0.34-0.42, so they paid 0.05-0.08 per step.
   - *Fix:*
     - `lift` weight goes to 5 and still stops growing at goal height.
     - `goal_coarse` becomes position based (weight 3, 10 cm scale). `goal_fine` gets weight 5 (2 cm scale), `hold_still` weight 4 with a sharp speed/spin kernel, and `in_tolerance` weight 3.
     - Near the goal these terms are worth about 3x the lift.
     - New `carry_away_penalty`: while the cup is off the table, it grows linearly with the cup's distance from the straight segment between its start position and the goal (3 cm dead band, up to -3). Going higher than the goal or off to the side costs more the further it goes. Lifting straight up costs nothing.
5. **The penalty for sideways cup movement was the wrong signal for a carried cup.** Its grasped branch saturated at 15 cm and applied in the air too (-0.6/step, growing with the lifted fraction).
   - *Fix:* `cup_push_penalty` now applies only while the cup is on the table: strict without a grasp, lenient with one. A lifted cup is steered by the goal terms and `carry_away_penalty`.
6. **Holding still was not worth enough.**
   - *Fix:* `hold_still` now has weight 4 and uses `exp(-speed/0.05 - spin/0.5)`. `in_tolerance` also needs the cup to be off the table. The success bonus and `in_tolerance` scale with grasp quality.

Unchanged: the approach terms and their resting decay, the open-hand term, table clearance, the cup-clearance "airborne" test (tilting is not lifting), the tilt penalty and regularisation. Where they used `grasp_q` they now use `hold_q`, which comes from the digits holding the cup.

Approximate per-step values:

| Situation | Reward per step |
|---|---|
| Hovering at the grasp spot | ~0.7 |
| Palm pressed on the cup | ~1.4 |
| Current grasp (thumb and middle wrapped, index and ring backs) on the table | ~1.0 |
| Full envelope on the table | ~7 |
| Current grasp carried 30 cm away | ~2 |
| Full envelope carried 30 cm away | ~10 |
| Current grasp at the goal | ~12 |
| Full envelope held still at the goal | ~27, plus ~25 per success |
