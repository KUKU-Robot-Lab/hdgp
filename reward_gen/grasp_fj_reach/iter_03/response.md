...
The right OpenArm with the DG-5F hand starts with the palm raised about 0.25 m above the table, outside the near table edge, about 0.38 m from the cup. It must pick up an upright cup whose radius (44-81 mm) and band half height (42-65 mm) change between environments. It has to use a power (envelope) grasp and hold the cup at a goal 0.21-0.28 m above its start. The stages are:

- **A. Oriented approach (about 0-3 s).** Move the palm centre quickly to a grasp spot on the side of the cup:
  - the palm centre at the cup surface, in the upper part of the graspable band, so the lower fingers stay clear of the table;
  - the palm normal pointing at the cup axis;
  - the fingers horizontal, i.e. perpendicular to the cup axis and tangent to the cup body, so they can go around it;
  - the hand open the whole way.

  The cup must stay untouched, upright and in place, and the hand must stay off the table.
- **B. Palm placement.** The palm surface presses lightly on the side of the cup while the cup stays upright. On its own this is only a preparation; it is not a goal state.
- **C. Wrap (the grasp itself).** Fingers wrap the cup body inside the band:
  - index, middle, ring and pinky flex around one side of the cup, and the thumb wraps the other side;
  - in the end the palm and the links of all five digits touch the cup;
  - small cup motion caused by closing the hand is normal here and must not be punished like shoving.
- **D. Lift.** The whole cup, including the lowest point of its bottom rim, leaves the table. The cup stays upright and is held by the wrap, not balanced or tipped.
- **E. Goal and hold.** Bring the cup to `goal_pos`, upright. Keep `goal_dist <= success_tol` for `success_hold_steps` steps, and stay still so successes keep being counted up to `max_successes`.

These rules apply at every stage: don't tip or shove an ungrasped cup, don't rest the hand or fingers on the table, and keep the motion smooth.

## 2. What went wrong in the last round

The main problem is that resting was paid better than grasping. The resting pose (palm leaning on the cup and tilting it, straight fingers pushing on the table) earned 0.44-0.50 per step. The wrap-and-lift pose earned 0.15-0.35 per step. PPO correctly went back to resting each time. The causes, term by term:

1. **Palm contact was paid while tilting the cup.**
   - `upright_grasp = exp(-(tilt/0.45)^2)` is still 0.96 at 5 deg, and the tilt penalty had a 10 deg dead band, so tilting 4.7-6 deg cost nothing.
   - The finger gate `(f_level - 0.5)/0.4` gave full credit to fingers pointing about 26 deg below horizontal. So "palm on a low part of the cup, fingers angled down onto the table" passed as oriented.
   - Result: palm_contact paid 0.23-0.29 per step.
2. **Lift was paid for tilting.**
   - `dz` is the rise of the cup centre. Tipping a cup of radius R about its bottom rim raises the centre by about R·sin(tilt), roughly 6 mm at 5 deg.
   - 30 % of the lift term was paid over the first 3 cm, and `hold_q` has a floor of 0.2 even with no fingers on the cup.
   - Result: lift paid 0.023 per step in the resting pose, as much as in the real lifting phases.
3. **The table penalty could not fire.** It only used `hand_z_min` below `table_z + 5 mm`. Link frames sit about half a finger thickness above the surface when a finger lies on the table, so the penalty stayed at 0.0000 while the fingers pushed on the table.
4. **Every wrap term was multiplied by `palm_on`.** `palm_on` required palm force, the strict orientation and a tight palm gap. When the fingers close and the cup starts to move, the palm partly leaves the cup (the palm touched at success in only 8-36 % of cases). The wrap reward therefore collapsed exactly when the wrap formed: wrap_fingers ≤ 0.006, envelope ≤ 0.001, curl ≤ 0.002 per step.
5. **Closing the fingers was penalised during the transition.** `finger_open` was active whenever `palm_on` was 0. When palm contact flickered during closing, closing cost up to 0.1.
6. **Cup-movement penalties ignored the grasp.** Push and tilt penalties treated a wrapped cup that was being lifted like a shoved cup (−0.05 to −0.12 each in the wrap-lift phases). The lost approach and palm rewards made the transition a net loss.
7. **Resting still paid near the end of the budget.** Hover terms decayed only to 40 % and on-table palm terms to 70 %.

## 3. What changes in the new reward

- **Stage A** is a small share: at most about 0.27 per step.
  - It decays to 20 % over the step budget unless the cup is grasped.
  - It is zero while the hand is on the table.
  - Ungrasped cup tilt removes it.
  - The orientation gate is stricter: palm normal within about 26 deg of the cup-axis direction for full credit, fingers within about 12 deg of horizontal (zero at about 27 deg).
- **Palm contact** counts only while the cup is upright before a grasp: full credit below 2.5 deg, zero at 5 deg, together with the strict orientation. Both limits relax as the grasp quality rises. The term decays like stage A unless the cup is grasped, and it is zero when the hand touches the table.
- **Grasp quality `grasp_q`** comes from the finger contacts on the cup body: 0 with one digit, 1 with the thumb opposed plus three fingers. It gates lift and goal, and its square root relieves the cup-motion penalties.
- **Wrap terms** are gated only by "palm at the grasp spot, loosely oriented". Palm force is not needed, so they survive the transition. The terms are:
  - `wrap_fingers`, superlinear in the number of touching digits and zero for a single digit;
  - `wrap_links`;
  - `finger_curl_onto_cup`, flexion of fingers that lie on or touch the cup body, so closing on air pays nothing;
  - `envelope`, all five digits with the thumb opposed, more with the palm on.

  Together they are worth about 4.3 per step for a full wrap, compared with about 0.7 for the palm alone and about 0.23 for hovering. A wrap left on the table slowly loses 40 % of its value.
- **`finger_open`** pays only while the palm is still away from the grasp spot (more than about 3 cm) and nothing is grasped. Closing near the cup is never penalised.
- **Lift** uses the clearance of the lowest bottom-rim point, `cup_z - table_z - b·cos(tilt) - R·sin(tilt)`, with `b` the start height of `cup_pos` above the table. This stays about 0 when the cup is tipped about its rim. The lift term is multiplied by `grasp_q` and uprightness, with weight 6; 40 % is paid once the cup is 2 cm clear, so starting the lift is clearly worth it.
- **Goal terms** are gated by clearance × `grasp_q` × uprightness.
- **Table penalty** uses the height of every finger link and fingertip frame, `hand_z_min` and the palm centre: 0 at 18 mm or more, full at 6 mm or less for the link frames, with weight 1.5.
- **Tilt and push penalties** are blended by the square root of `grasp_q`:
  - tilt without a grasp: 3 deg dead band, full at 13 deg; in a grasp: 15 deg dead band, full at 35 deg;
  - push without a grasp (cup on the table): 1 cm dead band, full at 5 cm; in a grasp: 8 cm dead band, full at 15 cm.

Approximate values per step early in an episode (contact indicators about 0.9):

| State | Reward per step |
|---|---|
| Hovering oriented at the grasp spot | ~0.23 |
| Palm pressed, cup upright | ~0.7 |
| Palm + thumb + 2 fingers | ~1.9 |
| Palm + 5 fingers wrapped, on the table | ~4.3 (decays to ~3.3) |
| Same grasp, cup 3 cm off the table | ~6.9 |
| Held still at the goal | ~15, plus ~24 per success |
| Last round's resting pose (5 deg tilt, fingers on the table) | about −1.7 |

Expected metric changes:
- `palm_touching` in resting phases and the lift term without a grasp go toward 0;
- `table_penalty` becomes nonzero early in training, then falls;
- `fingers_touching` and `grasp` rise;
- `lift` pays only when `task/lifted_frac` rises.
