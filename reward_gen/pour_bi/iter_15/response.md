...
Both arms work together: the right hand wraps the bead-filled source cup, the left hand wraps the empty receiver cup, both cups are lifted, the source cup's pouring lip is brought over the receiver mouth (above its rim), and only then is the source cup tilted past the fill-dependent release angle so the beads fall through the air into the receiver cup. The receiver cup stays upright and still, neither cup is dropped, the cups do not touch or nest, and spill stays small. Stages: (1) approach both cups, (2) power-grasp both, (3) lift both, (4) carry and aim the source lip over the receiver mouth, (5) tilt: pre-tilt below the latch limit, then the release tilt with the lip over the mouth, (6) transfer beads (increments), (7) hold after the pour.

## 2. Analysis of round 15 (iter_14)

Stages 1 to 4 are solved: both grasp flags about 0.8, both cups lifted, the lip parked about 5.3 cm (xy) from the receiver mouth, inside the 8.1 cm latch radius. The policy is stuck at stage 5. The mean source tilt is 32 deg, about 18 deg below theta_pre (about 50 deg). `tilt` never paid (0.004) and `pour_pose`, `pour_delta` and `success` stayed at 0. Three reasons in the reward code:

1. **Past theta_pre, tilting at the parked lip distance loses reward.** At lip_xy = 5.3 cm, `over` is only about 0.48 (full at 3.5 cm, zero at 7 cm). The income past theta_pre is `4 * over * fin_frac`, which is about 0.6 for 0.2 rad. The corridor penalty is `2.5 * tanh(excess * (1 - over) / 0.2)`, about -1.2 for the same 0.2 rad. On top of that, `src_up` (about 0.85) cuts roughly 15 % of the ~8/step raise/converge/bring/lift income. So the corridor puts a wall at theta_pre exactly where the lip sits.
2. **Tilting costs reward through a flickering grasp flag.** `lift_src` (2.0), `wrap_src` (1.5), `grasp_src` (1.0), `both_grasped` (1.0) and `both_lifted` (1.0) all multiply the boolean `src_grasped`. Over the round src_grasped fell from 0.98 to 0.76 while tilt and hand-foreign contacts rose. A thumb contact that flickers during rotation can wipe out up to ~6/step even though the cup is still carried 19 cm above the table. Iter_13 already moved the pour stack to carry phases, but these terms were left on the flag.
3. **The pre-tilt slope is shallow.** 4.0 over about 0.88 rad, times near_pre of about 0.8, gives about 3.6 per rad. That is small against the risk on the tilt axis (contacts, rot_speed, latch tail).

## 3. Changes in iter_15

- **One continuous tilt income, `tilt` (weight 10), replaces `pretilt` + `tilt`.** Income is theta_eff / tilt_target. The part below theta_pre is scaled by `near_pre` and needs the approach zone. The part above theta_pre is scaled by a new **safe gate** `safe`: 1 with the lip within 4.5 cm (xy), 0 at 7 cm, still 1.1 cm inside the env latch radius. Continuity check: at 32 deg the income is about 10 * 0.56 / 1.86 * 0.79 * 0.93 = 2.2 (was 2.07). At theta_pre it is about 4.7, and at the full release pose it is 10. Tilting in place also moves the lip inward, so the gate opens as the cup tilts.
- **The corridor penalty and `src_up` now use `(1 - safe)` instead of `(1 - over)`.** Tilt past theta_pre is free once the lip is within 4.5 cm. `pour_pose` and `pour_phase` use the same gate. `over` is removed.
- **Grasp-dependent income uses the held state `max(src_grasped, carry)` instead of the boolean flag.** This applies to `grasp_*`, `wrap_*`, `lift_*` and `hold_*`. `both_grasped` becomes held_src * held_rcv and `both_lifted` becomes src_carry * rcv_carry. On the table nothing changes (carry = 0). A cup carried high no longer loses about 6/step when a finger contact flickers during the rotation.
- Unchanged: bead increments, spill, success bonus, latch penalty, contact and stillness penalties, and all approach/aim geometry.

Expected shift: task/src_tilt_deg rises past 50 deg toward 80+, reward/tilt rises above 4.7, pour_pose and pour_delta become non-zero, and src_grasped stops falling as tilt grows. Watch premature_tilt_rate (it should stay below 0.02 because the tilt income past theta_pre requires the lip within 7 cm) and spill.
