...
The right hand has to pour beads from its cup into the cup held by the left hand. Stages:
1. **Approach and grasp (both arms):** each palm reaches its cup and closes into a power grasp (thumb plus other fingers on the cup).
2. **Lift:** both cups come off the table. The source goes higher (about 20 cm), the receiver is lifted moderately and kept upright and still.
3. **Converge and aim:** move the cups together without touching. The source rim sits 5 to 15 cm above the receiver rim, and the source pour LIP (`src_pour_lip_pos`) ends up over the receiver mouth.
4. **Pre-tilt:** tilt the source toward the receiver up to (premature limit − 10°). This is safe anywhere near the receiver because the env latch only fires above the limit.
5. **Release tilt:** with the lip within about 3.5 cm of the receiver mouth centre, tilt past the fill-dependent release angle so the beads fall through the air into the receiver.
6. **Hold:** keep both cups grasped, the receiver upright and the source over it until at least half of the beads have transferred with little spill.

Safety runs through every stage: no cup-cup contact, no hand contact with anything but its own cup, no nesting, and no premature tilt.

## 2. Analysis of round 14

- Stages 1 to 3 are solved and stable: both grasp rates are about 0.9, source lift 0.21 m, receiver lift 0.07 m, total reward about 20/step.
- `tilt`, `pour_pose`, `hold_*` and `success` stayed at exactly 0 all round. Only `pretilt` grew (0 → 1.1). The mean source tilt crept up from 8° to 26°, but the release segment starts at theta_pre ≈ 50°, so it was never reached.
- **Parking exploit:** `pretilt` and the tilt corridor were fully open anywhere inside `APPR_FULL = 10 cm` of lip xy. The policy moved the lip out to 9.9 cm, right on that edge. Tilting in place there was paid exactly as much as tilting over the mouth.
- **Pushed apart by contacts:** as the tilt grew, hand-foreign contact rose (src 0.04 → 0.13, rcv 0.03 → 0.08) and the cups separated (lip distance 0.044 → 0.099 m). `bring_together`, `aim`, `tilt` and `pour_pose` were multiplied by a hard `clean` gate. A touch near the receiver therefore wiped out about 4/step of income, while the same touch far away cost almost nothing. The net effect was a position-dependent push away from the receiver, which is the opposite of what the pour needs.

## 3. Changes (minimal, three levers)

1. **Soft clean gate plus a stronger additive contact penalty.** `bring_together` and `stack_gate` now use `clean_soft = 0.5 + 0.5·clean`. `FOREIGN_W_HELD` goes from 1.0 to 1.5, so contacts are still charged, and charged the same at any distance.
2. **Pre-tilt is proximity-weighted.** It is now multiplied by `near_pre = 0.25 + 0.75·exp(-10·lip_off)` (lip at 10 cm → 0.59, 5 cm → 0.81, over the mouth → 1.0). `PRE_W` goes from 3 to 4, so tilting pays most when done over the mouth, and the tilt gradient is steeper (0.08/deg instead of 0.06/deg).
3. **`APPR_FULL` goes from 0.10 to 0.08 m.** The fully open approach zone now ends at the env latch radius (8.1 cm), which takes away the edge the policy parked on. The allowed pre-tilt at 10 cm drops by less than 2°.

Expected effect: the lip distance falls back below 5 cm, `pretilt` keeps rising with the tilt passing 40° to 50°, and `tilt` and `pour_pose` become non-zero. Hand-foreign contact should stay about the same or fall slightly, because the additive penalty is now 1.5×.
