...
**Task stages.** (1) approach both cups; (2) wrap-grasp each cup (thumb + fingers on its own cup); (3) lift both — source clearly higher than the receiver, **both upright**; (4) carry the upright source until its mouth hovers above and within a few cm (xy) of the receiver mouth; (5) only then tilt the source toward the receiver so the pouring lip sits over the receiver mouth, tilting past the fill-dependent release angle; (6) beads fall through the air into the receiver, which is held still and level; (7) hold both grasps after the pour.

**What the feedback says.** The previous policy converged (success 0.77, 74 % beads, spill 6 %, receiver level, no vibration) but tilted the source **on the table and during the lift** (20° at step 77 on the table, 45° at step 91, 80° at 0.21 m lift, 0.26 m from the receiver). Every episode would now trip the new `premature_tilt` latch (> 30° while mouths > 0.10 m apart) and success would be impossible. The cause is in the reward, not the policy: `tilt` was gated by `aim_loose`, which still paid ~40 % at 0.30 m; `tilt_dir` had no distance gate at all; `tilt_premature` was free below 86°. So early tilting earned ~2/step and cost nothing. Everything else (grasp, lift, aim precision, receiver upright, smoothness, clearance) worked and is kept.

**Changes.**
1. **Distance-gated tilt.** A single `near` factor on the mouth-to-mouth xy distance (1 at ≤ 5 cm, 0 at ≥ 9 cm — 1 cm inside the env's 10 cm latch, smooth in between) now multiplies **all** tilt income (`tilt`, `tilt_dir`, `pour_pose`, `aim`). Far from the receiver, tilting earns exactly nothing.
2. **Source-upright constraint while far.** New `upright_src`: while the source is held (grasp flag or carried) and `far = 1 − near`, tilt beyond 11.5° is charged with `tanh(excess/0.2)`, weight 1.0 on the table rising to 2.5 once carried (20° carried → −1.6, 30° → −2.3). It is gated on the grasp so a random early policy that knocks the cup over is not taught to avoid the cup (the iter_07 receiver lesson). The same excess also scales `lift_src` and `bring_together` down (`src_up`, Gaussian σ 0.25 rad), so the lift is only fully paid for an upright cup. `premature_latch` adds −0.5/step once the env latch is set, so the value drops at the exact violating step even before success has ever been seen. The old `tilt_premature` is removed (subsumed).
3. **Hover instead of "same height".** `bring_together` used the lip height relative to the receiver rim, which for an upright cup rewards standing beside the receiver at the same height — from where the only way to get within 5 cm is to tilt early. It now uses a hover band on the **source mouth** 11–16 cm above the receiver rim (non-nesting, non-touching), falling back to the lip-based `aim_z` once near, so the carry path is: up, over, then tilt.
4. **Fill-adaptive release band.** `release_tilt = 1.25 + 0.60·(1 − fill)` reproduces the measured onset (72° full, ~96° at 6 beads); `pour_pose` ramps from 6° below to 26° above it and the linear `tilt` target sits 0.55 rad above it, so the policy is steered to the angle that actually releases beads for this episode's fill. `rot_speed` free band lowered 1.5 → 1.0 rad/s for a controlled pour with 30 mm beads.
5. **Receiver held still.** New `rcv_still` penalises receiver-cup speed above 3 cm/s while carried (weight 0.3, 1.0 during the pour / post-pour hold) — the operator's `task/rcv_palm_speed`. `upright_rcv` unchanged (phase-gated, doubled during the pour).
6. **Transfer everything.** The success bonus is scaled by `0.6 + 0.4·bead_in_target_frac` and the post-pour hold ramps to 0.8 instead of 0.5, so emptying the cup keeps paying after the 50 % success threshold; `pour_delta`/`spill_delta` stay ungated and per-bead (fractions are per-episode, so few beads → larger per-bead value automatically).

Expected: `task/premature_tilt_rate` → ~0, `task/tilt_far_deg` < 10°, `task/rcv_palm_speed` down, success recovers toward the previous level with beads-in-receiver higher.
