# iter_06 — reward function response

## 1. What the task means and which stages the robot goes through

Two arms each handle their own cup. The right (source) arm pours the beads and the left (receiver) arm holds the empty cup.

1. **Approach**: each palm moves to its own cup.
2. **Grasp**: the source hand forms a wrap grasp (thumb, the four fingers together and the palm all on the cup). The receiver hand grasps the receiver cup.
3. **Lift**: the source cup goes clearly higher (about 20 cm). The receiver cup goes up moderately (about 10 cm) and **stays upright, mouth to the sky**.
4. **Bring together**: the pouring lip of the source cup goes over the receiver mouth. The cups must not touch, the hands must touch only their own cups, and the cups must not nest.
5. **Tilt / pour**: the source cup rotates toward the receiver past about 110°, and the beads fall through the air into the receiver. **The receiver stays at 20° or less** (this is now part of `ctx.success`).
6. **Hold**: after the pour both grasps stay firm. The source hand keeps its palm on the cup instead of pinching it with the fingertips. Nothing is dropped and the arms do not vibrate.

## 2. Feedback analysis

**What converged (old action space, old success rule).** Approach, grasp, lift, bring-together and aim were all solved. At maximum randomisation the policy tilted the source cup to about 105°, `bead/in_target` reached about 0.77 and step-averaged success about 0.80. The previous pour stack (aim 2.2 + tilt 2.8 + pour_pose 2.0 + tilt_dir 0.8 + success 8) worked. That structure is kept. The run restarts from scratch (18-D action space), so every earlier stage must stay learnable.

**Problem A: receiver tilt (observation 4, success change).** The receiver was tilted by a median of 46° while the source was past 80°. `upright_rcv = -0.3·clamp(tilt,0,1)` cost only about −0.24/step at 46°. Tilting the receiver toward the source made aim/tilt/pour_pose easier and was worth several reward units. The reward therefore paid for this, and under the new success rule it is a failure. Two fixes:
- A multiplicative receiver-upright gate `rcv_up = exp(-(max(tilt-0.12,0)/0.20)^2)`. It is 1 below 7°, 0.61 at 15°, 0.27 at 20° and about 0 at 30° or more.
  - `tilt`, `tilt_dir` and `pour_pose` get the full gate, so pour income is only available with an upright receiver.
  - `aim`, `bring_together` and `lift_rcv` get a softened gate (floor 0.3 or 0.5). They keep a positioning gradient while the receiver is still tilted and pay more once it is levelled.
- A stronger additive penalty: `upright_rcv = -1.0·tanh(max(tilt-0.12,0)/0.25)` (−0.22 at 10°, −0.73 at 20°, about −1 at 46°).

**Problem B: source grasp collapse after the pour (observation 2).** The mechanism (knuckle straightening while the tip curls) is gone, because the four fingers now share one closure command. The policy can still loosen the hand once the beads are out. The reward barely noticed the collapse:
- Palm force fell from 7 N to 2.2 N, but `tanh(F/2)` only fell from 1.0 to 0.8, so `wq_src` lost about 0.06.
- `finger_frac` stayed 1 because the fingertips still pressed.

Fixes:
- `PALM_SCALE` goes from 2 N to 3 N: 7 N → 0.98, 2.2 N → 0.63, and the receiver's 3–4 N → 0.78–0.87.
- New `hold_src` / `hold_rcv` terms pay grasp quality once the beads have been delivered (`post_pour` ramps with `bead_in_target_frac` from 0 to 50 %). `hold_rcv` is also gated by `rcv_up`.
- The success bonus is scaled by source grip quality (0.6 + 0.4·wq). The success window is exactly the hold phase, so a loose pinch costs bonus there.

**Problem C: arm vibration (observation 3).** Two source palm-rotation commands switched sign on more than 80 % of steps. `action_rate = -0.02·mean((a-a')^2)` over 42 dims cost about 0.002/step, which is nothing next to a total of about 26. The new EMA filter (0.25) damps it but does not remove it: an alternating ±1 command still gives a ±0.14 target oscillation at 30 Hz, and the policy could use alternation to synthesise mid values.
- New per-arm `palm_rate_*` penalty on the **sum** of squared raw palm command changes: `-(0.02 + 0.18·grasped)·Σ(Δa)^2`.
  - It is bounded by construction (at most 24·w).
  - Before grasping the weight is small (0.02; random exploration costs about −0.1), so approach exploration is not suppressed from epoch 0.
  - Once the hand holds the cup, the observed switching (two dims × about 1.5) costs about −0.8/step. A smooth pour (Δa ≈ 0.1) costs about −0.01.
- Hand commands keep a small separate rate term.

**Exploits checked.**
- The receiver-upright gate cannot be satisfied by leaving the receiver on the table: `stack_gate` still needs both cups lifted.
- Holding the receiver level while the source cup pushes it is still charged by `cup_contact` and `clear`.
- `pour_delta` and `spill_delta` stay ungated, so the bead signal survives.
- `hold_*` pays a level, but only after the beads have left the source cup, which cannot be undone, and only while the grasp is real (`src_grasped`). Finishing the pour earlier therefore earns more hold reward.

**Expected metric movement.**
- The receiver tilt during the pour drops from about 46° to under 20°.
- Success is lower early (the new rule is stricter) and then climbs.
- The src-hand palm force stays high after the pour.
- Palm command sign-switch rate falls. `palm_rate_src` goes from about −0.8 to under −0.1.
- Approach, grasp and lift learning speed stays about the same.
