...
The right hand must grasp the bead-filled source cup and the left hand the empty receiver cup; both cups are lifted off the table; the source mouth is carried over the receiver mouth (source above, cups not touching, not nested); the source is tilted past its fill-dependent release angle so beads fall through the air into the receiver; the receiver is held upright and still; both cups stay held afterwards. Success = at least half the beads in the receiver with little spill, no premature tilt, no nesting, receiver <= 20 deg.

Stages: (1) approach + reach each cup, (2) power-grasp each cup, (3) lift both, (4) converge the mouths in xy while keeping the source mouth above the receiver rim, (5) tilt / pour pose once the mouths are near, (6) bead transfer increments, (7) hold after the pour. Constraints: source upright while far, receiver upright and still, no cup-cup / hand-foreign contact, no drops, smooth commands.

## 2. Analysis of round 11 (iter_10)

Measured: both cups grasped (0.87 / 0.84) and lifted (0.20 / 0.09 m), but the mouths never came closer than 0.29 m; aim, tilt, tilt_dir, pour_pose, success were exactly 0 all run, pour_delta 0.0003, success 0.0. The policy collects ~12/step from grasp-lift-hold terms and the only partially earned approach term is bring_together 0.48.

Why the last 0.29 m is not closed (from the reward as written):

- bring_together's xy shaping is `exp(-3 * mouth_xy)`: 0.42 at 0.29 m and 0.86 at 0.05 m, so closing the full gap is worth only ~0.65/step on a 12/step income. raise_src (0.77) and lift terms pay in full at 0.29 m; there is no ramp that pays for xy convergence before the 5-9 cm near gate.
- Three penalties fire exactly in the band the policy would have to cross: closing_speed is active for cup-centre distance < 0.30 m (the policy parked at 0.29 m, on its boundary) and charges any closing faster than 0.10 m/s; rcv_still (-0.245/step, the largest penalty) charges the receiver for moving faster than 0.03 m/s while carried, so the left arm is paid to freeze where it is; meet_rcv / meet_src charge the receiver for crossing the midline by more than 2 cm and the source by 5 cm, which leaves the hover xy targets almost no room.
- reach_src / reach_rcv sit at 0.99 from epoch 0 and add no gradient (anchors only).

Changes for iter_11 (last round): add a dedicated converge ramp on mouth-to-mouth xy distance, paid at 3.0 from 0.40 m down to 0.06 m and gated on both cups lifted, source upright, receiver upright-ish and the source mouth above the receiver rim (hover_wide); sharpen bring_together's xy factor to exp(-5 d); move closing_speed to inside 0.15 m and a 0.15 m/s threshold; halve rcv_still while carrying far (the pour-phase weight is unchanged, 1.0 near / post-pour); widen the meet free bands (receiver 6 cm, source 10 cm) with lighter weights; lower reach weight to 0.5 (it is saturated) so the standing income shrinks slightly. Pour-stage weights (aim 3, tilt 4, pour_pose 3, pour_delta 50) and all safety terms keep their structure.
