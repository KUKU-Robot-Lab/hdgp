...
The robot must move beads from a cup held in the right hand into a cup held in the left hand, through the air.
Both cups must be grasped and lifted, the source cup's **pouring lip** must be placed above the receiver's
mouth, and only then may the source cup be rotated past its fill-dependent release angle so that the beads
fall out. The receiver cup must stay upright, nothing may be dropped, the cups must not touch or nest, and
the hands must not touch anything except their own cup.

Stages:

1. **Reach** — each palm approaches its own cup on the table.
2. **Grasp** — thumb + fingers wrap each cup (`src_grasped`, `rcv_grasped`), contact freeze produces a power grasp.
3. **Lift** — both cups leave the table (source higher, since it must later be raised above the receiver rim).
4. **Bring together / aim** — the two cups converge near the midline; the source **lip** (not the rim centre)
   is brought over the receiver's mouth in xy, and the source cup is raised so that the lip stays above the
   receiver rim **at the tilt it will eventually reach**.
5. **Tilt** — the source cup rotates toward the receiver. Below `premature_tilt_limit` this is free anywhere;
   above it, the lip must already be within 8.1 cm of the receiver mouth or the episode is latched dead.
   The cup must go all the way past the *release* angle (limit + ~20°, i.e. ~80° at fill 0.76) before a
   single bead leaves.
6. **Pour** — beads leave the source cup and fall into the receiver (`d_in_target` > 0), with no spill.
7. **Hold** — after the transfer, keep both cups held and the receiver upright until the episode ends.

# 2. Analysis of round 16 (iter_15)

Everything up to and including stage 4 is solved: `src_grasped` 0.86, `rcv_grasped` 0.86, src lift 0.199 m,
`aim_dist` 0.126 m, lip distance 0.052 m, collisions all falling. Stage 5 stalls hard at **45 deg** while the
first bead leaves at **~80 deg** for this fill (limit 60.2 deg). `bead/in_target` is exactly 0 for 812 epochs,
`pour_pose` is exactly 0, `hold_*` exactly 0. So the whole reward mass (total 22.9) is earned by the carry
plateau, and the last 35 deg of rotation is never attempted. Three concrete causes:

**(a) Geometric self-contradiction between the lip-height gate and the hover decay.** The lip sits at
`rim centre + r(cosθ·d − sinθ·ẑ)`, r = 4.1 cm, so at 80 deg it is 4 cm **below** the rim centre. `part_rel`
(and `pour_pose`) require `lip_above`, i.e. lip ≥ 2 cm above the receiver mouth — that demands the source cup
be lifted ~6 cm higher than at 45 deg. But `hover_decay` (HOVER_FREE 0.07, σ 0.08) and `lip_high`
(LIP_Z_FREE 0.07) start *shrinking* `raise_src`, `converge`, `bring_together`, `aim` and the tilt income as
soon as the cup goes that high. The policy literally cannot satisfy both gates at once, and `pour_pose` = 0.0
exactly confirms the release pose was never reachable. **Fix: make the height budget a function of tilt** —
the allowed `dz_eq` and lip height grow with `r·sinθ`, so raising the cup while rotating is rewarded instead
of taxed; and `lip_above` becomes a soft gate with a floor so the tilt income does not vanish at the
moment the cup turns.

**(b) The marginal income of the last 35 deg is far too small.** The merged `tilt` term (weight 10, divided by
`tilt_target` ≈ 1.84 rad) pays ~5.4/rad, so 45 → 80 deg was worth ≈ +3.3 in the best case and much less after
the `safe · lip_above` gates — against a ~20-point carry plateau that any grasp flicker or aim loss puts at
risk. The measured slowdown (44.0 → 45.2 deg over the last 2 h) is exactly this. **Fix: split the income
again but with the weight on the far side** — 6.0 spread over 0 → θ_pre, **20.0** concentrated on
θ_pre → release (≈ +7.6 for the last 30 deg, plus a 2.0 overshoot ramp), and cut the carry plateau roughly in
half (approach/reach/grasp/wrap/lift/converge/aim/bring/meet all reduced) so the pour channel dominates once
both cups are carried.

**(c) Nothing pays for the pour pose or the transfer until a bead already moved.** `pour_pose` 3.0 was gated
on the unreachable pose, and `pour_delta` (100) fires only after success of stage 5. **Fix:** `pour_pose` to
8.0 with softened gates, a new `transferred` level term (5.0 on `bead_in_target_frac`, gated on upright
receiver and not nested) so the state of "beads already in the receiver" is worth holding, and `success` 10 → 20.
`spill_delta` stays at 50 and `pour_delta` rises to 120 so increments still dominate the level term.

Other changes: `grip` floor 0.3 → 0.5 (grasp-quality flicker during rotation was taxing every income term),
`safe` radius widened slightly (full within 5 cm, zero at 7.5 cm — still inside the 8.1 cm latch),
`premature_latch` 0.5 → 1.0 because the tilt push is now much stronger, `rot_speed` free band kept.
Terms that were flat and meaningless (`hold_*` at 0, `meet_src` 0.06) are kept but rescaled rather than
dropped, since they only become active in the stages beyond the current stall.

Expected metric movement: `src_tilt_deg` 45 → 70+, `src_cup_lift` 0.20 → 0.26, `pour_pose` > 0,
`bead/in_target` > 0, `premature_tilt_rate` possibly up (watch it; the corridor and the 1.0 latch penalty
are the brake), carry-plateau components roughly halved in value.
