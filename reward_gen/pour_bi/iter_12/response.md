## Analysis of iter_11 feedback

Facts: both cups grasped (0.87 / 0.85) and lifted (0.22 / 0.12 m), success 0, beads transferred 0, src tilt 6 deg.
aim_dist parked at ~0.20 and cups_center_dist at ~0.19 for the last 65 epochs. tilt, tilt_dir, pour_pose, pour_delta
are ~0 for the whole run: the policy never enters the near zone (mouth xy < 9 cm).

Why it parks (derived from the iter_11 code, not measured):
1. Gradient into the gap was not missing (converge + bring_together gave roughly 12 reward per metre), so something
   charged for entering. Inside `near`, rcv_still went from weight 0.25 to 1.0; at the measured receiver palm speed
   (rcv_still = -0.14 at weight 0.25, i.e. tanh term ~0.55) that is an extra ~-0.4/step the moment near opens.
2. What near paid in return was small: an upright source hovering 11-16 cm above the rim (exactly the pose converge and
   hover ask for) scores aim_z = exp(-((0.11-0.04)/0.06)^2) = 0.26, so aim ~0.4 and tilt ~0 until the lip has already
   dropped. Net at the 9 cm boundary is about zero: a plateau.
3. converge (2.0/step) was multiplied by the cup-contact gate, so any brush removed all of it on top of the additive
   -1.0 penalty. cup_collision_rate fell 0.38 -> 0.03 while distance stopped shrinking: the policy learned to stay away.
4. meet_rcv grew to -0.11 while meet_src stayed 0: only the receiver arm travels, and it is taxed for doing so. The
   source arm gets no gradient of its own (the mouth distance is a joint quantity).

Changes (all component names kept for the warm start; count stays 42):
- aim / tilt / pour_pose / pour_phase height factor: max(hover band, lip band) instead of the lip band alone, DZ_SIGMA
  0.06 -> 0.08. The hover pose is now a full-value aim pose and tilting from it pays immediately.
- aim opens on a wide mouth-xy gate (full at 6 cm, zero at 22 cm). tilt, tilt_dir, pour_pose keep the 9 cm near gate, so
  nothing pays tilt beyond the 10 cm premature-latch radius.
- rcv_still full weight only in pour_phase (tilt > ~46 deg in pose) instead of whenever near; free speed 0.03 -> 0.05.
  Operator requirement b (light while carrying far, full during pour) is kept.
- converge: linear in mouth xy (constant slope to 0), contact gate removed (additive cup_contact unchanged).
- bring_together: weight 1.5 -> 3.0, kernel exp(-10 d).
- meet_rcv / meet_src: crossing taxes replaced by positive per-arm income for bringing each cup mouth to the xy midpoint
  of the two spawn positions (1.0 source, 0.75 receiver, linear over 30 cm). Each arm now has a gradient that does not
  depend on the other arm.
Untested: nothing here was run against the environment; only ast.parse was done.
