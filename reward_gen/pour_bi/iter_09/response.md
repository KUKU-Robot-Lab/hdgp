# iter_09 — analysis and reward

## 1. Task and stages

The policy must (1) approach both cups, (2) wrap-grasp each with its own hand (thumb + finger contact, contact freeze), (3) lift both — the source stays upright, the receiver is raised moderately and then held still, (4) carry the upright source until its mouth hovers above and within a few cm (xy) of the receiver mouth, (5) only there lower the pouring lip to the receiver rim and tilt past the fill-dependent release angle so the beads fall through the air into the receiver, (6) hold both grasps after the pour with the receiver still upright. Success is env-judged: enough beads in the receiver, little spill, cups close, receiver ≤ 20°, not nested, no premature tilt (a *grasped* source > 30° while the mouths are > 10 cm apart).

## 2. What the feedback shows

- The run is invalid on the source side and only 159 epochs long. At epoch 1 the old latch fired in 19.8 % of environments from *ungrasped* knock-overs (`premature_latch` −0.099 = 0.198 × 0.5) while `src_grasped` was 0.000. A −0.5/step penalty that could be triggered without the policy's grasp, and could only be removed by staying away from the cup, was optimised exactly that way: latch 19.8 % → 0.06 %, source foreign contact 14.9 % → 2.0 %, source tilt 16.6° → 0.1°, `approach_src` frozen at 0.44 (= exp(−4·d) → the palm parked ~20 cm from the cup), `task/src_closure` 0.12, grasp 0.000 for the whole run. Same mechanism as the round-7 receiver avoidance. The environment now latches only a GRASPED cup, so the trigger the policy was fleeing no longer exists; every remaining source-side penalty (`upright_src`, `premature_latch`) is gated on the cup being held, and the shaping that produced 0.87 source grasp in rounds 6/8 is unchanged, so the source stage is expected to recover without touching its weights.
- The receiver side learned normally with the iter_08 terms: grasp 0.78, `lift_rcv` 1.43 (saturated), receiver 8.9° from upright, `wrap_rcv` 0.29. Three things in that data are worth acting on:
  1. `task/rcv_cup_lift` sat at 0.22 m (0.09–0.39 across checkpoints) — far above the 0.10 m target, because nothing above 0.10 m costs anything. Round 8 was at 0.19 m too. With the source lift capped at 0.20 m this makes the iter_08 hover band (source mouth 11–16 cm above the receiver rim, i.e. `h_src ≥ h_rcv + 0.11`) unreachable: `bring_together` would be identically zero and the whole pour stack unreachable by gradient. This is a latent dead zone in iter_08 that the source failure hid.
  2. `rcv_still` grew to −0.19 as the carry started: with only the 0.3 carry weight active, tanh ≈ 0.63 → the carried receiver was moving at ~0.14 m/s on average, and `rcv_cup_lift` swung between checkpoints. Operator requirement (b) asks for the opposite.
  3. `bead/spill` 0.8 % appeared with the source cup never touched — the spawn/settling floor (item 4). `spill_delta` was charging −30 × Δ for it at the start of every episode, an action-independent negative on the first steps.
- Nothing in the pour stack (`aim`, `tilt`, `pour_pose`, `pour_delta`, `hold_*`, `meet_*`) was ever reached, so those weights are kept as designed in iter_08 (all tilt income gated on the mouth-to-mouth `near`, upright penalty while far) — that design addressed round 8's on-table tilt at 0.26 m and has not yet been tested.

## 3. Changes in iter_09 (everything else is iter_08, unchanged)

1. **`raise_src` (new, 1.0)** — a relative-height ramp on `dz_mouth` (source mouth above receiver rim) from −4 cm → +11 cm, same Gaussian decay above 16 cm as the hover, gated on both cups lifted, not nested, source upright, receiver upright-soft — and NOT multiplied by the xy distance. It gives the missing gradient "put the source mouth above the receiver rim" before the mouths converge; `bring_together` still pays xy convergence only inside the hover band (an upright cup beside the receiver at the same height is still worth nothing, the iter_08 lesson).
2. **`LIFT_TARGET_SRC` 0.20 → 0.25 m** so the absolute lift term keeps its gradient up to the hover height when the receiver sits at 0.10–0.15 m.
3. **Receiver lift band**: `lift_rcv` is full up to 0.15 m and decays above (0.20 → 0.68, 0.22 → 0.46, 0.25 → 0.21). Income shaping, not a penalty, so no avoidance risk; it keeps the receiver in the moderate band the hover geometry and the source arm's reach need.
4. **`bring_together` 1.0 → 1.5** — the xy transport is now the only road to the near-gated pour income; from 0.30 m to 0.05 m the term rises 0.62 instead of 0.41.
5. **`rcv_still` carry weight 0.3 → 0.5** (pour total stays 1.0): 0.10 m/s → −0.22 while carried; a 3 cm/s lift is still inside the deadband and `lift_rcv` (2.0) dwarfs it, so lifting is not discouraged.
6. **`spill_delta` gated by a settling ramp** over the first 5 % of the episode (smoothstep of `episode_progress / 0.05`): the spawn floor is no longer charged; a knock-over later in the episode is charged in full.
7. `premature_latch` stays at −0.5/step: with the env fix it can only fire on a held cup, `upright_src` (−0.92 at 30° on the table, −2.3 carried) already shapes toward it, and the held-cup income (grasp 1.0 + wrap ≤ 1.5 + lift ≤ 2.0) exceeds it, so holding the cup remains net positive even in a latched episode — avoidance is not the cheaper policy any more.

Expected movement: `task/src_grasped` back to ≈ 0.85 (rounds 6/8), `lift_src` > 0, `rcv_cup_lift` settling to 0.10–0.15 m, `rcv_still` shrinking toward the −0.05 range, `raise_src` and `bring_together` rising from 0 once both cups are lifted, then `aim`/`tilt`/`pour_pose` and `bead/in_target` > 0 for the first time; `upright_src` should stay near 0 and `premature_latch` well below the 0.05 level seen at the round-8 tilt-on-table behaviour.
