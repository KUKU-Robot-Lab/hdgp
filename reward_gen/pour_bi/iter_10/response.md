...
## Task and stages (unchanged)

1. Approach: each palm to its own cup (right → source with beads, left → empty receiver); a hand can only close within 0.22 m of its cup.
2. Wrap-grasp both cups (thumb AND finger contact = `*_grasped`), lift both off the table, source kept upright.
3. Carry: source mouth raised above the receiver rim, mouths converge in xy, receiver held level and still.
4. Pour: only once the mouths are within a few cm, tilt the source past the fill-dependent release angle, beads fall into the receiver.
5. Hold: keep both grasps, receiver upright, until the episode ends.

## What the round-9 feedback says

* **Primary bottleneck: the source grasp took 290 epochs to appear** (0.002 at epoch 177 → 0.86 at 546) and left only ~250 epochs for lifting, so the run ended at source lift 0.056 m / tilt 7.6° / mouths 0.44 m apart — every pour-stack term (aim, tilt, pour_pose, pour_delta) stayed exactly 0. Round 8 reached 0.333 m / 70° at the same epoch.
* **Mechanism (checkpoint 150):** the source palm commands were pinned at the workspace limit (lateral −0.99) and the palm retreated from 0.165 m to 0.25 m, outside the 0.22 m closing radius, so the hand could never close. Two rewards paid for that retreat:
  1. `palm_rate_*` is charged on the raw, exploration-noisy actions. Pinning a command at ±1 clips the noise and zeroes the penalty — the receiver arm's `palm_rate_rcv` went −0.53 → −0.12 exactly that way, and once the source cup was held the 0.18 "held" extra cost −0.31/step, more than `lift_src` (+0.29) paid. The same outward saturation was seen in round 8 between epochs 200–558.
  2. At the first checkpoint the clumsy source hand collected `hand_foreign_src` −0.088, `rot_speed` −0.134 (a knocked cup spinning, not a pour) and `cup_contact` −0.026 ≈ −0.25/step, all attached to being near the cup, while `approach_src` (exp(−4d)) only lost ~0.1/step for backing off 8 cm (0.44 hovering vs 0.63 grasped). Backing off was net positive.
* Receiver side is healthy (grasp 0.78, lift ≈ 0.10 target, `lift_rcv` 1.34 largest income); `rcv_still` −0.27 and 19 % foreign contact will only be judged once the source arm reaches the carry phase.
* Later-stage terms (hover band, near gate, fill-adaptive release, latch penalty) have not yet been exercised by a policy that holds the source cup, so they are left untouched.

## Changes (surgical, stage 1–2 only)

1. **`reach_src` / `reach_rcv` (new, +1.0):** smoothstep ramp from 26 cm (0) to 14 cm (1) of palm–cup distance — a strong anchor exactly across the 0.22 m closing radius. Starting at 0.165 m the source hand now earns 0.89 and loses ~0.87/step by retreating to 0.25 m.
2. **Closing income ×5:** `grasp_src` closure part is now `1.0 · exp(−6d) · clamp(closure/0.35)` (closure normalised by the contact-freeze saturation, was 0.5·prox·raw closure ≈ 0.1) so closing the hand at the cup pays 0.5 before the grasp flag exists.
3. **Raw action-rate tax cut:** `palm_rate_*` 0.02 (+0.18 when held) → 0.01 flat, `hand_rate` 0.01 → 0.005. The env EMA already removes command chatter physically; the raw-rate penalty mostly taxed exploration noise and rewarded saturation.
4. **`palm_sat_src` / `palm_sat_rcv` (new, −0.15 max):** penalise palm commands beyond |0.7|, so parking the arm at the workspace limit (target–actual error 0.10 m at epoch 150, 144 N cup–cup peaks) is no longer free.
5. **Contact penalties phase-weighted:** `hand_foreign_*` weight 0.3 while that hand is not holding its cup, 1.0 once it holds/carries; `rot_speed` gated on the source being held (a knocked-over cup is not a pour). Fumbling on the table during exploration stops teaching "avoid the cup"; the real-robot hazards during carry/pour keep the full weight.

Expected movement: `task/src_grasped` should rise within the first ~100 epochs instead of 290, `palm_rate_*` ≈ −0.05 or less, `palm_sat_*` shrinking, then `lift_src` / `both_lifted` / `raise_src` / `bring_together` should leave zero.
