...
Two arms, two cups, one transfer. The right (source) hand must pick up the cup that holds the
beads, the left (receiver) hand must pick up the empty cup, both cups must leave the table, the
source cup's rim must be brought over the receiver cup's rim, and then the source cup must be
rotated past ~110° so the beads fall into the receiver cup. Throughout, the receiver cup has to
stay upright (otherwise the beads fall back out), neither cup may be dropped, and beads that land
outside both cups are lost forever. The environment declares success when at least half of the
beads are in the receiver cup with little spill and the cups are close together.

## Stages the robot must go through

1. **Approach (both arms, in parallel)** — each palm moves toward its own cup. Signal: palm–cup
   distance for each arm. Dense, always on, bounded with `exp(-k d)`.
2. **Grasp (both arms)** — fingers close around each cup; the contact-freeze controller turns a
   closure command into a wrap grasp. Signal: `src_grasped` / `rcv_grasped` (thumb + another
   finger in contact) plus the measured closure, but closure only counts when the palm is already
   near the cup (otherwise closing in the air is paid for nothing).
3. **Lift (both arms)** — raise each cup off the table to ~10 cm. Signal: `cup_z − spawn_z`,
   gated on that hand's grasp so a cup that is knocked upward without a grasp earns nothing.
4. **Align** — bring the source cup's mouth horizontally over the receiver cup's mouth, slightly
   above it. Signal: horizontal distance between the two rim centres and their vertical offset
   (source rim should be 3–12 cm above the receiver rim). Gated on both cups being grasped and
   lifted.
5. **Tilt / pour** — rotate the source cup toward ≥ 2.0 rad while aligned. Signal: `src_cup_tilt`
   gated on alignment; tilting when *not* aligned is penalised because that is exactly how beads
   get spilled on the table.
6. **Transfer** — the actual objective: pay for each *increment* of beads that lands in the
   receiver cup (`d_in_target`), charge for each increment that is lost (`d_spill`). Level-based
   payment is avoided so the policy is not paid for standing still with a filled cup.
7. **Constraints** — receiver cup upright at all times; either cup falling below its spawn height
   (dropped / knocked off) or the receiver cup toppling is penalised; tiny action-rate
   regularisation so the shaping terms, not smoothness, drive exploration.
8. **Success bonus** — a flat bonus on the environment's own `success` flag.

## Weight logic

Per-step shaping is bounded and roughly monotone along the stage chain:
approach (≤1 per arm) → grasp (≤1.5 per arm) → lift (≤2 per arm) → align (≤3) → tilt (≤3).
Each later stage is worth more than the one before it so there is always a gradient toward the
next stage, but no stage can be farmed. Bead transfer is a one-shot payment, so its weight is
large (50 × Δfrac → 2.5 per bead, 50 for the whole cup) and spill is charged at 20 × Δfrac
(−1 per bead) so pouring one bead into the target is worth more than losing two. The success
bonus (10) is paid per step while the env flag is true. Action-rate penalty is deliberately tiny
(0.02 on the mean squared delta) — it should never compete with any stage term.
