...
TESOLLO_RIGHT_SHORT = _dc_replace(
    ...
    #   회전오차 0.0000° · 관절한계 여유 0.312 rad(17.90°)**(dg5f-m 기준선 11.6° 보다 낫다).
    #   palm 이후 손 체인은 두 자산이 동일하므로 palm 이 맞으면 손끝도 맞는다.
    #   ★09.10 어댑터 판(10mm) 삽입 후 재계산했다 — 팔을 덜 뻗어도 같은 palm 포즈에
    #     닿으므로 한계 여유가 11.88° → 17.90° 로 늘었다.
    #   ★★09.10 그 위에 **물체에서 10mm 물러났다**(사용자 확정). 회전은 유지하고
    #     물체축→palm 수평방향 [−0.886, −0.464] 으로 평행이동한 곳을 목표로 다시 풀었다.
    #     기준은 **손가락이 물체 표면에서 3cm 이상**(사용자 지시)이고, 판정 자세는
    #     **감쌈(0.9 rad)** 이다 — 손가락을 움직이는 순간이 물체를 쓰러뜨리는 순간이라서다.
    #     셰이커(shaker_sweep, 최대반경 44mm) 실측 메시 최근접 거리:
    #         후퇴  0mm : 리셋 51.0 / 감쌈 24.7   ← 감쌈에서 3cm 미달
    #         후퇴 10mm : 리셋 52.8 / 감쌈 31.9   ← **채택**(3cm 넘는 최소값)
    #         후퇴 20mm : 리셋 55.6 / 감쌈 40.0
    #         후퇴 40mm : 리셋 59.5 / 감쌈 51.3   ← 컵 기준으로 정했던 구값, 과했다
    #     멀수록 접근 구간만 길어진다(팔 0.15 rad/s). 한계 여유 24.20° 로 셋 중 가장 크다.
    #     ⚠물체를 바꾸면 이 값은 무효다 — cup_big(반경 47mm, 원점 규약 다름) 기준으로는
    #       같은 자세가 감쌈에서 4.4mm 였다. 물체 교체 시 반드시 다시 잴 것.
    arm_reset_joint_pos=(-0.0729, 0.2479, -0.0574, 0.8405, 0.2238, 0.1198, 0.7866),
    init_joint_pos={
...
TESOLLO_RIGHT_SHORT_TL = _dc_replace(
    ...
    hand_action_limit_override={
        ...
        # ★★09.11 — `thumb_2`·`pinky_2` 는 `hand_open_pose` 와 `hand_grip_pose` 가 **같다**
        #   (엄지 대향 −1.57 · 새끼 뿌리 0.0). 즉 설계상 과제 내내 **움직이지 않는 관절**인데
        #   액션 자유도로 열려 있었고, `hand_curl`(액션한계 정규화 평균)이 이 둘을 통해
        #   **거꾸로 보상**했다:
        #     thumb_2 범위 [−2.7053, 0], 설계 −1.57 → 정규화 0.420. **0 으로 밀면 1.000**.
        #     pinky_2 범위 [0, 1.5708], 설계 0 → 정규화 0.000. **굽히면 1.000**.
        #   10칸 중 2칸이라 curl 을 최대 +0.158 까지 **손가락을 전혀 안 굽히고** 올릴 수 있다.
        #   fj_g1/g2 영상에서 엄지가 대향을 놓고 아래로 흔들린 것이 이 경사다.
        #   ★폭 0 은 부팅 가드가 죽인다(`폭 0 액션 칸`) — `_1` 들과 같은 ±0.01 규약으로 묶는다.
        r"r_hj_thumb_2$": (-1.5808, -1.5608),
        r"r_hj_pinky_2$": (-0.01, 0.01),
    },
