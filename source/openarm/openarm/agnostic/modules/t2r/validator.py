...
def make_fake_context(n: int = 16, *, num_fingers: int = 5, num_arm: int = 7,
    ...
    kw.update(bead_in_source_frac=fr[:, 0], bead_in_target_frac=fr[:, 1], bead_spill_frac=fr[:, 2],
              ...
              d_spill=r(n, lo=-0.1, hi=0.1), cups_nested=r(n) > 0.7, premature_tilt=r(n) > 0.9,
              src_pour_lip_pos=r(n, 3, lo=0.0, hi=0.6), src_tilt_toward_rcv=r(n, lo=-3.14, hi=3.14),
              premature_tilt_limit=r(n, lo=0.9, hi=1.5),
              cup_hit=r(n) > 0.8, pour_dir_xy=torch.nn.functional.normalize(r(n, 2, lo=-1.0, hi=1.0), dim=1),
              rcv_side_margin=r(n, lo=-0.3, hi=0.2), src_wrap_count=torch.floor(r(n, lo=0.0, hi=5.99)),
              rcv_wrap_count=torch.floor(r(n, lo=0.0, hi=5.99)),
              bead_fill_level=r(n, lo=0.0, hi=1.0), success=r(n) > 0.8,
