...
★09.15 사용자 "컵에 다가가는 palm_ee_x · 손가락 방향(palm_ee_z)" — fj_stage_i00 이 시작부터 손을 ~68° 돌려 손바닥을 컵 쪽으로
  맞추느라 접근이 늦었다(`approach_face` e1→e17 0.19→0.36 · 간극 0.255→0.278 m). 리셋 자세가 이미 법선 +y · 손가락 +x 라
  접근 래치에 손 방향(시작 자세 그대로 · 컵의 −y 쪽)을 넣는다.
...
#: 접근 완료 — 컵 축 → 손바닥 중심의 수평 방향(손바닥이 컵의 어느 쪽에 있나)
APPROACH_SIDE_DIR = (0.0, -1.0, 0.0)
#: 위 세 방향 각각의 cos 하한(≈ 45°)
...
APPROACH_CONDITIONS = ("gap", "facing", "orient", "pose")
...
def _radial_unit(palm_pos: torch.Tensor, cup_pos: torch.Tensor, cup_axis: torch.Tensor) -> torch.Tensor:
    """컵 축 → 손바닥, 축에 수직인 단위벡터 (N,3)."""
    v = palm_pos - cup_pos
    radial = v - (v * cup_axis).sum(-1, keepdim=True) * cup_axis
    return radial / radial.norm(dim=-1, keepdim=True).clamp(min=1e-6)
...
def palm_facing(palm_pos: torch.Tensor, palm_normal: torch.Tensor, cup_pos: torch.Tensor,
    ...
    return -(palm_normal * _radial_unit(palm_pos, cup_pos, cup_axis)).sum(-1)


def hand_orientation(palm_pos: torch.Tensor, palm_normal: torch.Tensor, finger_dir: torch.Tensor,
                     cup_pos: torch.Tensor, cup_axis: torch.Tensor) -> torch.Tensor:
    """min(법선·+y, 손가락·+x, (컵 축 → 손바닥)·−y) (N,). 1 = 시작 자세 방향 그대로 컵의 −y 쪽에 있다."""
    def _dir(d: tuple[float, float, float]) -> torch.Tensor:
        return torch.tensor(d, dtype=palm_pos.dtype, device=palm_pos.device)

    side = (_radial_unit(palm_pos, cup_pos, cup_axis) * _dir(APPROACH_SIDE_DIR)).sum(-1)
    normal = (palm_normal * _dir(APPROACH_PALM_NORMAL_DIR)).sum(-1)
    finger = (finger_dir * _dir(APPROACH_FINGER_DIR)).sum(-1)
    return torch.minimum(torch.minimum(side, normal), finger)
...
def approach_conditions(*, gap: torch.Tensor, facing: torch.Tensor, orient: torch.Tensor,
                        pose_dev: torch.Tensor) -> torch.Tensor:
    """접근 조건별 통과 (N,4) bool — 열 순서 = `APPROACH_CONDITIONS`."""
    return torch.stack([gap <= APPROACH_GAP_M, facing >= APPROACH_FACING_MIN,
                        orient >= APPROACH_ORIENT_MIN, pose_dev <= APPROACH_POSE_TOL], dim=1)


def update_gates(approach: torch.Tensor, envelope_count: torch.Tensor, envelope: torch.Tensor, *,
                 gap: torch.Tensor, facing: torch.Tensor, orient: torch.Tensor, pose_dev: torch.Tensor,
    ...
    """
    approach_new = approach | approach_conditions(gap=gap, facing=facing, orient=orient, pose_dev=pose_dev).all(dim=1)
