"""생성된 보상 코드 검증 (grasp_fj_t2r 포크) — 정적 검사 + 가짜 RewardContext 드라이런, Isaac 불요.
...
NUM_FINGERS, NUM_LINKS, NUM_HAND, NUM_ARM, NUM_ACTIONS = 5, 3, 19, 7, 26
...
def make_fake_context(n: int = 16, *, device: str = "cpu", seed: int = 0) -> RewardContext:
    """형태만 맞춘 무작위 ctx. 물리적 정합은 보장하지 않는다(드라이런 전용)."""
