"""Loader of generated stage-1 reward code (IKER t2r fork).

파일 계약:
    def compute_reward(ctx: RewardContext) -> tuple[torch.Tensor, dict[str, torch.Tensor]]
        return total (N,), {"term_name": (N,), ...}

★`exec` 로 읽되 이름공간은 `torch`·`math`·`RewardContext` 뿐이다(text2reward 와 같은 방식).
  검증은 validator 가 먼저 한다 — 로더는 서명과 반환 형태만 강제한다.
★경로가 비면 **영 보상**(terms 없음) — 부팅·무작위 롤아웃 전용.
"""
...
def load_reward_fn(path: str | None) -> tuple[RewardFn, str]:
    ...
    ns: dict = {"torch": torch, "math": math, "RewardContext": RewardContext,
                "__name__": "iker_t2r_generated_reward"}
    exec(compile(src, path, "exec"), ns)   # noqa: S102 — 생성 코드는 validator 가 먼저 거른다
...
def call_reward_fn(fn: RewardFn, ctx: RewardContext) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    ...
    if not isinstance(total, torch.Tensor) or tuple(total.shape) != (n,):
        raise RuntimeError(f"{ENTRY_NAME} 의 reward 는 (N,)=({n},) 텐서여야 한다: "
    ...
    for k, v in terms.items():
        if not isinstance(v, torch.Tensor) or tuple(v.shape) != (n,):
            raise RuntimeError(f"{ENTRY_NAME} terms['{k}'] 는 (N,) 텐서여야 한다: "
