"""Validation of generated UNIFIED reward code (IKER t2r fork) — static check + fake RewardContext dry run, no Isaac.
...
NUM_ARM, NUM_HAND, NUM_ACTIONS, NUM_KEYPOINTS = 7, 20, 26, 4
NUM_FINGERS, NUM_LINKS = 5, 3
...
def make_fake_context(n: int = 16, *, device: str = "cpu", seed: int = 0) -> RewardContext:
    ...
    return RewardContext(
        ...
        episode_steps=360, control_dt=0.1,
        lift_height=grasp.lift_height_m, hold_radius=grasp.hold_radius_m, hold_slip_speed=grasp.hold_rel_speed,
        thumb_curl_min=grasp.thumb_curl_min_rad, latch_steps=grasp.latch_steps,
        ...
        link_shoe_gap=r(n, NUM_FINGERS, NUM_LINKS, lo=0.0, hi=0.2),
        link_shoe_force=r(n, NUM_FINGERS, NUM_LINKS, lo=0.0, hi=5.0), palm_shoe_force=r(n, lo=0.0, hi=5.0),
        hand_q=r(n, NUM_HAND), hand_qd=r(n, NUM_HAND), hand_q_norm=r(n, NUM_HAND, lo=0.0, hi=1.0),
        hand_target_norm=r(n, NUM_HAND, lo=0.0, hi=1.0), thumb_curl=r(n, lo=-0.2, hi=1.0),
        ...
        palm_gap=r(n, lo=0.0, hi=0.2), palm_shoe_dist=r(n, lo=0.0, hi=0.3), slip_speed=r(n, lo=0.0, hi=0.4),
        ...
        held=r(n) > 0.0, hold_count=torch.floor(r(n, lo=0.0, hi=25.0)), latched=r(n) > 0.0, carried=r(n) > 0.0,
        placed=r(n) > 0.0, released=r(n) > 0.0, resting=r(n) > 0.0, still=r(n) > 0.0, home=r(n) > 0.0,
        stable_count=torch.floor(r(n, lo=0.0, hi=25.0)), success=r(n) > 0.8, start_held=r(n) > 0.5,
