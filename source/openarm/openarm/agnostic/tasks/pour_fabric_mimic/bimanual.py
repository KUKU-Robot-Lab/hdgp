"""BimanualPair — RobotProfile 두 개(좌/우)를 하나의 양팔 태스크 뷰로 합성한다 (pour_fabric_mimic 판).

★원본 `pour_fabric/bimanual.py` 과 다른 점: 프로필 출처가 모듈 레지스트리가 아니라 이 트랙의
  `robot_profiles.PROFILES`(RH56F1 Fabrics 변형)이고, fabric 26 DOF 슬라이스 헬퍼가 있다.
...
from . import robot_profiles as _local
...
def _build_pairs() -> None:
    ...
    for p in _local.PROFILES.values():
...
# =============================================================================
# 양팔 26 DOF fabric 의 측별 슬라이스 — RH56F1 fabric 클래스 상수와 같은 값
# (`openarm_rh56f1_pose_fabric.py`: [0:7]=r_arm [7:13]=r_hand [13:20]=l_arm [20:26]=l_hand).
# =============================================================================
FABRIC_NUM_JOINTS = len(_local.FABRIC_JOINT_ORDER)


def fabric_slots(profile: _rp.RobotProfile) -> tuple[slice, slice]:
    """(팔 슬라이스, 손 슬라이스) — 프로필 측(r|l)과 관절 수로 유도. 리터럴 인덱스 금지."""
    a, h = profile.num_arm_joints, profile.num_hand_joints
    base = 0 if side_of(profile) == "r" else a + h
    return slice(base, base + a), slice(base + a, base + a + h)
