...
(0 = upright, π/2 = horizontal). The source cup holds a fixed number of beads in this track. \
The first bead leaves at about 70° with the cup full, and a full cup has lost a fifth of its \
beads by 80° and half by 86°. `ctx.src_cup_mouth_pos` and `ctx.rcv_cup_mouth_pos` are the \
rim centres.
...
20° — and the cups NOT nested). You may add a bonus on it but you cannot redefine it. \
Beads only count as "in the receiver" once they have LEFT the source cup — pushing \
