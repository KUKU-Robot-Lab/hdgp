"""grasp_ua — fabric 제어 기반 파지·리프트·이송 환경 설정.
...
@configclass
class GraspUAEnvCfg(DirectRLEnvCfg):
    ...
    **같은 자리 warm(D1, B1 ep_12000 인계) 대조**: success 0.185 · min 0.170 ·
    ...
    #   실효성 확인: `grasp_ua_control.py` 는 `set_joint_position_target` 으로 **목표만**
    ...
    events: GraspUAEventCfg = GraspUAEventCfg()
    ...
    def _assert_gain_branch(self, profile) -> None:
        ...
        if bool(self.use_real_gains) != _real:
            ...
            raise RuntimeError(
                f"[grasp_ua] 게인 분기 불일치 — cfg 는 {_want} 을 요구하는데 "
                f"프로필은 {_got} 으로 조립됐다.\n"
                "  환경변수로 맞춰라: "
...
@configclass
class GraspUATesolloRightEnvCfg(GraspUAEnvCfg):
    profile_name: str = "tesollo_right"


@configclass
class GraspUARh56f1RightEnvCfg(GraspUAEnvCfg):
    """Inspire RH56F1 우손 — 언더액추 손(구동 6 / 물리 12).

    ★여기 세 값이 "저자유도·언더액추 손이라서 달라지는 전부"다. 나머지는 프로필이
      흡수한다. 값 자체는 전부 **기존 스위치**이고 새 메커니즘이 아니다.
    """

    profile_name: str = "rh56f1_right"
    # ★구동관절 6개 = 액션 슬롯 6개. 기본 `coupled3`(채널×손가락)로 두면 4지의 채널1이
    #   아무 관절에도 안 걸려 액션 4개가 통째로 죽는다(프로필 `hand_finger_channels` 참조).
    hand_layout: str = "per_finger"
    # ★★감쌈 판정 분기. `synergy_contact_freeze` 의 "닿은 마디만 얼리고 원위가 계속
    #   감는다"는 드레이프 생성은 **손가락당 구동관절이 2개 이상일 때만** 성립한다.
    #   RH56F1 은 하나뿐이라 관절 동결 = 그 손가락 정지이고, 감쌈은 하드웨어 결합
    ...
    use_hand_repulsion: bool = False
    # ★★자세 노브 끄기. `oppose_grip_delta_rad` 는 "대향 손가락의 채널1 관절"을
    #   고치는데, 그 채널이 가리키는 관절이 로봇마다 다르다 —
    #     tesollo : ch1 = `r_hj_thumb_2` = **대향(opposition)** 관절
    #     RH56F1  : ch1 = `r_hj_thumb_2` = **굴곡** 드라이브(한계 [0, 0.475])
    #   RH56F1 에 −0.6 을 얹으면 grip 이 한계 밖으로 나가 0 으로 잘리고 엄지 굴곡이
    #   영영 안 움직인다(09.02 프로브 실측: 300스텝 폐쇄 지령에도 thumb_2 = 0.0001).
    ...
    #   놓이면 손가락이 컵과 팔 사이에서 짓이겨지고 관절이 한계 밖으로 밀린다.
    #   랜덤 탐색은 그걸 매 스텝 한다.
    #   09.02 실측(random 1024 env 200스텝, 최악값):
    #        0.020 m/step(1.2 m/s) → mimic_err 665 · 관절오차 598 · 씬 발산
    #        0.005 m/step(0.3 m/s) → mimic_err 8.8 · 관절오차 7.7 · palm_to_cup 0.17 유지
    ...
    #     앵커 = 스폰 + (−0.070, −0.035, +0.097)
    #     → 케이지 = 스폰 + (0, 0, 0.080) = 파지중심(스폰+0.03) + 0.050 m
    #   tesollo 값(−0.066, −0.022, +0.085)은 그 손의 케이지 오프셋에 맞춘 것이라
    #   여기 쓰면 케이지가 파지중심보다 110mm 위에 뜬다(델타 0.10 으로 못 내려온다).
    palm_anchor_offset_xyz: tuple = (-0.070, -0.035, 0.097)


@configclass
class GraspUAGripperLeftEnvCfg(GraspUAEnvCfg):
