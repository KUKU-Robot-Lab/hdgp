...
RH56F1_RIGHT = RobotProfile(
    ...
    #   ★★09.02 open 을 하한에서 **띄운다**(0.00 → 0.05). 구동관절 하한이 정확히
    #     0.0 이라 open=0 이면 손이 관절한계에 걸터앉은 채 시작한다 — 접촉이 조금만
    #     들어와도 즉시 한계 밖으로 나가고, PD 는 그 지점에서 복원력이 0 이다.
    #     실측: `r_hj_thumb_2` 가 −0.2469 까지 밀렸다(하한 0).
    hand_open_pose=(1.57, 0.05, 0.05, 0.05, 0.05, 0.05),
