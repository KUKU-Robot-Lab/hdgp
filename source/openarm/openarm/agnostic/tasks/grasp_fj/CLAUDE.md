...
| 손 액션한계 [lo, hi] = URDF 한계(soft = 하드, factor 1.0 을 부팅 대조) ∩ 프로필 `hand_action_limit_override` — 규칙 "**하한 = open 자세**": 네 손가락 `_3/_4` + 엄지 `_4` 하한 0, 엄지 `_3` 하한 −0.5(pre-curl open), 나머지 URDF 전폭. ⚠`thumb_2` 는 외전이 아니라 **대향** [−π, 0] 전폭(a=0 = 현 고정값 −1.571) — 좁힐지 별도 확정 | SHARPA 는 PIP/DIP 하한 0 이라 원시 한계가 안전했지만 테솔로 `_3/_4` 는 ±1.571 대칭 — a=−1 이 손등 −90°(08.23 exploit). 접촉 항 0개라 **범위가 유일한 방어선**. 부팅에서 정규식 미매칭(fullmatch)·폭 0 칸·리셋 자세 범위 밖을 죽인다 |
