...
class GraspS2REnvCfg(DirectRLEnvCfg):
    ...
    # ---- 파지 기하 (approach 항의 대향 목표점) ----------------------------------------
    # ★★이 트랙의 **유일한 형상 의존 상수**다. grasp_v1 의 `fingertip_side_dist` 는
    #   "엄지는 컵 축 한쪽, 나머지는 반대쪽" 대향점까지의 거리라 컵 반경이 필요하다.
    #   cup_big 실측(접촉 중앙 59~65mm). 다물체로 확장할 때 여기부터 물체별 값으로
    #   바꾸거나(object_bank 의 base_grasp_radius), 손 기하 파생으로 대체한다.
    object_grasp_radius: float = 0.062
    object_grasp_z_offset: float = 0.03      # 물체 원점 ↔ 파지 높이
    enclosure_thumb_weight: float = 0.6      # 대향 거리에서 엄지 가중
