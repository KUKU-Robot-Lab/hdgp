...
class GraspS2REnvCfg(DirectRLEnvCfg):
    ...
    **같은 자리 warm(D1, B1 ep_12000 인계) 대조**: success 0.185 · min 0.170 ·
    ...
    # ★★★09.11 F1 — **리미터 해제(0.0 = 통과)**. 정책 → fabric 다이렉트.
    #
    # 왜. 09.11 실측에서 리미터가 **98.4% 포화**였다(`cmd_step_raw` 0.144 m/step vs
    #   상한 0.02 = 요청의 7.2배). 리미터는 노름 스케일이라 방향만 보존하고 크기를
    #   버린다 → 정책이 내는 **크기 정보가 제어에 전달되지 않았다**. 포화 구간에서는
    #   ∂결과/∂|a| = 0 이라 "덜 요청하라"는 학습 신호도 없다.
    #
    # 구조. 리미터가 fabric **앞**에 있어서 fabric 은 정책의 요청을 본 적이 없다.
    #   fabric 은 비례 저역통과(멀면 세게·가까우면 약하게)라 크기를 실어 나르는데,
    #   그 앞에 경성 포화를 두어 같은 일을 먼저, 정보를 버리는 방식으로 하고 있었다.
    #   DEXTRAH 는 `scale(a, lo, hi) → tensor_clamp → set_features` 가 전부다 —
    #   리미터·앵커·EMA·이전지령 상태가 **하나도 없다**(소스 확인).
    #
    # 안전 근거(사용자 판단 + 수치). DEXTRAH palm 박스는 x1.2×y1.4×z1.0 m 라
    #   스텝당 목표 점프가 최대 **2.10 m** 인데 실기에서 리미터 없이 돈다.
    #   우리 델타 박스 ±0.10 은 점프 최대 **0.346 m** 로 **6.1배 작다**.
    #   ★델타 박스는 더 줄일 수 없다 — 앵커가 스폰+(−0.066,−0.022,+0.085)라
    #     파지(앵커 −0.085)와 리프트(앵커 +0.035)를 모두 담으려면 z 는 ≥0.085 가
    #     필요하다. 즉 "액션 크기를 줄여서 따라가게 한다"는 이미 충족된 상태다.
    #
    # 배포 정합. `sim2real/scripts/grasp_s2r_palm_command.py:_rate_limit` 은
    #   `limit <= 0.0` 이면 target 을 그대로 통과시키고, 그 값을 런 dump 에서 읽는다
    #   (`scalar("palm_cmd_rate_limit_m", 0.02)`). 따라서 **배포가 자동으로 따라온다** —
    #   계약을 깨지 않는다. ⚠단 dump 에 키가 없으면 폴백 0.02 이므로 dump 를 볼 것.
    #
    # ⚠08.27 위험. 리미터 없이 **게이트 사다리** 보상이면 랜덤워크가 된다(그때 실패).
    #   지금은 E1 이 dense 쪽으로 옮겨 놨다 — 수입의 75%가 `enclosure`(접촉 floor 0.3
    #   블렌드)와 `transfer`(exp) 로 전 구간 0 이 아니다. 같은 구성이 아니지만
    #   같은 위험 축이므로 `diag/*` 로 감시한다.
    ...
    # ★★09.10 E1 — W1 활성(0.0 → 5.0 / 0.5). 과제 정의가 "다양한 무게, **흔들림**에서
    #   파지를 잃지 않는 것" 이고, 형제 `grasp_fj` 는 자기 CLAUDE.md 에서 이 항을
    #   "파지 품질을 강제하는 **유일한** 장치" 라고 못박는다.
    # ★질량 정규화(N/kg)라 위 질량 DR (0.5~2.5배)과 **간섭하지 않는다** — 힘/무게 비가
    #   전 구간 0.81 로 일정하다(E|F| = m·5.0·1.5958 vs mg).
    # ⚠W1 의 원 audit 근거는 D3 수입비(래치후 44.3 vs 래치전 8.3 = 5.3배)였는데
    #   E1 이 그 비를 바꿨다: 명목 3.95배 → **2.30배**(마진 1.72배 축소).
    #   여전히 2.3배라 "안 들면 외란 없음" 회피가 이득이 되지는 않지만, 이 값을 올릴
    #   때(W2 = 20/2.0)는 그 비를 **다시** 계산할 것 — 임계는 자기가 재는 양의 범위가
    #   움직이면 의미가 달라진다(관측 0161).
    wrench_force_scale: float = 5.0           # N / kg   (W1)
    wrench_torque_scale: float = 0.5          # N·m / kg (W1)
