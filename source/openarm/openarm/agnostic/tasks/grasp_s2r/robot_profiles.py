...
TESOLLO_RIGHT_SHORT = _dc_replace(
    ...
    #   ★★09.10 그 위에 **컵에서 40mm 물러났다**(사용자 지시 "너무 가깝다"). 회전은 유지하고
    #     컵축→palm 수평방향 [−0.886, −0.464] 으로 평행이동한 곳을 목표로 다시 풀었다.
    #     이유: 물러나기 전에는 **손가락을 17° 만 굽혀도 컵에 닿고 34° 에서 7mm 파고들었다**
    #     (FK 실측). 감쌈 자세(0.9 rad)에서는 최소 −22.4mm 관통이라 인벨롭이 기하학적으로
    #     불가능했다. 40mm 물러나면 감쌈 자세에서 +11.9mm 여유가 남는다.
    #     한계 여유는 22.12° 로 더 늘었다. 물러나기 전 값은
    #     (−0.0330, 0.1378, 0.1625, 0.8157, −0.0097, 0.2097, 0.7680) 였다.
    arm_reset_joint_pos=(-0.1679, 0.2115, 0.1202, 0.8958, 0.0936, 0.2219, 0.8097),
    init_joint_pos={
