...
class GraspRightEnv(DirectRLEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # firm envelope(07.21): tip(distal)+근위(middle) 두 마디 동시접촉 손가락 수.
        # 손끝만 닿는 shallow 파지와 구분 — slip 방지 firm 성공 게이트(success_flag)에 사용.
        self.num_firm_fingers = (
            self.distal_binary_contact_buf & self.middle_binary_contact_buf
        ).sum(dim=-1)
        height_delta = (
        ...
        # palm orientation(07.22, approach 하위): 손바닥 법선(palm +z)↔palm→물체 방향 정렬.
        # approach 앞에 계산해 approach 에 곱한다 — orientation 독립 term 은 "멀리서 손바닥만 겨냥"
        # local min 유발(palm4 실측 palm_dist 0.27 정체) → 접근이 주, 정렬은 접근 방향 보정(사용자 지시).
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # success bookkeeping(07.21 firm envelope): goal 도달 + firm(tip+근위 K개 동시접촉) + 유효 물체.
        # ADR firm_success(min_fingers 2→4)로 K 상승. ADR 트리거 in_success_region 은 불변(붕괴 회피).
        valid_cup = ~(out_x | out_y | fallen)
        _k_firm = (
            int(round(float(self.grasp_adr.get_param("firm_success", "min_fingers"))))
            if self.grasp_adr is not None
            else int(self.cfg.success_min_firm_fingers)
        )
        firm_grip = self.num_firm_fingers >= _k_firm
        self.success_flag.copy_(self.in_success_region & firm_grip & valid_cup)
