...
class GraspLeftEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: GraspLeftEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.right_arm_dof_indices: list[int] = []
        for name in cfg.right_arm_joint_names:
            if name in self.robot.joint_names:
                self.right_arm_dof_indices.append(self.robot.joint_names.index(name))
        ...
        self._hand_mimic_dof_indices = [
            self.robot.joint_names.index(name) for name in LEFT_HAND_MIMIC_JOINT_NAMES
        ]
        ...
        _tip_names = ["l_hl_thumb_4", "l_hl_index_2", "l_hl_middle_2", "l_hl_ring_2", "l_hl_pinky_2"]
        self.fingertip_body_indices: list[int] = [
            self.robot.data.body_names.index(name) for name in _tip_names
        ]
        _palm_name = "l_hl_palm_sensor"
        ...
        right_vals = [
            ...
            for idx in self.right_arm_dof_indices
        ]
        self.right_arm_zero_pos = (
            to_torch(right_vals, device=self.device)
            .unsqueeze(0).repeat(self.num_envs, 1)
        )
        self.right_arm_zero_vel = torch.zeros(
            self.num_envs, len(self.right_arm_dof_indices), device=self.device
        )
        ...
        # l_aj_7 bias 도 접근 자세별 (side=cfg 기본값, top-down=전용값). 07.13 이식.
        self.pregrasp_l_aj7_bias_by_pose = to_torch(
            [cfg.pregrasp_l_aj7_bias, PREGRASP_R_AJ7_BIAS_TOPDOWN], device=self.device,
        )
        ...
        # ----------------------------------------------------------------
        # Distillation (Dagger 계약). teacher 학습(distillation=False)엔 미사용.
        # ----------------------------------------------------------------
    ...
    def _setup_scene(self) -> None:
        ...
        _DISTAL_LINKS = ["l_hl_thumb_4", "l_hl_index_2", "l_hl_middle_2", "l_hl_ring_2", "l_hl_pinky_2"]
        _MIDDLE_LINKS = ["l_hl_thumb_2", "l_hl_index_1", "l_hl_middle_1", "l_hl_ring_1", "l_hl_pinky_1"]
    ...
    def _setup_geometric_fabrics(self) -> None:
        ...
        self.fabric = OpenArmRh56f1LeftPoseFabric(
        ...
        self._reset_fabric = OpenArmRh56f1LeftPoseFabric(
    ...
    def _apply_action(self) -> None:
        ...
        # ---- 왼팔: 고정 자세 ----
        self.robot.set_joint_position_target(
            self.right_arm_zero_pos, joint_ids=self.right_arm_dof_indices
        )
    ...
    def compute_student_policy_observations(self) -> torch.Tensor:
        """student obs (116) — 물체 privileged state 없음(RGB 에서 추론). 좌팔 fabric 슬라이스."""
        student_obs = torch.cat([
            ...
            self.object_goal,                # 3
            self.actions,                    # 12
            self.fabric_q[:, NUM_ROBOT_DOF:],    # 13 (좌팔)
            self.fabric_qd[:, NUM_ROBOT_DOF:],   # 13
            self.fabric_qdd[:, NUM_ROBOT_DOF:],  # 13
        ], dim=-1)   # 116
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # 컵 전용 upleft 가정이 부당. 종료 = 물체 이탈/낙하 + timeout.
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None) -> None:
        ...
        if self.demo_grasp_reset_bank is not None:
            ...
            # l_aj_7(손목, arm index 6)을 낮춰 palm을 물체 높이로 내림. fabric은 +y 수평
            # 유지 위해 l_aj_7을 높게 잡아 palm이 물체 rim에 뜨므로(probe 확정) bias로
            # 끌어내린다. bias 후 실제 palm(FK)로 anchor를 정합해 정책 시작 시 palm 튐 방지.
            # 접근 자세별 bias(side 0.3 / top-down 0.6, 07.13 이식) — per-env 적용.
            aj7_bias = self.pregrasp_l_aj7_bias_by_pose[pose_id]
        ...
        full_pos[:, self.right_arm_dof_indices] = self.right_arm_zero_pos[0]
        if _sp_vel > 0.0:
