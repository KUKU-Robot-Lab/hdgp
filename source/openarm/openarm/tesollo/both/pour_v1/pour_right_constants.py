...
# Observation space
# ---------------------------------------------------------------------------
# ★[both/pour_v1] 왼팔 obs 를 arm 7관절로 줄여 55 → 51.
#   구 pour_sensor 는 왼팔이 arm7+그리퍼2=9 였다. pour_v1 왼손은 DG-5FS 20관절인데
#   그건 warm 파지자세로 **동결**되므로(정책 미제어) 관측에 넣을 행동 정보가 없다.
#   ⚠ pour_sensor 체크포인트는 이 차원 변경으로 호환되지 않는다 — 로봇·물리도 다르므로
#     애초에 fresh 학습이 전제다. pour_v1 내부의 frozen→learned 인계는 **action 15D 불변**이
#     조건이고 그것은 유지된다.
...
NUM_CRITIC_EXTRAS = 35   # left_arm(14: arm7 pos+vel)+distal(10)+cup_h(1)+rho(1)+demo(9)
