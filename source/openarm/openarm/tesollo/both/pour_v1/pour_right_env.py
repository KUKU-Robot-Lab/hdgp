...
from .pour_right_preset import (
    ...
    RIGHT_HAND_BODY_NAME_CANDIDATES,
...
# ★[both/pour_v1] warm 캐시가 나왔어야 하는 로봇 자산. 캐시에 `robot_usd` 출처가 기록돼
#   있고 이와 다르면 로더가 hard fail 한다 (기록이 없으면 경고만).
_EXPECTED_ROBOT_USD = "openarm_tesollo_bi_s_rl"
...
class PourRightEnv(DirectRLEnv):
    ...
    def _update_spout_z_lock_ref(self) -> None:
        """z-lock 기준 높이(env-local)를 1차 저역통과로 갱신한다.

        ★[both/pour_v1] `pour_spout_z_lock` 은 주둥이 z 를 receiver 입구 높이 + margin 으로
          **해석적으로 강제**한다. 구 pour_sensor 에서는 receiver 컵이 kinematic 이라 이 기준이
          준정적이었지만, pour_v1 은 왼손이 그 컵을 물리적으로 쥐고 있어 흔들린다.
          기준을 그대로 쓰면 왼컵 진동이 오른팔 목표로 직결되어 양팔이 서로를 흔들 수 있다.
          `pour_spout_z_lock_lpf_alpha=1.0` 이면 필터가 사실상 무효(구 동작과 동일).
        """
        raw = self._target_opening_w[:, 2] - self.scene.env_origins[:, 2]
        alpha = float(self.cfg.pour_spout_z_lock_lpf_alpha)
        if alpha >= 1.0:
            self._spout_z_lock_ref = raw
            return
        # 리셋 직후(미초기화 env)는 raw 로 즉시 시작 — 램프인 지연이 초기 조준을 망치지 않게.
        fresh = ~self._spout_z_lock_ref_valid
        prev = torch.where(fresh, raw, self._spout_z_lock_ref)
        self._spout_z_lock_ref = alpha * raw + (1.0 - alpha) * prev
        self._spout_z_lock_ref_valid |= fresh

    def _resolve_body_index(self, candidates: Sequence[str], *, label: str) -> int:
        """후보 이름을 우선순위대로 찾아 body index 를 돌려준다. 하나도 없으면 예외.

        ★[both/pour_v1] 기존 코드는 `name in body_names else -1` 형태였다. -1 은 파이썬에서
          **마지막 body** 를 뜻하므로, 이름이 바뀌면 에러 없이 엉뚱한 링크를 제어/관측하게 된다.
          DG-5FS 마운트 체인은 fixed 조인트가 4개(mount→adapter→base→palm→palm_alias)라
          Isaac 임포트에서 링크가 병합될 여지가 크므로, 조용한 실패를 명시적 실패로 바꾼다.
        """
        ...
        raise RuntimeError(
            f"[pour_v1] {label} body 를 찾을 수 없다. 후보={list(candidates)} / "
            f"사용 가능한 body={names}"
        )
    ...
    def __init__(self, cfg: GraspRightEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # ★[both/pour_v1] 구 코드는 이름이 없으면 -1(=마지막 body)로 조용히 빗나갔다.
        #   DG-5FS 는 마운트 체인이 fixed 조인트 4개라 임포트 시 링크가 병합될 수 있어
        #   이 조용한 실패가 실제로 터진다 → 후보 탐색 + 실패 시 예외로 바꾼다.
        self.palm_body_index: int = self._resolve_body_index(
            RIGHT_HAND_BODY_NAME_CANDIDATES, label="오른손 palm"
        )
        ...
        # ----------------------------------------------------------------
        # [both/pour_v1] 왼팔 action 인프라
        #   왼팔 7관절(l_aj_*)만 RL delta 제어.
        #   ★왼손은 DG-5FS 20관절(l_hj_<finger>_*)이며 **warm 파지자세 position-hold** 로 컵을
        #     물리적으로 쥔다 — 구 pour_sensor 의 2-DOF 그리퍼 + kinematic-follow 를 대체한다.
        # ----------------------------------------------------------------
        ...
        # 왼손 20관절 — preset 순서(finger×joint)를 그대로 써서 warm state 열 순서와 정합시킨다.
        _missing_lh = [n for n in LEFT_HAND_JOINT_NAMES if n not in self.robot.joint_names]
        if _missing_lh:
            raise RuntimeError(
                f"[pour_v1] 왼손 관절 누락 {_missing_lh[:4]}… — USD 가 bi_s_rl(양손 DG-5FS)인지 확인. "
                f"구 sensor_rl 은 왼손이 2-DOF 그리퍼라 이 태스크를 돌릴 수 없다."
            )
        self.left_hand_dof_indices = [
            self.robot.joint_names.index(n) for n in LEFT_HAND_JOINT_NAMES
        ]
        # 왼손 pre-grasp rest (warm 적용 전 spawn 자세 유지용)
        self.left_hand_rest = to_torch(
            [LEFT_ARM_REST_JOINT_POS.get(n, 0.0) for n in LEFT_HAND_JOINT_NAMES],
            device=self.device,
        ).unsqueeze(0).repeat(self.num_envs, 1)
        # 왼손 그립 유지 목표 (N,20). reset 에서 왼쪽 warm state 의 파지자세로 덮어쓴다.
        #   warm bank 가 없으면 rest 로 남아 컵을 쥐지 못한다 → reset 에서 명시적으로 경고.
        self.left_hand_grip_targets = self.left_hand_rest.clone()
        # 왼손 제어/관측 기준 body + jacobian index (fixed base → idx-1)
        #
        # ★좌우 앵커 비대칭 — 의도된 것이다.
        #   오른손: `palm_center_pos` = r_hl_palm + R·[0.028,0,0.04] (= palm_ee, **계산값**)
        #   왼손  : l_hl_palm 그대로
        #   `palm_ee` 는 body_names 에 **없다** — USD 의 `*_hj_palm_ee` 가 fixed joint 라
        #   Isaac 임포트에서 palm 으로 병합된다(실측: l_hl_* body 28개에 palm_ee 부재).
        #   그래서 오른손도 body 를 읽는 게 아니라 오프셋을 수동으로 더한다.
        #
        #   왼팔에 같은 오프셋을 도입하지 않는 이유: DifferentialIK 의 jacobian 이
        #   `l_hl_palm` body 기준으로 나온다. palm_ee 를 제어점으로 삼으려면 오른팔처럼
        #   "target 을 palm_ee 로 만들고 IK 직전에 palm 으로 역변환"하는 단계가 필요한데,
        #   지금은 target 캡처(_left_tcp_rest_env) · 현재 pose · jacobian 삼자가 모두
        #   l_hl_palm 이라 일관되고 정확하다. 역변환을 넣으면 오차 지점만 하나 늘어난다.
        #   낙하 판정·scripted receiver 는 모두 **차분**을 쓰므로 고정 오프셋이 상쇄된다.
        #   ⚠ 그래서 이 앵커를 바꿀 때는 위 세 곳(1381 캡처 / IK / 낙하 기준선)을 **동시에**
        #     바꿔야 한다. 하나만 바꾸면 약 4.9cm 어긋난다.
        self._left_hand_body_index = self._resolve_body_index(
            LEFT_HAND_BODY_NAME_CANDIDATES, label="왼손 palm"
        )
        ...
        # ★[both/pour_v1] 컵 follow offset/quat 버퍼는 제거했다 — 물리 파지라 문제가 없다.
        #
        # ★★ 왼팔 TCP "rest" 를 **per-env** 로 바꾼다 (구 pour_sensor 는 전 env 공통 상수였다).
        #   구조상 이유: pour_sensor 의 왼팔은 항상 LEFT_ARM_REST 자세에서 시작했으므로
        #   FK 로 계산한 단일 상수가 곧 rest 였다. pour_v1 의 왼팔은 **warm 파지자세**에서
        #   시작하므로 env 마다 rest 가 다르다. 상수를 그대로 쓰면
        #     · frozen 모드가 왼팔을 FK-of-rest 로 끌어당겨 파지가 깨지고
        #     · workspace 클램프 박스도 엉뚱한 곳에 중심을 두게 된다.
        #   왼쪽 warm 뱅크의 palm pose 를 쓰지 않는 이유: 그 값은 grasp 쪽 `palm_center_pos`
        #   (= palm_ee, 손바닥 중심)라서 우리 IK 제어점인 `l_hl_palm` 과 약 4.9cm 어긋난다.
        #   프레임 규약을 추측하지 않고, **리셋 후 첫 스텝에 실제 왼손 pose 를 캡처**한다
        #   (hold 구간이 물리 안착을 담당하므로 이 시점 값이 곧 안정 rest 다).
        self._left_tcp_rest_env = self._left_tcp_rest_pos_b.repeat(self.num_envs, 1).contiguous()
        self._left_tcp_min_env = self._left_tcp_min.repeat(self.num_envs, 1).contiguous()
        self._left_tcp_max_env = self._left_tcp_max.repeat(self.num_envs, 1).contiguous()
        self._left_tcp_rest_captured = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        self._left_tcp_wr = _wr.clone()          # (1,3) 상한 half-extent
        self._left_tcp_wr_min = _wr_min.clone()  # (1,3) 하한 half-extent (z 는 별도 캡)
        ...
        # ★[both/pour_v1] 왼팔 warm spec 요구 — receiver 컵도 이제 **물리 파지**라
        #   스폰된 컵 스케일과 warm 파지자세의 컵 스케일이 같아야 한다(구 pour_sensor 는
        #   kinematic-follow 라 이 정합이 필요 없었다). 없으면 0.9배 컵을 1.0배 파지자세로
        #   쥐어 손가락이 허공을 잡거나 벽을 파고든다.
        if _sset:
            _lmap = tuple(getattr(self.cfg, "left_warm_spec_map", ()))
            if len(_lmap) < len(_sset):
                raise ValueError(
                    f"left_warm_spec_map({_lmap})가 left_target_cup_scale_set({_sset})보다 짧음"
                )
            self._tgt_spec_env = torch.tensor(
                [int(_lmap[i]) for i in range(len(_sset))],
                dtype=torch.long, device=self.device,
            )[torch.arange(self.num_envs, device=self.device) % len(_sset)]  # (N,)
        else:
            self._tgt_spec_env = None
        self._left_warm_spec_pools: dict[int, torch.Tensor] | None = None
        ...
        _bead_offsets = []
        # ★[both/pour_v1] 배치를 `openarm.common.bead_assets.bead_offsets_in_cup` 로 이관.
        #   grasp 수집과 pour 복원이 **같은 배치**를 써야 한다(수치 동일 — 거동 변화 없음).
        _bead_offsets = _bead_offsets_in_cup(self.num_beads)
        ...
        # ★[both/pour_v1] 왼컵 낙하 플래그/율. `_get_dones()` 가 먼저 채우지만, 첫 프레임이나
        #   보상만 계산되는 경로에서 AttributeError 가 나지 않도록 여기서 초기화한다.
        self._left_cup_dropped = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        self._left_cup_dropped_rate = torch.zeros((), device=self.device)
        # ★[both/pour_v1] z-lock 기준 높이의 저역통과 상태 (`_update_spout_z_lock_ref`).
        self._spout_z_lock_ref = torch.zeros(self.num_envs, device=self.device)
        self._spout_z_lock_ref_valid = torch.zeros(
            self.num_envs, dtype=torch.bool, device=self.device
        )
        ...
        # ★[both/pour_v1] 왼팔 warm bank (별개 뱅크). count=0 이면 왼팔 warmstart 비활성.
        # ★[both/pour_v1] warm 캐시의 비드 상태 (없으면 None → 기존 소환 경로)
        self._warmstart_bead_state = None
        self._left_warm_count = 0
        self._left_warm_arm_pos = torch.zeros(1, NUM_ARM_DOF, device=self.device)
        self._left_warm_hand_pos = torch.zeros(1, NUM_HAND_DOF, device=self.device)
        self._left_warm_palm_pose = torch.zeros(1, 7, device=self.device)
        self._left_warm_cup_pose = torch.zeros(1, 7, device=self.device)
        # 왼컵 낙하 판정 기준선 (리셋 시점의 palm↔컵 거리 / 컵 z). `_get_dones` 에서 사용.
        self._left_cup_ref_dist = torch.zeros(self.num_envs, device=self.device)
        self._left_cup_ref_z = torch.zeros(self.num_envs, device=self.device)
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        # ★[both/pour_v1] per-env 왼팔 rest 캡처 — 리셋 직후 첫 스텝에 실제 왼손 pose 를 rest 로 잡는다.
        #   warm 파지자세는 env 마다 다르므로 FK 상수를 rest 로 쓰면 frozen 이 파지를 깨뜨린다.
        _uncaptured = ~self._left_tcp_rest_captured
        if bool(_uncaptured.any()):
            _lh_pos_w = self.robot.data.body_pos_w[:, self._left_hand_body_index]
            ...
            self._left_tcp_min_env = self._left_tcp_rest_env - self._left_tcp_wr_min
            # target 도 rest 로 맞춰 첫 스텝에 팔이 튀지 않게 한다.
            self.left_tcp_target_pos_b = torch.where(
                _m, self._left_tcp_rest_env, self.left_tcp_target_pos_b
            )
            self._left_tcp_rest_captured |= _uncaptured

        _recv_mode = self.cfg.receiver_control_mode
        if _recv_mode == "frozen":
            _new_left_target = self._left_tcp_rest_env.clone()
            ...
            _new_left_target[:, 2] = self._left_tcp_rest_env[:, 2]                  # z는 per-env rest 고정
            _new_left_target = torch.clamp(_new_left_target, self._left_tcp_min_env, self._left_tcp_max_env)
            ...
            _new_left_target = torch.clamp(
                ...
                self._left_tcp_min_env, self._left_tcp_max_env,
            )
        if self.cfg.episode_hold_steps > 0:
            ...
            _new_left_target = torch.where(
                _lhold, self._left_tcp_rest_env, _new_left_target
            )
        ...
        if self._warmstart_collect_mode:
            ...
            # ★[both/pour_v1] z-lock 기준 높이를 저역통과시켜 둔다 (아래 두 z-lock 지점이 공유).
            #   왼컵이 물리 파지로 바뀌어 입구 높이가 진동하므로, 그 진동을 오른팔 목표에
            ...
            if self.cfg.pour_spout_z_lock:
                ...
                pour_point_target[:, 2] = self._spout_z_lock_ref + self.cfg.pour_z_margin
            if self.cfg.pour_approach_pivot == "palm":
            ...
            if self.cfg.pour_orient_release:
                ...
                if self.cfg.pour_spout_z_lock:
                    ...
                    _spout_target_bl[:, 2] = self._spout_z_lock_ref + self.cfg.pour_z_margin
    ...
    def _apply_action(self) -> None:
        ...
        # ---- 왼손: warm 파지자세 position-hold (컵을 물리적으로 쥔 채 유지) ----
        # ★[both/pour_v1] 구 pour_sensor 는 여기서 ① 2-DOF 그리퍼를 rest 로 고정하고
        #   ② target 컵을 왼손에 **kinematic-follow** 로 매 스텝 써 넣었다(= 물리 파지 아님).
        #   pour_v1 은 왼손 20관절을 warm 파지자세로 유지해 **물리적으로** 컵을 잡는다.
        #   따라서 컵 pose 를 강제로 써 넣는 코드는 제거했다 — 컵은 물리로만 움직인다.
        self.robot.set_joint_position_target(
            self.left_hand_grip_targets, joint_ids=self.left_hand_dof_indices
        )
        self.robot.set_joint_velocity_target(
            torch.zeros_like(self.left_hand_grip_targets), joint_ids=self.left_hand_dof_indices
        )
    ...
    def _get_observations(self) -> dict:
        ...
        # ★[both/pour_v1] 왼팔 obs 는 **arm 7관절만** 쓴다.
        #   구 pour_sensor 는 `left_arm_dof_indices`(arm7 + 그리퍼2 = 9)를 그대로 넣었다.
        #   pour_v1 은 왼손이 DG-5FS 20관절이라 같은 코드가 27개를 흘려보내 obs 가 91 로 터진다.
        #   왼손은 warm 파지자세 position-hold(정책이 제어하지 않음)라 관절값에 행동 정보가 없다
        #   → 제어 대상인 arm 7관절만 관측한다 (actor obs 55 → 51, critic extras 39 → 35).
        left_arm_joint_pos_clean = self.robot.data.joint_pos[:, self.left_arm_only_dof_indices]
        left_arm_joint_vel_clean = self.robot.data.joint_vel[:, self.left_arm_only_dof_indices]
        ...
        actor_obs = torch.cat([
            ...
            left_arm_joint_pos,         # 7  (왼팔 arm 만 — 왼손은 동결)
        ...
        critic_obs = torch.cat([
            actor_obs_clean,                                   # 105
            left_arm_joint_pos_clean,                          # 7
            left_arm_joint_vel_clean,                          # 7
            distal_binary,                                     # 5
            distal_force_norm,                                 # 5
            cup_height_delta,                                  # 1
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        total = (
            ...
            # ★[both/pour_v1] 왼컵 낙하 페널티. 종료만 시키고 페널티가 없으면 "빨리 끝내기"가
            #   손해가 아니게 되어(잔여 보상 스트림 포기 = 이미 손해지만 부족) 회피 학습이 약하다.
            - self.cfg.left_cup_drop_penalty * self._left_cup_dropped.float()
        )
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # ★[both/pour_v1] 왼컵(receiver) 낙하 — dynamic 전환으로 새로 생긴 실패 모드.
        #   구 pour_sensor 는 왼컵이 kinematic-follow 라 절대 떨어지지 않았고, 그래서 이 종료가
        #   존재하지 않았다. pour_v1 은 왼손이 실제로 쥐고 있으므로 놓칠 수 있다.
        #   이를 종료로 잡지 않으면 "컵을 떨어뜨린 뒤에도 조준 보상만 긁는" 경로가 열린다.
        #   접촉센서 대신 기하로 판정한다(왼손엔 센서를 안 붙였다 — obs 차원 불변 유지):
        #     ① palm↔컵 거리가 리셋 기준선 대비 임계 이상 벌어짐  또는  ② 컵이 기준선보다 크게 낮아짐
        #   hold 구간은 물리 안착 중이라 제외한다.
        if bool(self.cfg.left_cup_drop_enable):
            _lcup_pos_w = self.left_target_cup.data.root_pos_w
            _lhand_pos_w = self.robot.data.body_pos_w[:, self._left_hand_body_index]
            _lcup_dist = torch.norm(_lcup_pos_w - _lhand_pos_w, dim=-1)
            _drop_active = (
                (self.episode_length_buf >= self.cfg.episode_hold_steps)
                & (~torch.full_like(bead_fallen, self._warmstart_collect_mode, dtype=torch.bool))
            )
            left_cup_dropped = _drop_active & (
                ((_lcup_dist - self._left_cup_ref_dist) > self.cfg.left_cup_drop_dist_m)
                | ((self._left_cup_ref_z - _lcup_pos_w[:, 2]) > self.cfg.left_cup_drop_z_m)
            )
        else:
            left_cup_dropped = torch.zeros_like(bead_fallen)
        # `_get_dones()` 는 `_get_rewards()` 보다 먼저 호출되므로(DirectRLEnv 순서),
        # per-env 플래그를 보관해 보상에서 낙하 페널티로 쓴다.
        self._left_cup_dropped = left_cup_dropped
        self._left_cup_dropped_rate = left_cup_dropped.float().mean()
        ...
        terminated = (
            ...
            | left_cup_dropped          # ★[both/pour_v1] receiver 컵 놓침 = 실패
        )
        truncated  = (
        ...
        for _name, _flag in (
            ...
            ("left_cup_dropped", left_cup_dropped),   # ★[both/pour_v1] 왼컵 낙하율
        ):
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None) -> None:
        ...
        # ★[both/pour_v1] 왼팔도 warm 파지자세로 시작 (뱅크 없으면 rest 로 degrade).
        _left_sample = self._sample_left_warm(env_ids)
        self._write_left_warm_joints(full_pos, _left_sample)
        self.robot.write_joint_state_to_sim(full_pos, full_vel, env_ids=env_ids)
        if _left_sample is not None:
            self.left_hand_grip_targets[
        ...
        self._write_left_warm_joints(pregrasp_full_pos, _left_sample)
        ...
        self._place_left_cup(env_ids, _left_sample)
        ...
        # 왼팔 TCP target/orientation 복원 + IK 내부상태 리셋.
        # ★[both/pour_v1] rest 는 warm 파지자세라 env 마다 다르다 → 여기서 상수로 못 정한다.
        #   캡처 플래그만 내려서, 다음 스텝에 실제 왼손 pose 를 per-env rest 로 다시 잡게 한다.
        #   (일단 FK 상수를 넣어두는 것은 캡처 전 1스텝 동안의 안전한 초기값 용도다.)
        ...
        self._left_tcp_rest_captured[env_ids] = False
    ...
    # ★[both/pour_v1] `_get_left_cup_follow_pose()` 는 제거했다.
    #   receiver 컵은 더 이상 왼손 pose 를 따라 써 넣는 kinematic body 가 아니고,
    #   왼손이 물리적으로 쥐는 dynamic 강체다. 컵 pose 는 물리 결과로만 정해진다.
    ...
    # ------------------------------------------------------------------
    # ★[both/pour_v1] 왼팔 warm state 적용 — reset 경로 4곳이 공유한다
    ...
    def _sample_left_warm(self, env_ids: Sequence[int]):
        ...
        ★receiver 컵 스케일이 섞이면(`left_target_cup_scale_set`) env 별 요구 spec 에
          맞는 풀에서만 뽑는다 — 스케일이 다른 파지자세를 쓰면 손가락이 허공을 잡거나
          컵 벽을 파고든다. scale_set 이 없으면 전 풀에서 독립 샘플링한다.

        좌/우를 독립 샘플링하는 근거: 두 팔은 y 로 20cm 떨어져 각자의 컵을 쥐므로
        조합 제약이 없다고 본다. 자기충돌이 관측되면 페어 제한으로 바꿔야 한다.
        """
        if self._left_warm_count <= 0:
            return None
        env_ids_t = torch.as_tensor(env_ids, dtype=torch.long, device=self.device)
        n = int(env_ids_t.numel())
        if self._left_warm_spec_pools is not None:
            spec_req = self._tgt_spec_env[env_ids_t]
            pick = torch.empty(n, dtype=torch.long, device=self.device)
            for k, pool in self._left_warm_spec_pools.items():
                m = spec_req == k
                cnt = int(m.sum())
                if cnt:
                    pick[m] = pool[torch.randint(pool.shape[0], (cnt,), device=self.device)]
        else:
            pick = torch.randint(self._left_warm_count, (n,), device=self.device)
        return (
            self._left_warm_arm_pos[pick],    # (n,7)  l_aj_*
            self._left_warm_hand_pos[pick],   # (n,20) l_hj_*
            self._left_warm_cup_pose[pick],   # (n,7)  env-local pos+quat_wxyz
        )

    def _write_left_warm_joints(self, full_pos: torch.Tensor, sample) -> None:
        """`full_pos` 에 왼팔 관절을 채운다. sample=None 이면 기존 rest 자세."""
        if sample is None:
            full_pos[:, self.left_arm_dof_indices] = self.left_arm_zero_pos[0]
            return
        full_pos[:, self.left_arm_only_dof_indices] = sample[0]
        full_pos[:, self.left_hand_dof_indices] = sample[1]

    def _place_left_cup(self, env_ids: Sequence[int], sample) -> torch.Tensor:
        """receiver 컵을 배치하고 world pose(n,7)를 반환한다.

        sample 이 있으면 **왼쪽 grasp 성공 당시의 실제 컵 자세**로 텔레포트한다
        (오른쪽과 같은 규약 — upright 강제는 손-컵 상대자세를 깨뜨린다).
        sample 이 없으면 구 FK 고정배치로 degrade 한다.
        또한 낙하 판정 기준선(palm↔컵 거리 / 컵 z)을 여기서 갱신한다.
        """
        env_ids_t = torch.as_tensor(env_ids, dtype=torch.long, device=self.device)
        n = env_ids_t.numel()
        if sample is None:
            left_cup_pose_w = self._get_left_cup_fk_pose(env_ids=env_ids)
        else:
            left_cup_pose_w = sample[2].clone()
            left_cup_pose_w[:, :3] += self.scene.env_origins[env_ids_t]
        self._left_target_cup_fixed_pose_w[env_ids_t] = left_cup_pose_w
        zero_vel = torch.zeros(n, 6, device=self.device)
        self.left_target_cup.write_root_pose_to_sim(left_cup_pose_w, env_ids=env_ids)
        self.left_target_cup.write_root_velocity_to_sim(zero_vel, env_ids=env_ids)

        # 낙하 판정 기준선. body_pos_w 는 write 직후 stale 할 수 있어 왼손 위치는
        # 관절 write 이후의 최신값을 쓰되, 첫 프레임 오차는 hold 구간이 흡수한다.
        left_hand_w = self.robot.data.body_pos_w[env_ids_t, self._left_hand_body_index]
    ...
    def _load_left_warmstart_cache_from_disk(self) -> bool:
        """★[both/pour_v1] **왼팔** grasp 캐시를 로드해 `_left_warm_*` 버퍼를 채운다.

        오른팔 로더와 형식은 같지만(arm7 + hand20 + palm7 + cup local pose) 뱅크가
        별개다 — 왼팔은 left/grasp_v1(스폰 y=+0.10, receiver 컵), 오른팔은
        right/grasp_v1(스폰 y=-0.10, source 컵)에서 각각 수집된다.

        성공 시 True. 실패 시 False 를 반환하고 호출부가 FK 고정배치로 degrade 한다.
        **조용히 실패하지 않는다** — 실패 사유를 반드시 출력한다.
        """
        paths = tuple(getattr(self.cfg, "left_warm_state_paths", ()) or ())
        if not paths:
            print("[pour_v1] left_warm_state_paths 미설정 → 왼팔 warmstart 비활성.", flush=True)
            return False
        try:
            ...
            bank = PourWarmStateBank.from_hdf5_paths(
                paths,
                device=self.device,
                # 왼팔 grasp 스폰 z 도 오른팔과 동일(0.297) — 불일치면 hard fail 시켜
                # 좌우 캐시가 서로 다른 기하에서 수집된 사고를 잡는다.
                expected_object_spawn_z=self.cfg.object_spawn_z,
                # palm workspace 경계는 오른팔 기준이라 왼팔에 적용하면 오탐이 난다 → 생략.
                expected_palm_bounds=None,
                object_spec_filter=_lf if _lf else None,
                expected_robot_usd=_EXPECTED_ROBOT_USD,
            )
        except (FileNotFoundError, KeyError, ValueError) as exc:
            print(f"[pour_v1] 왼팔 warm-state 로드 실패: {exc}", flush=True)
            return False

        n = len(bank)
        if n == 0:
            print("[pour_v1] 왼팔 warm-state 캐시가 비어 있다.", flush=True)
            return False

        self._left_warm_arm_pos = bank.arm_joint_pos.clone()      # (n,7)  l_aj_*
        self._left_warm_hand_pos = bank.hand_joint_pos.clone()    # (n,20) l_hj_*
        self._left_warm_palm_pose = bank.palm_pose_quat_xyzw.clone()  # (n,7)
        left_cup = torch.zeros(n, 7, device=self.device)
        left_cup[:, :3] = bank.cup_pos_local
        left_cup[:, 3:7] = bank.cup_quat_wxyz
        self._left_warm_cup_pose = left_cup
        self._left_warm_count = n
        # ★spec 매칭: receiver 컵 스케일이 섞이면(left_target_cup_scale_set) env 별로
        #   **그 스케일에서 실제로 파지한** warm state 만 골라야 한다. 스케일이 다른
        #   파지자세를 쓰면 손가락이 허공을 잡거나 컵 벽을 파고든다.
        self._left_warm_spec_pools = None
        if self._tgt_spec_env is not None:
            _spec = bank.object_spec_idx
            if (_spec < 0).all():
                raise ValueError(
                    "왼팔 warm 캐시가 미태깅(object_spec_idx 전부 -1)인데 receiver 컵 "
                    "scale_set 이 켜져 있다. 스케일별 파지를 매칭할 수 없다 — 재수집 필요."
                )
            pools: dict[int, torch.Tensor] = {}
            for k in torch.unique(self._tgt_spec_env).tolist():
                pool = (_spec == int(k)).nonzero(as_tuple=False).squeeze(-1)
                if pool.numel() == 0:
                    raise ValueError(
                        f"왼팔 warm 캐시에 spec {k} 상태가 없다 "
                        f"(보유 spec: {sorted(set(_spec.tolist()))}). "
                        "left_warm_spec_map 또는 수집 범위를 확인할 것."
                    )
                pools[int(k)] = pool
            self._left_warm_spec_pools = pools
            print(
                "[pour_v1] 왼팔 spec 매칭 풀: "
                + ", ".join(f"spec{k}={int(v.numel())}" for k, v in sorted(pools.items())),
                flush=True,
            )
        print(
            f"[pour_v1] 왼팔 warm bank 로드 완료: {n} states "
            f"(spec_filter={_lf if _lf else '없음'})",
            flush=True,
        )
        return True
    ...
    def _load_warmstart_cache_from_disk(self) -> bool:
        ...
        # ★[both/pour_v1] 비드를 채워 수집한 캐시면 그 상태를 보관해 리셋에서 복원한다.
        #   개수가 pour 의 bead_count 와 다르면 복원이 불가하므로 버리고 자체 소환으로 degrade.
        self._warmstart_bead_state = None
        if bank.bead_state is not None:
            _k_cache = int(bank.bead_state.shape[1])
            _k_pour = int(self.cfg.bead_count)
            if _k_cache == _k_pour:
                self._warmstart_bead_state = bank.bead_state.clone()
                print(
                    f"[pour_v1] warm 캐시에 비드 상태 있음 (k={_k_cache}) → "
                    "리셋에서 복원, hold-end 지연 소환 생략.",
                    flush=True,
                )
            else:
                print(
                    f"[pour_v1][WARN] 비드 개수 불일치 (캐시 {_k_cache} vs pour {_k_pour}) → "
                    "복원 포기, 기존 hold-end 소환 경로 사용.",
                    flush=True,
                )
        else:
            print(
                "[pour_v1] warm 캐시에 비드 상태 없음 → hold-end 지연 소환 경로 사용. "
                "(빈 컵 파지에 나중에 하중이 걸리므로 grasp 유지력을 확인할 것)",
                flush=True,
            )
    ...
    def _build_warmstart_reset_cache(self) -> None:
        ...
        # ★[both/pour_v1] 왼팔 뱅크는 오른팔 소스 설정과 **독립**으로 먼저 로드한다.
        #   오른팔이 rollout 으로 degrade 하더라도 왼팔 파지 시작은 유지되는 게 낫고,
        #   반대로 왼팔만 없을 때도 오른팔 경로는 그대로 돌아야 한다.
        if source == "disk" and not self._load_left_warmstart_cache_from_disk():
            print(
                "[pour_v1][WARN] 왼팔 warm bank 없음 → 왼손이 컵을 쥐지 않은 채 시작한다. "
                "receiver 컵은 FK 고정배치로 degrade. "
                "양손 파지 실험을 하려면 left/grasp_v1 캐시를 먼저 수집할 것.",
                flush=True,
            )
    ...
    def _reset_from_warmstart_cache(self, env_ids: Sequence[int]) -> None:
        ...
        self.robot.write_joint_state_to_sim(full_pos, full_vel, env_ids=env_ids)
        if _left_sample is not None:
            self.left_hand_grip_targets[env_ids_t2] = _left_sample[1]
        ...
        left_cup_pose = self._place_left_cup(env_ids, _left_sample)

        # ★[both/pour_v1] 비드 처리 — 두 경로
        #   ① warm 캐시가 비드까지 담고 있으면 **그 상태를 복원**한다. 수집 시점에 이미
        #      비드가 든 컵을 잡은 상태이므로, 파지와 하중이 처음부터 정합한다.
        #      (pour 는 손을 warm 자세로 동결하는 수동 스프링이라, 나중에 하중을 더하면
        #       흡수하지 못해 컵을 놓칠 수 있다 — 그래서 수집 시 채우는 쪽이 맞다.)
        #   ② 없으면 기존 경로: 숨겨두고 hold 종료 후 안정화된 컵 위치에 소환.
        if self._warmstart_bead_state is not None:
            _bs = self._warmstart_bead_state[pick].clone()          # (n,k,13) env-local
            _bs[:, :, :3] += self.scene.env_origins[env_ids_t2].unsqueeze(1)
            self.beads.write_object_state_to_sim(_bs, env_ids=env_ids)
            self._beads_spawned[env_ids] = True     # 재소환 금지 (이미 컵 안)
        else:
            self._hide_beads(env_ids)
            self._beads_spawned[env_ids] = False
    ...
    def _reset_from_deep_tilt_bank(self, env_ids: torch.Tensor) -> None:
        ...
        self.robot.write_joint_state_to_sim(full_pos, full_vel, env_ids=env_ids)
        if _left_sample is not None:
            self.left_hand_grip_targets[env_ids] = _left_sample[1]
