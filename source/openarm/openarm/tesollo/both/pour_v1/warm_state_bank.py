...
class PourWarmStateBank:
    ...
    # ★[both/pour_v1] 비드를 채워 수집한 캐시만 보유. (N, k, 13) env-local, 없으면 None.
    #   있으면 pour 가 "비드가 든 컵을 쥔 상태"로 복원하고, 없으면 자체 소환으로 degrade.
    bead_state: torch.Tensor | None
    ...
    def from_hdf5_paths(
        ...
        *,
        ...
        expected_robot_usd: 기대하는 로봇 자산 이름(부분 일치, 예 "openarm_tesollo_bi_s_rl").
        ...
        # ★[both/pour_v1] 비드 상태는 **모든** 파일에 있고 개수가 같을 때만 사용한다.
        #   일부만 있으면 합칠 수 없으므로 전체를 버리고(경고) pour 자체 소환으로 degrade.
        _bead_chunks = [c.get("bead_state") for c in chunks]
        if all(b is not None for b in _bead_chunks):
            _ks = {int(b.shape[1]) for b in _bead_chunks}
            if len(_ks) == 1:
                merged["bead_state"] = np.concatenate(_bead_chunks, axis=0)
            else:
                print(
                    f"[PourWarmStateBank][WARN] 파일별 비드 개수 불일치 {sorted(_ks)} → "
                    "bead_state 무시(pour 자체 소환으로 degrade).",
                    flush=True,
                )
        elif any(b is not None for b in _bead_chunks):
            print(
                "[PourWarmStateBank][WARN] 일부 파일에만 bead_state 가 있다 → "
                "bead_state 무시(pour 자체 소환으로 degrade).",
                flush=True,
            )
        ...
        return cls(
            ...
            bead_state=(
                _to_t(merged["bead_state"], device) if "bead_state" in merged else None
            ),
    ...
    with h5py.File(path, "r") as h5:
        ...
        for k, v in h5.attrs.items():
            ...
            try:
                ...
                continue
        # ★[both/pour_v1] 자산 출처(robot_usd) 확인.
        #   2026-08-17 사고: 로봇 자산이 DG-5F → DG-5FS 로 교체됐는데 warm state 텐서 차원이
        #   같아서(arm7+hand20) 구 캐시가 **에러 없이** 로드됐다. 손 기하가 달라 palm/컵 상대
        #   자세가 어긋난 초기상태로 학습이 돌 수 있었다.
        #   현재 grasp 쪽 export 는 이 속성을 기록하지 않으므로, 없으면 경고만 하고
        #   있으면 불일치를 잡는다 (기록이 들어오면 자동으로 방어가 켜진다).
        # ★[both/pour_v1] 비드 상태 (선택). 없으면 키를 넣지 않는다.
        if "bead_state" in grp:
            out["bead_state"] = np.asarray(grp["bead_state"], dtype=np.float32)
...
def _warn_on_robot_usd_mismatch(
    ...
    if not tagged:
        print(
            ...
            f"({[p.name for p in resolved]}). 이 캐시가 현재 로봇 자산(DG-5FS)에서 나온 것인지 "
            "자동 확인할 수 없다 — 좌/우 grasp 를 신 USD 로 재수집했는지 직접 확인할 것.",
            flush=True,
        )
        return
    ...
    if bad:
        raise ValueError(
            ...
            "구 로봇 자산에서 수집한 캐시다 — 손 기하가 달라 초기 파지가 어긋난다. 재수집 필요."
        )
