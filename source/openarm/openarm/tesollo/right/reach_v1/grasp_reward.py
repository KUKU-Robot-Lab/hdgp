...
def compute_grasp_reward_terms(
    *,
    ...
    # 1. [Coarse] 원거리 선형 거리 감점 (먼 곳에서 컵 방향으로 빠른 유도)
    approach_coarse = -_cfg_float(cfg, "approach_coarse_weight", 0.25) * palm_to_cup_dist

    # 2. [Fine] 근거리 Tanh 정밀 밀착 가점 (10cm 이내에서 목표 위치 락온)
    fine_std = _cfg_float(cfg, "approach_fine_std", 0.10)
    approach_fine = _cfg_float(cfg, "approach_fine_weight", 0.20) * (1.0 - torch.tanh(palm_to_cup_dist / fine_std))

    # 3. [Align] 손바닥 정면-컵 방향 정렬 보상
    align_reward = _cfg_float(cfg, "approach_align_weight", 0.10) * upright_quality
    ...
    approach = pre_lift_gate * (
        ...
        + approach_fine
        + align_reward
        - _cfg_float(cfg, "approach_xy_penalty_weight", 0.0) * torch.relu(cup_xy_displacement - xy_margin)
        - _cfg_float(cfg, "approach_tilt_penalty_weight", 0.0) * torch.relu(cup_tilt_deg - tilt_margin)
    )
