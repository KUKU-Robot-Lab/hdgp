...
# teacher(actor) policy obs 중 물체 privileged 항목 (student 는 이걸 못 본다).
# [08-21] object_rot_noisy/object_scale/onehot 은 이미 actor obs 에서 제거됐다
# (미학습 신규 물체 일반화 개편 — teacher 도 물체 pos 만 본다). 남은 건 pos 뿐이라
# student 와의 유일한 차이가 "물체가 어디 있는지 아느냐"로 좁혀졌다.
...
def test_teacher_obs_dim_matches_action_dim():
    """actor/critic/student obs 고정 차원 계약.

    [08-21] actor(teacher) obs 는 물체 onehot/scale/rotation 제거로 193(base, 물체 수
    무관) + 15(접촉력) = 208 로 재정의됐다. critic/student 는 이번 개편과 무관(불변).
    """
    assert C["NUM_OBS_BASE"] == 208
