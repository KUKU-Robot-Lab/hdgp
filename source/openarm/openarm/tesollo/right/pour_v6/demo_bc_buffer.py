...
@dataclass(frozen=True)
class DemoEpisodeFk:
    """52D FK obs 에피소드 (DiffusionActor용, datagen_info cup 없음)."""
    obs: Tensor      # (T, 52)
    actions: Tensor  # (T, 11)
    pour_mask: Tensor  # (T,) bool
...
def _load_episode(
    ...
    _LEGACY_OBS_DIM = 60  # _load_episode()는 60D obs를 생성 (LSTM BC 레거시)
    if obs.shape[1] != _LEGACY_OBS_DIM:
        raise RuntimeError(f"{path}: obs dim {obs.shape[1]} != {_LEGACY_OBS_DIM}")
...
# ---------------------------------------------------------------------------
# _load_episode_fk — 52D FK obs (DiffusionActor용)
# ---------------------------------------------------------------------------
...
def _load_episode_fk(
    ...
    """HDF5 → 52D FK obs + 11D action (datagen_info cup 미사용).

    eef_pose/left (left palm FK) = target cup 위치 추정.
    eef_pose/right (right palm FK) = source cup 위치 추정.
    cup-local basis 계산에는 datagen_info cup pos 사용 (action 일관성).
    """
    with h5py.File(path, "r") as h5:
        demo = h5["data"]["demo_0"]
        n_raw = int(demo["obs"]["right_arm_joint_pos"].shape[0])
        ...
        hand_pos  = torch.as_tensor(demo["obs"]["right_hand_joint_pos"][s], dtype=torch.float32, device=device)
        joint_vel = torch.as_tensor(demo["obs"]["right_joint_vel"][s],      dtype=torch.float32, device=device)
        left_arm  = torch.as_tensor(demo["obs"]["left_arm_joint_pos"][s],   dtype=torch.float32, device=device)
        r_eef_mat = torch.as_tensor(demo["obs"]["datagen_info"]["eef_pose"]["right"][s],        dtype=torch.float32, device=device)
        l_eef_mat = torch.as_tensor(demo["obs"]["datagen_info"]["eef_pose"]["left"][s],         dtype=torch.float32, device=device)
        tgt_mat   = torch.as_tensor(demo["obs"]["datagen_info"]["target_eef_pose"]["right"][s], dtype=torch.float32, device=device)
        src_cup   = torch.as_tensor(demo["obs"]["datagen_info"]["object_pose"]["source_cup"][s],dtype=torch.float32, device=device)
        tgt_cup   = torch.as_tensor(demo["obs"]["datagen_info"]["object_pose"]["target_cup"][s],dtype=torch.float32, device=device)
        pour_start= torch.as_tensor(demo["obs"]["datagen_info"]["subtask_start_signals"]["pour_start"][s], dtype=torch.bool, device=device)
    ...
    r_palm_quat = _quat_xyzw_from_matrix(r_eef_mat[:, :3, :3])          # (T, 4) xyzw
    l_palm_pos  = l_eef_mat[:, :3, 3]                                   # (T, 3)
    right_cup_pos_rel = src_cup[:, :3, 3] - r_palm_pos                  # (T, 3)
    left_vel    = torch.zeros(T, 7, dtype=dt, device=device)            # (T, 7)
    finger_grasp_progress = _finger_grasp_progress(hand_pos, device, dt)  # (T, 5)

    # ---- Action 계산 (기존 _load_episode() 와 동일) ----
    base_pos  = r_eef_mat[0, :3, 3].unsqueeze(0).expand(T, -1)
    base_quat = _quat_xyzw_from_matrix(r_eef_mat[0:1, :3, :3]).expand(T, -1)

    tgt_pos_act  = tgt_mat[:, :3, 3]
    tgt_quat_act = _quat_xyzw_from_matrix(tgt_mat[:, :3, :3])

    pos_delta  = tgt_pos_act - base_pos
    dq         = _quat_mul(tgt_quat_act, _quat_conj(base_quat))
    rotvec_w   = _axis_angle_from_quat(dq)
    ...
    # last_palm_actions = shift-1
    last_palm_actions = torch.zeros(T, NUM_PALM_ACTION, dtype=dt, device=device)
    last_palm_actions[1:] = palm_act[:-1]
    ...
    if obs.shape[1] != NUM_OBSERVATIONS:
        raise RuntimeError(f"{path}: FK obs dim {obs.shape[1]} != {NUM_OBSERVATIONS}")
    if actions.shape[1] != NUM_ACTIONS:
        raise RuntimeError(f"{path}: action dim {actions.shape[1]} != {NUM_ACTIONS}")
    ...
    if pour_start.any():
        ...
        src_quat = _quat_xyzw_from_matrix(src_cup[:, :3, :3])
        cos_thr = math.cos(math.radians(_POUR_PHASE_TILT_DEG))
        cup_up_z = _quat_apply(src_quat, torch.tensor(_UP_AXIS_B, dtype=dt, device=device).expand(T, -1))[:, 2]
        pour_mask = cup_up_z < cos_thr
