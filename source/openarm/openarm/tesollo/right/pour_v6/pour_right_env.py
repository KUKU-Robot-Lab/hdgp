...
class PourRightEnv(DirectRLEnv):
    ...
    def _get_observations(self) -> dict:
        ...
        # DiffusionActor 52D obs: 양팔 FK, datagen_info cup 없음
        left_arm_joint_pos = self.robot.data.joint_pos[:, self.left_arm_dof_indices][:, :7]  # (N, 7)
        left_arm_joint_vel = torch.zeros(self.num_envs, 7, device=self.device)               # (N, 7) — kinematic
        # right palm quat (wxyz → xyzw 변환)
        palm_quat_wxyz = self.robot.data.body_quat_w[:, self.palm_body_index]  # (N, 4) wxyz
        right_palm_quat_xyzw = torch.cat(
            [palm_quat_wxyz[:, 1:4], palm_quat_wxyz[:, 0:1]], dim=-1
        )  # (N, 4) xyzw
        # left palm pos (target cup 위치 추정)
        left_palm_pos = (
            self.robot.data.body_pos_w[:, self.left_hand_body_index]
        ...
        actor_obs = torch.cat([
            ...
            palm_center_pos,            # 3  (right palm pos FK)
            right_palm_quat_xyzw,       # 4  (right palm quat xyzw)
            left_palm_pos,              # 3  (left palm pos = target cup 추정)
            right_cup_pos_rel_palm,     # 3  (source cup relative to right palm)
            last_palm_actions,          # 6
        ], dim=-1)   # 52D
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # v3: force balance between thumb and others (upright only)
        ...
        v3_tilt = compute_v3_tilt_reward(
            ...
            pour_align_gate_scale=self.cfg.pour_align_gate_scale,
            pour_tilt_target_deg=self.cfg.pour_tilt_target_deg,
            pour_tilt_sharpness=self.cfg.pour_tilt_sharpness,
        )
        ...
        r_pour_dist = compute_v3_pour_dist_reward(
            ...
            pour_point_tilt_threshold_deg=self.cfg.pour_point_tilt_threshold_deg,
        )
