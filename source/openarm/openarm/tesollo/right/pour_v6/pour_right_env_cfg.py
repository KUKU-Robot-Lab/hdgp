...
class PourRightEnvCfg(DirectRLEnvCfg):
    ...
    pour_align_gate_scale: float = 8.0            # pour_aligned_gate = exp(-scale × mouth_xy)
    ...
    weight_premature_tilt: float = 1.0
    ...
    # Phase 1 BC 학습 시 False, Phase 2 RL 시 True로 설정
    # -----------------------------------------------------------------------
    enable_left_arm_domain_random: bool = True
    left_arm_domain_noise_scale: float = 0.03   # σ (rad/joint). ~3cm EEF 이동
    left_arm_domain_noise_clip: float = 0.06    # ±rad 클립 (~6cm EEF 최대)
