...
def capture_mask(
    ...
    cup_rel_drift_deg: torch.Tensor,
    ...
    *,
    ...
    A state qualifies only if it is simultaneously:
      ...
      - grasp intact: ``cup_rel_drift_deg <= drift_max`` and
        ``num_contacts >= contacts_min``. Without this, a slipping grasp at deep
        tilt gets captured and restored, so boot envs start broken and the bank
        self-poisons into a downward spiral (lstm_test3 failure mode).
    ...
    return (
        ...
        & (cup_rel_drift_deg <= drift_max)
        & (num_contacts >= contacts_min)
    )
