...
## v6 Diffusion BC + skrl PPO 구조 (2026-05-29 추가)

### Phase 1→2 전환 체크리스트

```
Phase 1 완료 확인:
  - /home/user/rl_ws/hdgp/log/diffusion_bc/v6/bc_5000.pt 존재
  - 학습 loss < 0.05 (1000 epoch 이후)

Phase 2 시작 전:
  - pour_right_env_cfg.py: enable_left_arm_domain_random = True
  - NUM_OBSERVATIONS = 52 (현재값, 변경 불필요)
...
python3 train_bc.py --epochs 5000 --output_dir /home/user/rl_ws/hdgp/log/diffusion_bc/v6
```
...
python3 /home/user/rl_ws/hdgp/scripts/reinforcement_learning/skrl/train.py \
  --task 5g_pour_right-v6-diffusion \
  --agent skrl_diffusion_ppo_cfg_entry_point \
  --num_envs 128
```
...
| 파일 | 역할 |
|-----|------|
| `diffusion_bc/__init__.py` | 패키지 |
| `diffusion_bc/net.py` | DiffusionBCNet (UNet1D) |
| `diffusion_bc/buffer.py` | DemoBCDatasetV2 (52D FK obs) |
| `diffusion_bc/trainer.py` | Phase 1 학습 루프 |
| `diffusion_actor_skrl.py` | skrl GaussianMixin Actor |
| `train_bc.py` | Phase 1 launch script |
| `config/agents/skrl_diffusion_ppo_cfg.yaml` | Phase 2 PPO config |

### 핵심 변경

- `NUM_OBSERVATIONS`: 60→52 (DiffusionActor 양팔 FK obs)
- `demo_bc_buffer.py`: `_load_episode_fk()` 추가, 기존 60D 검증 하드코딩으로 수정
- `pour_right_env.py`: `_get_observations()` → 52D, `_set_left_demo_reference()` → domain random 추가
- `pour_right_env_cfg.py`: `enable_left_arm_domain_random` 파라미터 추가
