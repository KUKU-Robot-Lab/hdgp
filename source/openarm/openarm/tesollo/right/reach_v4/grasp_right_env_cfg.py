...
class GraspRightEnvCfg(DirectRLEnvCfg):
    ...
    pregrasp_noise_x:      float = 0.02   # ±2.0cm 팔 진입 시작 위치 노이즈 (X)
    pregrasp_noise_y:      float = 0.02   # ±2.0cm 팔 진입 시작 위치 노이즈 (Y)
    pregrasp_noise_z:      float = 0.01   # ±1.0cm 팔 진입 시작 위치 노이즈 (Z)
    ...
    # -----------------------------------------------------------------------
    standoff_target_dist: float = 0.08          # 8cm Standoff Target in XY (컵 표면 앞 3.5cm)
    approach_xy_weight: float = 0.30            # XY 수평 거리 선형 감점 가중치
    approach_xy_fine_weight: float = 0.35       # XY 수평 8cm 정밀 밀착 Tanh 보상 가중치
    approach_xy_fine_std: float = 0.05          # XY 정밀 밴드폭 (5cm)
    approach_z_weight: float = 0.30             # Z 높이 오차 선형 감점 가중치
    approach_z_fine_weight: float = 0.35        # Z 높이 정밀 일치 Tanh 보상 가중치
    approach_z_fine_std: float = 0.03           # Z 정밀 밴드폭 (3cm)
    approach_align_weight: float = 0.20         # 손바닥 정면(+X) 컵 대면 정렬 보상
    approach_down_align_weight: float = 0.10    # 손가락(+Z) 하향 자세 안정 정렬 보상

    # 1. 목표 지점 감속 및 정지 보상 (Velocity Damping / Settling)
    approach_vel_damping_weight: float = 0.20   # 목표 도달 시 손바닥 감속 정지 보상 가중치
    approach_vel_damping_std: float = 0.05      # 감속 기준 선속도 폭 (5cm/s)
    approach_vel_gate_xy_std: float = 0.03      # 감속 보상 활성화 XY 게이트 폭 (3cm)
    approach_vel_gate_z_std: float = 0.02       # 감속 보상 활성화 Z 게이트 폭 (2cm)

    # 2. 테이블 바닥 긁힘 방지 (Table Clearance Guard)
    table_clearance_weight: float = 5.0         # 손끝 테이블 침범 페널티 가중치
    table_surface_z: float = 0.208              # 테이블 상판 env-local Z 높이 [m]
    table_clearance_margin: float = 0.015       # 손끝 안전 여유 마진 (1.5cm)

    # 3. 명확한 Reach 성공 판정 기준 (Success Metric)
    reach_success_xy_tol: float = 0.012         # XY 수평 거리 허용 오차 (1.2cm)
    reach_success_z_tol: float = 0.015          # Z 높이 허용 오차 (1.5cm)
    reach_success_align_min: float = 0.85       # 손바닥 정면 정렬 최소값 (cos 31도)
    reach_success_disp_max: float = 0.005       # 컵 최대 허용 변위 (5mm, 무충돌)
    reach_success_hold_steps: int = 5           # 성공 판정 요구 연속 유지 스텝수 (5 steps @60Hz)

    # 컵 충돌 페널티 및 기타 가드
    cup_lin_vel_penalty_weight: float = 2.0     # 컵 선속도 페널티
    cup_ang_vel_penalty_weight: float = 0.5     # 컵 각속도 페널티
    approach_xy_penalty_weight: float = 5.0     # 컵 변위 페널티
    freeze_hand: bool = True                    # 손가락 20축 완전 동결 (PREGRASP 자세 고정)
    early_collision_terminate: bool = True      # 컵 2.5cm 이상 밀릴 시 조기 종료

    # RH56F1 shared grasp-v2 reward contract (Grasp/Lift disabled for pure Reach)
    approach_weight: float = 2.0
    approach_sharpness: float = 8.0
    approach_tilt_penalty_weight: float = 0.08
    grasp_weight: float = 0.0
    ...
    grasp_xy_threshold: float = 0.015
    ...
    # object_spawn_xy_range: enable_adr=False 일 때만 쓰이는 fallback(±8cm). 기본은
    ...
    robot_cfg: ArticulationCfg = ArticulationCfg(
        ...
        actuators={
            ...
            # ★08.18 reach_v4(sensor_rl): 좌측은 2-DOF 프리즈매틱 그리퍼(l_hj_gripper_1/2).
