...
class GraspRightEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: GraspRightEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.reach_hold_steps_buf      = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        # ※ grasp_v2는 다물체 형상적응(닿는 곳서 멈춤)이 의도라 미적용 — v1(단순 실린더)만.
        ...
        # Reach v2: 손가락 20축 완전 동결 (Action Freezing)
        if getattr(self.cfg, "freeze_hand", False):
            self.finger_close_buf.zero_()
            hand_target = self.hand_open_pose.unsqueeze(0).expand(self.num_envs, -1)
        else:
    ...
    def _compute_intermediate_values(self) -> None:
        ...
        if self.palm_body_index >= 0:
            palm_body_pos = self.robot.data.body_pos_w[:, self.palm_body_index, :] - env_origins
            palm_quat = self.robot.data.body_quat_w[:, self.palm_body_index]
            # r_hl_palm_ee offset: [X=+0.028m (피부 정면), Y=0.0m, Z=+0.040m (손바닥 중심)]
            palm_ee_offset_local = torch.tensor([0.028, 0.0, 0.040], device=self.device).expand(self.num_envs, -1)
            palm_ee_offset_world = quat_apply(palm_quat, palm_ee_offset_local)
            self.palm_center_pos = palm_body_pos + palm_ee_offset_world
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # Reach v3: 손바닥 정면(+X) 법선 벡터 및 손가락(+Z) 하향 자세 정렬
        palm_to_cup_xy = grasp_center[:, :2] - self.palm_center_pos[:, :2]
        palm_to_cup_dist_xy = palm_to_cup_xy.norm(dim=-1)
        palm_to_cup_dist_z = torch.abs(self.palm_center_pos[:, 2] - grasp_center[:, 2])
        ...
        fingertip_min_z = self.fingertip_pos[:, :, 2].min(dim=-1).values
        ...
        total, reward_terms, _ = compute_grasp_reward_terms(
            ...
            palm_to_cup_dist_xy=palm_to_cup_dist_xy,
            palm_to_cup_dist_z=palm_to_cup_dist_z,
            ...
            cup_lin_vel=self.cup.data.root_lin_vel_w,
            cup_ang_vel=self.cup.data.root_ang_vel_w,
            palm_alignment=palm_alignment,
            palm_down_alignment=palm_down_alignment,
            palm_lin_vel=palm_lin_vel,
            fingertip_min_z=fingertip_min_z,
            cfg=self.cfg,
        )
        ...
        self.extras["task/reach_success_rate"] = self.success_flag.float().mean()
        if self.grasp_adr is not None:
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # Reach v4: 컵 충돌 시 조기 종료 (Early Collision Termination)
        collision_knocked = torch.zeros_like(out_x)
        cup_xy_disp = (self.object_pos[:, :2] - self.object_init_pos[:, :2]).norm(dim=-1)
        if getattr(self.cfg, "early_collision_terminate", False):
            # 컵이 2.5cm 이상 밀리거나 20도 이상 기울어지면 충돌로 판정하여 즉시 실패 종료
            collision_knocked = (cup_xy_disp > 0.025) | (cup_z_world[:, 2] < 0.94)

        terminated = out_x | out_y | fallen | tipped_active | collision_knocked
        truncated  = self.episode_length_buf >= self.max_episode_length - 1

        # Reach v4: 명확한 Reach 성공 판정 (Standoff XY 거리, Z 높이, 손바닥 정렬, 무충돌, 연속 hold_steps 유지)
        standoff_target = getattr(self.cfg, "standoff_target_dist", 0.08)
        xy_tol = getattr(self.cfg, "reach_success_xy_tol", 0.012)
        z_tol = getattr(self.cfg, "reach_success_z_tol", 0.015)
        align_min = getattr(self.cfg, "reach_success_align_min", 0.85)
        disp_max = getattr(self.cfg, "reach_success_disp_max", 0.005)
        hold_target = getattr(self.cfg, "reach_success_hold_steps", 5)

        palm_to_cup_xy = self.object_pos[:, :2] - self.palm_center_pos[:, :2]
        dist_xy = palm_to_cup_xy.norm(dim=-1)
        dist_xy_err = torch.abs(dist_xy - standoff_target)
        dist_z_err = torch.abs(self.palm_center_pos[:, 2] - self.object_pos[:, 2])
        ...
        valid_cup = ~(out_x | out_y | fallen | tipped_active | collision_knocked)

        reach_condition_now = (
            (dist_xy_err <= xy_tol)
            & (dist_z_err <= z_tol)
            & (palm_align >= align_min)
            & (cup_xy_disp <= disp_max)
            & valid_cup
        )
        self.reach_hold_steps_buf = torch.where(
            reach_condition_now,
            self.reach_hold_steps_buf + 1,
            torch.zeros_like(self.reach_hold_steps_buf),
        )
        reach_success = self.reach_hold_steps_buf >= hold_target
        self.success_flag.copy_(reach_success)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None) -> None:
        ...
        self.reach_hold_steps_buf[env_ids] = 0
