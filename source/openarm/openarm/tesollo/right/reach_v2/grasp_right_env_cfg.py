...
class GraspRightEnvCfg(DirectRLEnvCfg):
    ...
    # -----------------------------------------------------------------------
    # Reach v2 Core Parameters (Standoff, Finger Freezing, Disturbance)
    # -----------------------------------------------------------------------
    standoff_target_dist: float = 0.08          # 8cm Standoff Target (컵 표면 앞 3.5cm)
    approach_coarse_weight: float = 0.30        # Coarse linear penalty weight
    approach_fine_weight: float = 0.35          # Fine tanh reward weight
    approach_fine_std: float = 0.05             # 5cm width for tanh peak
    approach_align_weight: float = 0.15         # Palm normal to cup direction alignment
