...
class GraspRightEnv(DirectRLEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # Reach v2: 손바닥 정면 벡터-컵 방향 벡터 내적 정렬 (Palm Normal Alignment)
        palm_to_cup_xy = grasp_center[:, :2] - self.palm_center_pos[:, :2]
        palm_to_cup_dir = torch.zeros(self.num_envs, 3, device=self.device)
        palm_to_cup_dir[:, :2] = palm_to_cup_xy / palm_to_cup_xy.norm(dim=-1, keepdim=True).clamp(min=1e-6)

        palm_quat = self.robot.data.body_quat_w[:, self.palm_body_index]
        palm_normal_local = torch.tensor([0.0, 0.0, 1.0], device=self.device).expand(self.num_envs, -1)
