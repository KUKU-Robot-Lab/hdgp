...
def compute_grasp_reward_terms(
    *,
    ...
    # -----------------------------------------------------------------------
    # Reach v2: 8cm Standoff Target & Disturbance Penalty
    # -----------------------------------------------------------------------
    # 1. [Standoff Error] 컵 중심 8cm(표면 앞 3.5cm) 기준 거리 오차
    standoff_target = _cfg_float(cfg, "standoff_target_dist", 0.08)
    dist_error = torch.abs(palm_to_cup_dist - standoff_target)

    # 2. [Coarse] 원거리 선형 거리오차 감점 (먼 곳에서 8cm 지점으로 빠른 유도)
    approach_coarse = -_cfg_float(cfg, "approach_coarse_weight", 0.30) * dist_error

    # 3. [Fine] 8cm 정밀 밀착 Tanh 가점 (8cm 정밀 안착 시 피크 만점, 과접근 시 감점)
    fine_std = _cfg_float(cfg, "approach_fine_std", 0.05)
    approach_fine = _cfg_float(cfg, "approach_fine_weight", 0.35) * (1.0 - torch.tanh(dist_error / fine_std))

    # 4. [Align] 손바닥 정면-컵 방향 벡터 내적 정렬 보상
    align_quality = upright_quality if palm_alignment is None else palm_alignment.clamp(0.0, 1.0)
    align_reward = _cfg_float(cfg, "approach_align_weight", 0.15) * align_quality

    # 5. [Disturbance Penalty] 컵 선속도/각속도/변위 외란 감점
