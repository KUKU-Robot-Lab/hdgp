...
# capture_mask
# ---------------------------------------------------------------------------
def test_capture_mask_accepts_deep_tilt_over_target_with_beads():
    # tilt deep, beads in source, pour-point over target → qualifies
    tilt = torch.tensor([0.45])
    src = torch.tensor([0.95])
    mouth = torch.tensor([0.03])
    mask = capture_mask(tilt, src, mouth, tilt_min=0.40, src_min=0.80, mouth_max=0.08)
    assert bool(mask[0]) is True


def test_capture_mask_rejects_shallow_tilt():
    tilt = torch.tensor([0.30])  # below tilt_min
    src = torch.tensor([0.95])
    mouth = torch.tensor([0.03])
    mask = capture_mask(tilt, src, mouth, tilt_min=0.40, src_min=0.80, mouth_max=0.08)
...
def test_capture_mask_rejects_already_poured_seed():
    # beads no longer in source → would be a free-pour seed (hacking) → reject
    tilt = torch.tensor([0.45])
    src = torch.tensor([0.40])  # below src_min
    mouth = torch.tensor([0.03])
    mask = capture_mask(tilt, src, mouth, tilt_min=0.40, src_min=0.80, mouth_max=0.08)
    assert bool(mask[0]) is False
...
def test_capture_mask_rejects_not_over_target():
    tilt = torch.tensor([0.45])
    src = torch.tensor([0.95])
    mouth = torch.tensor([0.20])  # above mouth_max (not over target)
    mask = capture_mask(tilt, src, mouth, tilt_min=0.40, src_min=0.80, mouth_max=0.08)
    assert bool(mask[0]) is False


def test_capture_mask_elementwise_batch():
    tilt = torch.tensor([0.45, 0.30, 0.45, 0.45])
    src = torch.tensor([0.95, 0.95, 0.40, 0.95])
    mouth = torch.tensor([0.03, 0.03, 0.03, 0.20])
    mask = capture_mask(tilt, src, mouth, tilt_min=0.40, src_min=0.80, mouth_max=0.08)
...
# ---------------------------------------------------------------------------
def _make_bank(capacity=4, num_arm=7, num_hand=20, num_beads=3):
