...
class GraspRightEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: GraspRightEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        if getattr(cfg, "collect_sdf_cup_assets", False):
            ...
            for _a in cfg.cup_cfg.spawn.assets_cfg:
                if "cup_big" in str(getattr(_a, "usd_path", "")):
                    ...
                    _n += 1
            print(f"[grasp_sensor] collect_sdf_cup_assets: {_n} cup assets → {_sdf_path}", flush=True)
        ...
        if getattr(cfg, "collect_pour_matched_gains", False):
            ...
            print("[grasp_sensor] collect_pour_matched_gains: arm→400/80, abduction→200/35", flush=True)
        ...
        # ★2026-08-18 palm_delta_xyz 가 축별 튜플 (x, y, z). 구 스칼라 설정도
        #   그대로 받도록 정규화한다(외부 실험 cfg 가 float 를 넣을 수 있다).
    ...
    def _setup_scene(self) -> None:
        ...
        if bool(getattr(self.cfg, "collect_with_beads", False)):
            ...
            print(
                f"[grasp_sensor] collect_with_beads: {int(self.cfg.collect_bead_count)} beads "
    ...
    def _setup_geometric_fabrics(self) -> None:
        ...
        self.open_tesollo_fabric = OpenArmTeoslloPoseFabric(
            ...
            # ★08.18 grasp_sensor(a1): DG-5F 원본 fabric URDF 사용 — sensor_rl USD 와 정합
            #   (right/pour_v1 의 기본값과 동일). bi_s URDF 를 쓰면 손 FK 가 어긋난다.
    ...
    def _build_home_pose(self) -> None:
        ...
        홈 자세의 도달성·비가림은 URDF FK/IK 로 사전 검증했다(cfg 주석 참조).
        """
        ...
        # 홈은 절대 workspace 안에 있어야 한다 — 벗어나면 조용히 클램프되지 않고 알린다.
        ...
        print(f"[grasp_sensor] 고정 홈 palm={hp} → q_home="
        ...
        _have = self.left_arm_zero_pos[0][:NUM_ARM_DOF]
        _err = (_have - _want).abs().max().item()
        if _err > 0.05:
            raise ValueError(
                "유휴 팔 rest 가 홈의 미러와 어긋난다(최대 오차 "
                f"{_err:.4f} rad > 0.05). preset 의 LEFT_ARM_REST_JOINT_POS 를 "
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # ★08.19 P5(reward-audit ACCEPT): 실리프트 증거 추가. 구 정의(latch & tip>=2)는
        #   텔레포트 리셋에서 거의 공짜라, 들지 않은 latch 가 ADR(wrench/질량/스폰) 램프와
        #   warm entry 를 오염시켰다. 이 플래그는 ①ADR trigger ②episode_success_rate 로깅
        #   ③transfer_entry(warm export 입구) 세 곳에 쓰인다 — 전부 강화가 옳은 방향.
        #   높이 임계 0.5×(2cm)인 이유: 순간 지표라 full 4cm 요구 시 fraction 이 과소해
        #   ADR 영구 정지 위험 — "확실히 들리기 시작"의 최소 증거로 절충(정지 시 0.25× 완화).
        #   ⚠ episode_success_rate 의 정의가 이 커밋부터 바뀐다 — 이전 run 과 비교 금지.
        _lifted_evidence = (
            (self.object_pos[:, 2] - self.object_init_pos[:, 2])
            >= 0.5 * float(self.cfg.lift_success_height)
        )
        _upright_evidence = cup_z_world[:, 2] >= math.cos(
            math.radians(float(self.cfg.success_upright_max_deg))
        )
        self.success_flag.copy_(
            self.lift_ready_latched_buf & grasped & valid_cup
            & _lifted_evidence & _upright_evidence
        )
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None) -> None:
        ...
        if self.demo_grasp_reset_bank is not None:
            ...
            # ★2026-08-18 축별 분리. x 와 y 의 목표 폭이 달라(±0.05 / ±0.10)
            #   스칼라 하나로는 표현이 안 됐다.
            _x_range = (
