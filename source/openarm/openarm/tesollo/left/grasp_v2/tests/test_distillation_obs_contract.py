...
grasp_left_env.py 전체는 isaaclab/fabrics 없이 import 할 수 없으므로,
...
_ENV_SRC = _TASK_DIR / "grasp_left_env.py"
_CONSTANTS_SRC = _TASK_DIR / "grasp_left_constants.py"
...
def test_teacher_obs_dim_matches_action_dim():
    ...
    """
    assert C["NUM_OBS_BASE"] == 193 + 5
    assert C["NUM_CRITIC_OBS_BASE"] == 247 + 5
    assert C["NUM_STUDENT_OBS"] == 185 + 5
