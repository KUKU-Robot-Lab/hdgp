...
class GraspLeftEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: GraspLeftEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        if cfg.enable_adr:
            ...
            # DEXTRAH starting_adr_increments 이식(right 동일): distillation 은 env 를
            # teacher 작동점(만렙)에 고정 — ADR 0 이면 abduction 잠김·시연 왜곡.
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        if self.cfg.finger_control_mode == "per_finger":
            if bool(self.cfg.couple_four_fingers):
                ...
                # action 으로 thumb+2~3지 국소최적에 고착한 것을 원천 차단(right
                # lstm_test1 3600ep 렌더 실증). 4지를 공통 신호(평균)로 묶어 "특정
                # 손가락만 이탈"을 표현 불가하게 한다. 엄지(0)는 opposition 회전 위해
                # 독립 유지. 접촉 시 개별 동결(gate20)은 그대로라 각 손가락이 닿는
                # 지점서 멈춤 → 최종 조합은 물체가 결정. cfg.couple_four_fingers 주석 참조.
