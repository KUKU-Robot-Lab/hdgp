"""Demo-derived reset references for 5g_grasp_left_v1-2.

right/grasp_v1 의 좌우 미러. HDF5 스키마 키("obs/right_arm_joint_pos" 등)는
원본 데모 데이터셋(pour_v1_a11~a20.hdf5, 오른팔 레코딩)의 실제 필드명이므로
그대로 유지한다 — grasp_left_env_cfg.py 의 enable_demo_grasp_reset 은 기본
False 이며, 이 클래스는 왼팔 데모 데이터셋이 별도로 준비되기 전까지 사용하지
않는다(다물체 확장으로 인해 grasp_v2 도 동일 이유로 데모 리셋을 껐다).
"""
