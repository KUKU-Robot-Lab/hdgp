...
v3: PhysX 5 PBD Particle System 기반 액체 시뮬레이션 + 리워드 재설계
  - v2: Rigid Body bead(20개) → v3: PhysX PBD particle(50개)
  - Spill: mass conservation (source/target 컵 모두 밖 = spill)
  - Success: 10개 이상 particle target cup 입구 통과

Action (11D):
  [0:6]  6D palm pose → Fabrics IK → arm 7 DOF
  [6:11] 5D per-finger lerp (freeze_grasp=True → 항상 grasp 유지)
...
from pxr import Gf, Sdf, UsdGeom, PhysxSchema, UsdShade
...
class GraspRightEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: GraspRightEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # --- Particle 오프셋 (source cup 내부 초기 배치, 원형 층 구조) ---
        self.num_particles = int(self.cfg.num_particles)
        _particle_offsets = []
        particles_per_layer = 5
        for i in range(self.num_particles):
            layer = i // particles_per_layer
            slot = i % particles_per_layer
            angle = (2.0 * math.pi * slot / particles_per_layer) + (0.35 * layer)
            radius = 0.012 + 0.004 * (layer % 2)
            z = 0.008 + self.cfg.particle_spacing * layer
            _particle_offsets.append([radius * math.cos(angle), radius * math.sin(angle), z])
        self._particle_offsets_source_cup_b = torch.tensor(_particle_offsets, device=self.device)
        ...
        # --- Particle 상태 버퍼 (v3: PhysX PBD, bead → particle 대체) ---
        N, K = self.num_envs, self.num_particles
        self._particle_pos_w = torch.zeros(N, K, 3, device=self.device)
        self._particle_in_target = torch.zeros(N, K, dtype=torch.bool, device=self.device)
        self._particle_in_source = torch.zeros(N, K, dtype=torch.bool, device=self.device)
        self._particle_crossed_target_mouth = torch.zeros(N, K, dtype=torch.bool, device=self.device)
        self._prev_particle_target_local_z = torch.full((N, K), 10.0, device=self.device)
        self._particle_cross_count = torch.zeros(N, dtype=torch.long, device=self.device)
        self._particle_cross_fraction = torch.zeros(N, device=self.device)
        self._particle_in_target_fraction = torch.zeros(N, device=self.device)
        self._particle_in_source_fraction = torch.zeros(N, device=self.device)
        self._particle_centroid_w = torch.zeros(N, 3, device=self.device)
        self._spill_ratio = torch.zeros(N, device=self.device)
        # particle system USD 경로 (per-env, _setup_particles에서 채워짐)
        self._particle_point_paths: list[str] = []
    ...
    def _setup_scene(self) -> None:
        ...
        # PhysX PBD Particle System 생성 (clone 이후 per-env 개별 생성)
    ...
    # ------------------------------------------------------------------
    # PhysX 5 PBD Particle System 관련 메서드 (v3 신규)
    # ------------------------------------------------------------------
    def _setup_particles(self) -> None:
        """per-env PhysX PBD 파티클 시스템 생성 (clone 이후 호출)."""
        self._stage = omni.usd.get_context().get_stage()
        self._particle_point_paths = []

        for env_id in range(self.num_envs):
            env_prefix = f"/World/envs/env_{env_id}"
            sys_path = Sdf.Path(f"{env_prefix}/FluidSystem")
            pts_path = Sdf.Path(f"{env_prefix}/FluidParticles")

            # Particle System 생성
            particleUtils.add_physx_particle_system(
                self._stage,
                sys_path,
                particle_contact_offset=self.cfg.particle_spacing * 0.5,
                contact_offset=self.cfg.particle_spacing * 0.75,
                rest_offset=self.cfg.particle_spacing * 0.35,
                solid_rest_offset=0.0,
                fluid_rest_offset=self.cfg.particle_spacing * 0.3,
                solver_position_iterations=4,
                simulation_owner=Sdf.Path("/physicsScene"),
            )

            # 유체 재질 생성 및 바인딩
            mat_path = Sdf.Path(f"{env_prefix}/FluidMaterial")
            self._apply_particle_material(mat_path, sys_path)

            # Points prim (파티클 위치 저장)
            points = UsdGeom.Points.Define(self._stage, pts_path)
            points.CreatePointsAttr([Gf.Vec3f(0.0, 0.0, -10.0)] * self.num_particles)
            points.CreateWidthsAttr([self.cfg.particle_spacing] * self.num_particles)

            # Particle Set API 적용 및 시스템 연결
            particle_api = PhysxSchema.PhysxParticleSetAPI.Apply(points.GetPrim())
            particle_api.CreateParticleSystemRel().SetTargets([sys_path])

            # IsFluid 속성 설정
            physx_particle_api = PhysxSchema.PhysxParticleAPI.Apply(points.GetPrim())
            physx_particle_api.CreateSelfCollisionAttr(True)

            self._particle_point_paths.append(str(pts_path))

    def _apply_particle_material(self, mat_path: Sdf.Path, sys_path: Sdf.Path) -> None:
        """PhysxParticleMaterialAPI 생성 및 파티클 시스템에 바인딩."""
        material_prim = UsdShade.Material.Define(self._stage, mat_path)
        particle_mat_api = PhysxSchema.PhysxParticleMaterialAPI.Apply(material_prim.GetPrim())
        particle_mat_api.CreateViscosityAttr(self.cfg.viscosity)
        particle_mat_api.CreateSurfaceTensionAttr(self.cfg.surface_tension)
        particle_mat_api.CreateCohesionAttr(self.cfg.cohesion)
        particle_mat_api.CreateAdhesionAttr(self.cfg.adhesion)
        UsdShade.MaterialBindingAPI.Apply(
            self._stage.GetPrimAtPath(sys_path)
        ).Bind(material_prim)

    def _get_particle_positions(self) -> None:
        """USD PointsAttr를 통해 파티클 위치 읽기 → self._particle_pos_w 업데이트.

        Note: CPU-via-USD 방식. GPU tensor 직접 접근은 PhysX sim view API가 필요하며
              Isaac Lab에서 미지원. 성능 이슈 시 decimation 적용 고려.
        """
        for env_id in range(self.num_envs):
            pts_prim = self._stage.GetPrimAtPath(self._particle_point_paths[env_id])
            if not pts_prim.IsValid():
                continue
            positions = UsdGeom.Points(pts_prim).GetPointsAttr().Get()
            if positions is not None and len(positions) == self.num_particles:
                self._particle_pos_w[env_id] = torch.tensor(
                    [[p[0], p[1], p[2]] for p in positions],
                    dtype=torch.float32,
                    device=self.device,
                )

    def _compute_initial_particle_positions(
        self, cup_pos: torch.Tensor, cup_quat: torch.Tensor
    ) -> torch.Tensor:
        """source cup 기준 로컬 오프셋 → world 좌표 변환. (N, K, 3)"""
        n = cup_pos.shape[0]
        local_offsets = self._particle_offsets_source_cup_b.unsqueeze(0).expand(n, -1, -1)
        cup_quat_exp = cup_quat.unsqueeze(1).expand(-1, self.num_particles, -1)
        particle_pos_w = cup_pos.unsqueeze(1) + quat_apply(
            cup_quat_exp.reshape(-1, 4),
            local_offsets.reshape(-1, 3),
        ).reshape(n, self.num_particles, 3)
        return particle_pos_w  # (N, K, 3)

    def _reset_particles(self, env_ids: Sequence[int], cup_root_state: torch.Tensor) -> None:
        """파티클을 source cup 내부로 재배치 (에피소드 reset 시)."""
        n = len(env_ids)
        cup_pos = cup_root_state[:n, :3]
        cup_quat = cup_root_state[:n, 3:7]
        particle_pos_w = self._compute_initial_particle_positions(cup_pos, cup_quat)

        for i, env_id in enumerate(env_ids):
            pts_prim = self._stage.GetPrimAtPath(self._particle_point_paths[env_id])
            if not pts_prim.IsValid():
                continue
            positions = particle_pos_w[i].cpu().tolist()
            UsdGeom.Points(pts_prim).GetPointsAttr().Set(
                [Gf.Vec3f(*p) for p in positions]
            )

        # 파티클 버퍼 초기화
        self._particle_in_target[env_ids] = False
        self._particle_in_source[env_ids] = False
        self._particle_crossed_target_mouth[env_ids] = False
        self._prev_particle_target_local_z[env_ids].fill_(10.0)
        self._particle_cross_count[env_ids] = 0
        self._particle_cross_fraction[env_ids] = 0.0
        self._particle_in_target_fraction[env_ids] = 0.0
        self._particle_in_source_fraction[env_ids] = 0.0
        self._particle_centroid_w[env_ids].zero_()
        self._spill_ratio[env_ids] = 0.0

    def _hide_particles(self, env_ids: Sequence[int]) -> None:
        """파티클을 시야 밖(z=-10)으로 이동 (warmstart 수집 모드용)."""
        hidden_pos = [Gf.Vec3f(0.0, 0.0, -10.0)] * self.num_particles
        for env_id in env_ids:
            pts_prim = self._stage.GetPrimAtPath(self._particle_point_paths[env_id])
            if pts_prim.IsValid():
                UsdGeom.Points(pts_prim).GetPointsAttr().Set(hidden_pos)
    ...
    def _compute_particle_flags(self) -> None:
        """파티클이 source/target cup 내부 또는 target mouth를 통과했는지 계산.

        v3 변경:
          - bead(RigidObjectCollection) → particle(PhysX PBD, USD 경유 위치 읽기)
          - Spill: mass conservation (source AND target 컵 모두 밖 = spill)
            v2: bead_env_z < 0.230 (z-threshold) → v3: ~in_source & ~in_target
        """
        # 파티클 위치 읽기 (USD PointsAttr 경유)
        self._get_particle_positions()
        particle_pos_w = self._particle_pos_w  # (N, K, 3)

        self._particle_centroid_w.copy_(particle_pos_w.mean(dim=1))
        ...
        particle_xy_to_target = torch.norm(pos_in_target[..., :2], dim=-1)
        particle_in_target = (
            (particle_xy_to_target <= self.cfg.target_inner_radius)
            & (pos_in_target[..., 2] >= self.cfg.target_inside_z_min)
            & (pos_in_target[..., 2] <= self.cfg.target_inside_z_max)
        )
        self._particle_in_target.copy_(particle_in_target)
        ...
        particle_xy_to_source = torch.norm(pos_in_source[..., :2], dim=-1)
        particle_in_source = (
            (particle_xy_to_source <= self.cfg.source_inner_radius)
            & (pos_in_source[..., 2] >= self.cfg.source_inside_z_min)
            & (pos_in_source[..., 2] <= self.cfg.source_inside_z_max)
        )
        self._particle_in_source.copy_(particle_in_source)

        # ---- Target mouth 통과 판정 (누적) ----
        mouth_crossed_now = (
            (particle_xy_to_target <= self.cfg.target_inner_radius)
            & (self._prev_particle_target_local_z > self.cfg.target_mouth_z)
            & (pos_in_target[..., 2] <= self.cfg.target_mouth_z)
        )
        self._particle_crossed_target_mouth |= mouth_crossed_now
        self._particle_cross_count.copy_(self._particle_crossed_target_mouth.sum(dim=-1).long())
        self._particle_cross_fraction.copy_(self._particle_crossed_target_mouth.float().mean(dim=-1))
        self._particle_in_target_fraction.copy_(self._particle_in_target.float().mean(dim=-1))
        self._particle_in_source_fraction.copy_(self._particle_in_source.float().mean(dim=-1))

        # ---- Spill: mass conservation (v2 z-threshold → v3 source∩target 밖) ----
        particle_spilled = ~self._particle_in_target & ~self._particle_in_source
        self._spill_ratio.copy_(particle_spilled.float().mean(dim=-1))
        self._prev_particle_target_local_z.copy_(pos_in_target[..., 2])
    ...
    def _compute_intermediate_values(self) -> None:
        ...
        # Particle flags & spill (v3: PhysX PBD)
        self._compute_particle_flags()
    ...
    def _get_observations(self) -> dict:
        ...
        bead_pos_clean = self._particle_centroid_w
        ...
        particle_pos_rel_source_cup = quat_apply_inverse(
            right_cup_quat_clean,
            bead_pos_clean - right_cup_pos_clean,
        )
        particle_pos_rel_target_cup = quat_apply_inverse(
            left_cup_quat_clean,
        ...
        critic_obs = torch.cat([
            ...
            distal_force_norm,                                 # 5
            cup_height_delta,                                  # 1
            particle_pos_rel_source_cup,                       # 3
            particle_pos_rel_target_cup,                       # 3
            ...
            self._particle_cross_fraction.unsqueeze(1),         # 1
        ], dim=-1)   # 143D
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # ---- Benefit: tilt_control (v3 신규: 부드러운 기울기 유도) ----
        # cos(θ) × exp(-scale × |dθ/dt|): 기울임 정도와 기울임 속도 안정성 동시 보상
        prev_tilt_cos = self._source_up_dot_world  # 이전 스텝 값 재활용
        tilt_rate = torch.abs(prev_tilt_cos - target_tilt_cos)
        tilt_control_reward = aligned_directional_tilt_benefit * torch.exp(
            -self.cfg.reward_tilt_control_scale * tilt_rate
        )

        # ---- Benefit: actual liquid transfer to target (particle 기반) ----
        pour_accuracy_benefit = self._particle_cross_fraction
        ...
        # v3: mass conservation spill (source∩target 밖 파티클 비율)
        ...
        task_benefit = (
            ...
            + self.cfg.benefit_tilt_control_weight * tilt_control_reward  # v3 신규
        )
        outcome_benefit = (
        ...
        self.extras["particle_in_source_rate"]  = self._particle_in_source_fraction.mean()
        self.extras["particle_in_target_rate"]  = self._particle_in_target_fraction.mean()
        self.extras["particle_cross_fraction"]  = self._particle_cross_fraction.mean()
        self.extras["particle_cross_count"]     = self._particle_cross_count.float().mean()
        self.extras["reward_tilt_control"]      = tilt_control_reward.mean()
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        self.success_flag.copy_(self._particle_cross_count >= self.cfg.success_particle_cross_count)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None) -> None:
        ...
        # ---- 파티클 초기화 (v3: PhysX PBD) ----
        if self._warmstart_collect_mode:
            self._hide_particles(env_ids)
        else:
            self._reset_particles(env_ids, cup_root_state[:, :7])
        ...
        # particle 버퍼는 _reset_particles에서 초기화됨 (warmstart 모드에서만 여기서 추가 초기화)
        if self._warmstart_collect_mode:
            self._particle_in_target[env_ids] = False
            self._particle_in_source[env_ids] = False
            self._particle_crossed_target_mouth[env_ids] = False
    ...
    # _sample_bead_states_inside_cup, _hide_beads → v3에서 _compute_initial_particle_positions,
    # _reset_particles, _hide_particles로 대체 (위 PhysX particle 메서드 블록 참조)
    ...
    def _reset_from_warmstart_cache(self, env_ids: Sequence[int]) -> None:
        ...
        # v3: PhysX PBD 파티클 초기화
        self._reset_particles(env_ids, cup_pose_world)
        ...
        self.middle_contact_force_raw[env_ids].zero_()
        self.middle_binary_contact_buf[env_ids] = False
        # particle 버퍼는 _reset_particles에서 이미 초기화됨
        self._no_tip_force_steps[env_ids] = 0
        self.success_flag[env_ids] = False
