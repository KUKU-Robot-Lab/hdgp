...
v3: PhysX 5 PBD Particle System + 리워드 재설계
    Warmstart(v7 grasp) 기반, 컵이 이미 파지·들린 상태로 시작 → transport + tilt + pour 학습

Action (11D):
  [0:6]  6D palm pose (x,y,z,ez,ey,ex) → Fabrics IK → arm 7 DOF
  [6:11] 5D per-finger lerp (freeze_grasp=True → 항상 1.0 강제)

Actor Observation (101D) — sim2real 가능 (v2와 동일):
  arm_joint_pos:            7
  arm_joint_vel:            7
  finger_joint_pos:        20
  finger_joint_vel:        20
...
Critic Extra (42D) — sim-only privileged:
  ...
  cup_height_delta:         1
  particle_pos_rel_source:  3  ← v2 bead_pos_rel_source_cup
  particle_pos_rel_target:  3  ← v2 bead_pos_rel_target_cup
  mouth_distance:           1
  mouth_xy_distance:        1
  mouth_z_clearance:        1
  ...
  particle_cross_fraction:  1  ← v2 bead_in_target_flag
  Total:                   42

Critic Total: 101 + 42 = 143D (v2와 동일)
