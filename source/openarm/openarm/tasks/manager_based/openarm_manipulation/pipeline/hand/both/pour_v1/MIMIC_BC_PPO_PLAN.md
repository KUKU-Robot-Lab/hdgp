...
hdgp/source/openarm/openarm/tasks/manager_based/openarm_manipulation/
  pipeline/hand/both/pour_v1_mimic/
    [ ] __init__.py
    [ ] pour_mimic_env.py
    [ ] pour_mimic_env_cfg.py
    [ ] pour_mimic_subtask.py
    config/
      [ ] __init__.py
```
...
gym.register(
    id="OpenArm-Pour-V1-v0",             # BC 평가용 (Mimic 없음)
    entry_point="...pour_mimic_env:PourMimicEnv",
    kwargs={"cfg": PourMimicEnvCfg()},
)
gym.register(
    id="OpenArm-Pour-V1-Mimic-v0",       # 데이터 증강용
    entry_point="...pour_mimic_env:PourMimicEnv",
...
```
sim2real/scripts/
  [ ] ros2_demo_recorder.py       핵심 수집 스크립트 (신규)
  [ ] ros2_teleop_device.py       IsaacLab Se3Device 커스텀 구현 (신규)
```
...
./isaaclab.sh -p scripts/imitation_learning/isaaclab_mimic/annotate_demos.py \
  --task OpenArm-Pour-V1-Mimic-v0 --auto \
...
./isaaclab.sh -p scripts/imitation_learning/robomimic/train.py \
  --task OpenArm-Pour-V1-v0 --algo bc_rnn \
...
./isaaclab.sh -p scripts/imitation_learning/robomimic/play.py \
  --task OpenArm-Pour-V1-v0 --num_rollouts 50 \
  --horizon 600 \
...
Phase 1 (7~10일): PourMimicEnv 구현  ← 핵심 병목
  ├─ pour_v1_mimic/__init__.py
  ├─ pour_mimic_env.py  (ManagerBasedRLMimicEnv + Fabrics 내장)
  ├─ pour_mimic_env_cfg.py
  └─ pour_mimic_subtask.py

Phase 2 (5~7일): ROS2 텔레옵 브리지
  ├─ ros2_demo_recorder.py
  └─ ros2_teleop_device.py
