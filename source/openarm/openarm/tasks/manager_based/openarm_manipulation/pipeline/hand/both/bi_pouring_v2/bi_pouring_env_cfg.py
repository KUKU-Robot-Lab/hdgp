...
class BiPouringEnvCfg(DirectRLEnvCfg):
    ...
    robot_cfg: ArticulationCfg = ArticulationCfg(
        ...
        actuators={
            ...
            # hand / left-holder: 순수 시각용 고정 형상 → stiffness=0, damping=0.
            # write_joint_state_to_sim으로 매 step teleport하므로 PD 불필요.
            # stiffness>0 이면 cup-hand 접촉 시 PD 증폭 → arm 역전파 불안정 유발.
    ...
    right_source_cup_cfg: RigidObjectCfg = RigidObjectCfg(
        ...
        spawn=UsdFileCfg(
            ...
            # contact_offset=-0.1: cup-robot 충돌 비활성화.
            # bead-cup 충돌도 꺼지지만, bead는 _apply_action에서 매 step
            # source cup 프레임에 teleport하므로 물리 충돌 불필요.
