...
def _scale_actions(raw_actions: torch.Tensor, lower: torch.Tensor, upper: torch.Tensor) -> torch.Tensor:
    scaled = 0.5 * (raw_actions + 1.0) * (upper - lower) + lower
    return torch.max(torch.min(scaled, upper), lower)
