...
class PourEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: PourEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.distal_contact_force_raw  = torch.zeros(self.num_envs, NUM_FINGERTIPS, device=self.device)
        self.distal_binary_contact_buf = torch.zeros(self.num_envs, NUM_FINGERTIPS, dtype=torch.bool, device=self.device)

        self.middle_contact_force_raw  = torch.zeros(self.num_envs, NUM_FINGERTIPS, device=self.device)
        self.middle_binary_contact_buf = torch.zeros(self.num_envs, NUM_FINGERTIPS, dtype=torch.bool, device=self.device)
