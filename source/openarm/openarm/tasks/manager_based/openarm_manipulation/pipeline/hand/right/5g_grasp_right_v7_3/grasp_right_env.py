...
Episode (18s @ 60Hz):
  ...
  Transport phase (840~1079): palm target transport
"""
...
class GraspRightEnv(DirectRLEnv):
    ...
    Episode:
      ...
      Lift      phase (step 480~719):  palm target lift + frozen hand
      Stabilize phase (step 720~839):  lifted-pose hold
      Transport phase (step 840~1079): palm target transport
    """
    ...
    def __init__(self, cfg: GraspRightEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.demo_lift_arm_target = (
            self.demo_grasp_reset_bank.lift_medoid_arm_joint_pos
        ...
        self.lift_palm_target_pose_buf = torch.zeros(self.num_envs, 6, device=self.device)
        ...
        self._prev_total_contact_force_buf = torch.zeros(self.num_envs, device=self.device)
        ...
        self._success_upright_cos = math.cos(math.radians(cfg.success_upright_max_deg))
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        is_transport = self.episode_length_buf >= TRANSPORT_START_STEP
        ...
        # ---- Lift 진입 시 finger grip과 palm lift target 캡처 ----
        ...
        if just_entering_lift.any():
            current_palm = self.palm_pose_targets[just_entering_lift]
            lift_target = current_palm.clone()
            lift_target[:, 2] = lift_target[:, 2] + float(self.cfg.lift_palm_z_delta)
            lift_target = torch.max(
                torch.min(lift_target, self.palm_maxs.unsqueeze(0)),
                self.palm_mins.unsqueeze(0),
            )
            self.lift_palm_start_pose_buf[just_entering_lift] = current_palm
            self.lift_palm_target_pose_buf[just_entering_lift] = lift_target
        ...
        # prelift = demo-derived fixed target when demo reset is enabled, else legacy j4 lift.
        if self.demo_lift_arm_target is not None:
            actual_prelift = self.demo_lift_arm_target.unsqueeze(0).expand(self.num_envs, -1)
        else:
        ...
        just_entering_transport = self.episode_length_buf == TRANSPORT_START_STEP
        if just_entering_transport.any():
            ...
            goal = self.object_goal[just_entering_transport]
            transport_target = self._compute_transport_palm_target(current_palm, current_object, goal)
        ...
        lift_palm_pose = torch.lerp(
            ...
            self.lift_palm_target_pose_buf,
            lift_progress,
        )
        ...
        transport_palm_pose = torch.max(torch.min(transport_palm_pose, self.palm_maxs), self.palm_mins)

        palm_pose = torch.where(
            is_transport.unsqueeze(1),
            transport_palm_pose,
            torch.where(is_post_grasp.unsqueeze(1), lift_palm_pose, palm_pose),
        )
        ...
        fabric_hand_target = torch.where(
            is_post_grasp.unsqueeze(1),
            self.lift_finger_pos_buf,
            hand_target,
        )
        self.fabric_q[:, NUM_ARM_DOF:] = fabric_hand_target
        self.fabric_qd[:, NUM_ARM_DOF:].zero_()

    def _apply_action(self) -> None:
        is_post_grasp = self.episode_length_buf >= LIFT_START_STEP
    ...
    # ------------------------------------------------------------------
    # Observations: Actor 110D | Critic 146D
    # ------------------------------------------------------------------
    def _get_observations(self) -> dict:
        ...
        # 9. cup-to-goal vector (3D) + phase ratio (1D)
        ...
        if actor_obs.shape[1] != NUM_OBSERVATIONS:
            raise RuntimeError(
                f"[v7-3] Actor obs dim mismatch: {actor_obs.shape[1]} != {NUM_OBSERVATIONS}"
            )

        # ==== Critic extra obs (36D) — clean state 사용 ====
        # cup velocity (6D)
        ...
        # critic actor_obs_clean (110D) — clean state 재조합
        actor_obs_clean = torch.cat([
            ...
            binary_contact,
            last_actions,
            self.object_goal - cup_pos_clean,
            phase_step_ratio,
        ], dim=-1)   # 110D
        ...
        if critic_obs.shape[1] != NUM_CRITIC_OBSERVATIONS:
            raise RuntimeError(
                f"[v7-3] Critic obs dim mismatch: {critic_obs.shape[1]} != {NUM_CRITIC_OBSERVATIONS}"
            )
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        is_grasp_phase = self.episode_length_buf < LIFT_START_STEP            # (N,) bool
        ...
        # ---- R6. stabilize ----
        stabilize_gate = (self.is_stabilize_phase | self.is_transport_phase).float()
        cup_lin_speed = self.cup.data.root_lin_vel_w.norm(dim=-1)
        cup_ang_speed = self.cup.data.root_ang_vel_w.norm(dim=-1)
        total_contact_force = self.contact_force_raw.sum(dim=-1)
        force_delta_abs = (total_contact_force - self._prev_total_contact_force_buf).abs()
        ...
        r6_stabilize = (
            ...
            * torch.exp(-force_delta_abs / max(float(self.cfg.stabilize_force_delta_threshold), 1e-6))
        ...
        # ---- R7. transport ----
        transport_gate = self.is_transport_phase.float()
        lifted = cup_height_delta > self.cfg.lift_success_height
        grasped = self.num_contacts_buf >= MIN_CONTACTS_FOR_SUCCESS
        upright_success = cup_uprightness >= self._success_upright_cos
        ...
        r7_transport = (
            ...
            * lifted.float()
            * grasped.float()
            * upright_success.float()
            * torch.exp(-goal_dist / max(float(self.cfg.transport_goal_dist_threshold), 1e-6))
        )
        ...
        self.extras["stabilize_reward"]      = r6_stabilize.mean()
        self.extras["transport_reward"]      = r7_transport.mean()
        ...
        self.extras["phase_lift"]            = self.is_lift_phase.float().mean()
        self.extras["phase_stabilize"]       = self.is_stabilize_phase.float().mean()
        self.extras["phase_transport"]       = self.is_transport_phase.float().mean()
        ...
        self._prev_total_contact_force_buf.copy_(total_contact_force)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # success: transport phase + lifted + grasped + upright + goal proximity.
        ...
        grasped = (self.num_contacts_buf >= MIN_CONTACTS_FOR_SUCCESS)
        upright_success = cup_z_world[:, 2] >= self._success_upright_cos
        goal_dist = (self.object_pos - self.object_goal).norm(dim=-1)
        transport_success = compute_transport_success_mask(
            lifted=lifted,
            grasped=grasped,
            upright=upright_success,
            goal_dist=goal_dist,
            goal_dist_threshold=self.cfg.transport_goal_dist_threshold,
        )
        self.success_flag.copy_(in_transport & transport_success)
        ...
        self.extras["transport_success_now"] = self.success_flag.float().mean()

        return terminated, truncated

    def _sample_transport_goals(self, env_ids: torch.Tensor, obj_pos_local: torch.Tensor) -> torch.Tensor:
        n = len(env_ids)
        goal_x = self.cfg.transport_goal_x_center + (
            torch.rand(n, device=self.device) - 0.5
        ) * 2.0 * self.cfg.transport_goal_xy_range
        goal_y = self.cfg.transport_goal_y_center + (
            torch.rand(n, device=self.device) - 0.5
        ) * 2.0 * self.cfg.transport_goal_xy_range
        goal_z = obj_pos_local[:, 2] + self.cfg.transport_goal_z_offset + (
            torch.rand(n, device=self.device) - 0.5
        ) * 2.0 * self.cfg.transport_goal_z_range
        goals = torch.stack([goal_x, goal_y, goal_z], dim=1)

        mins = torch.tensor(
            [
                self.cfg.obj_out_x_min + 0.02,
                self.cfg.obj_out_y_min + 0.02,
                self.cfg.obj_fallen_z + self.cfg.lift_success_height,
            ],
            dtype=goals.dtype,
            device=self.device,
        )
        maxs = torch.stack([
            torch.tensor(self.cfg.obj_out_x_max - 0.02, dtype=goals.dtype, device=self.device),
            torch.tensor(self.cfg.obj_out_y_max - 0.02, dtype=goals.dtype, device=self.device),
            self.palm_maxs[2] - 0.02,
        ])
        return torch.max(torch.min(goals, maxs.unsqueeze(0)), mins.unsqueeze(0))

    def _compute_transport_palm_target(
        self,
        current_palm: torch.Tensor,
        current_object: torch.Tensor,
        goal: torch.Tensor,
    ) -> torch.Tensor:
        goal_delta = goal - current_object
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None) -> None:
        ...
        self.object_goal[env_ids] = self._sample_transport_goals(env_ids, obj_pos_local)
        ...
        lift_palm_target = pregrasp_palm_pose.clone()
        lift_palm_target[:, 2] = lift_palm_target[:, 2] + float(self.cfg.lift_palm_z_delta)
        lift_palm_target = torch.max(
            torch.min(lift_palm_target, self.palm_maxs.unsqueeze(0)),
            self.palm_mins.unsqueeze(0),
        )
        self.lift_palm_start_pose_buf[env_ids] = pregrasp_palm_pose
        self.lift_palm_target_pose_buf[env_ids] = lift_palm_target
        self.transport_palm_start_pose_buf[env_ids] = lift_palm_target
        self.transport_palm_target_pose_buf[env_ids] = lift_palm_target
        ...
        if self.demo_lift_arm_target is not None:
            prelift_arm = self.demo_lift_arm_target.unsqueeze(0).expand(n, -1)
        else:
        ...
        self._prev_total_contact_force_buf[env_ids] = 0.0
