...
#
#   · 컵 자세는 **게이트**(AND) — 크게 기울어 들리면 성공으로 치지 않는다.
#     40° 를 넘으면 실패. cos(40°) = 0.766.
#   · jaw 수평은 **연속 보너스** — 게이트로 넣으면 겨우 붙은 파지가 한꺼번에 무너진다.
#     품질 = 1 - |sin(기울기)| 라 0° → 1.0, 18.6° → 0.68, 30° → 0.50.
import math as _math

CUP_UPRIGHT_MAX_TILT_DEG = 40.0
CUP_UPRIGHT_MIN_COS = _math.cos(_math.radians(CUP_UPRIGHT_MAX_TILT_DEG))   # 0.766
JAW_LEVEL_REWARD_WEIGHT = 5.0     # lifting 15 의 1/3 — 자세만 맞추는 국소최적을 막는 비율
