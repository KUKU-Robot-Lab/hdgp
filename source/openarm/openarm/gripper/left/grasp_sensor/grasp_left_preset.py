...
#     jaw 방위 θ = 30°, 접근축을 수평에서 아래로 φ = 15° (거의 수평 측면 파지)
...
GRASP_PALM_EULER_ZYX_DEG = (-150.0, 75.0, 180.0)
...
CUP_SPAWN_X_CENTER = 0.30         # tesollo/left/grasp_v1 과 동일
# ★y 는 0.20 → 0.28 로 바깥으로 옮겼다. grasp_v1(20관절 손)은 y=0.20 을 쓰지만 2지
#   그리퍼는 **jaw 를 수평으로 유지**해야 해서 자세 제약이 훨씬 빡빡하고(손목 j6 ±45°),
#   y<=0.20 에서는 정확 파지자세가 아예 안 나온다(실측 지도 전 x 에서 X).
#   y>=0.22 부터 전 구간 성립하므로 중심을 0.28 로 둔다.
CUP_SPAWN_Y_CENTER = 0.28
CUP_SPAWN_X_RANGE = 0.03          # ±m — x 0.27~0.33
CUP_SPAWN_Y_RANGE = 0.06          # ±m — y 0.22~0.34
