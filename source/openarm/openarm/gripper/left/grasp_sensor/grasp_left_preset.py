...
# ══════════════════════════════════════════════════════════════════════════════
# fab_test40 — palm 액션 2-스케일 (앵커 미세 박스)
#
# ★★t22~t39 열여덟 판이 `gate_rate < 0.5%` 였던 **정량적 원인**이다.
#   액션이 절대 규약이라 정책 지령 지터 = `σ × box_half`. t39 실측이 이 식을 따른다:
#       ep 65 σ0.91 → 예측 150mm / 실측 lateral  86mm
#       ep235 σ0.67 → 예측 111mm / 실측        115mm
#       ep406 σ0.60 → 예측  99mm / 실측         85mm
#   `grasp_ok` 문턱은 lateral < 30 mm 이고 **물리 여유는 ±13.25 mm**(개구 84.5 − 단면 58)/2.
#   현재 박스(half 165·170mm)의 노이즈 바닥:
#       σ0.60 → 142mm ✗   σ0.35 → 83mm ✗   σ0.20 → 47mm ✗   ← 어떤 σ 에서도 못 넘는다
#   보상을 어떻게 고쳐도 그리퍼가 닫힐 수 없었다. t38 이 σ0.20 까지 눌린 뒤에야 접촉이
#   살아난 것도, 그때조차 리프트가 0 이었던 것도 같은 식으로 설명된다.
#
# ★박스 축소는 **실측으로 기각됐다**(`docs/eval/fab_runs/palm_action_resolution.md`,
#   10,240 표본): 이송 지령이 x [0.246, 0.597] 로 박스를 거의 다 쓴다. 반면 파지 지령은
#   그 안 **115·66·79 mm 구석**에만 몰린다(적중률 0.42%).
#   → 균일 해상도 하나로 두 국면을 못 쓴다. **문맥 의존 박스**가 답이다.
#
# 액션 의미는 끝까지 **절대 규약**이다(kuka `compute_absolute_action` 유지).
# 바뀌는 것은 `[-1,1]` 을 무엇에 대해 펼치느냐 하나뿐이다:
#     COARSE  cmd = BOX_CENTER + a·BOX_HALF       (이송)
#     FINE    cmd = anchor     + a·FINE_HALF      (파지)
PALM_FINE_HALF = (0.0575, 0.033, 0.0395)  # m. 실측 파지 포락(115·66·79 mm)의 정확히 절반
# ★설계안에 없던 항 — **회전도 같은 크기의 지렛대다.** 팜→턱 지렛대가 140 mm 라
#   회전 ±45° 는 σ0.35 에서 턱을 38 mm 흔든다. FINE 위치 이득(19·12·14 mm)을 그대로
#   삼키므로 FINE 국면에서는 회전 박스도 함께 좁힌다(±11.3° → σ0.35 에서 9.6 mm).
PALM_FINE_EULER_HALF_RAD = 0.197          # rad = 11.3°
PALM_TO_JAW_LEVER = 0.140                 # m. 회전 지터 환산용(실측: 팜 z 0.121~0.130 → 턱 0.2475)
# 전환 — 래치 + 히스테리시스. 기존 게이트 술어와 같은 자를 쓴다(턱–컵 거리).
#   ⚠ ENTER < EXIT 여야 경계에서 두 지형을 오가지 않는다.
PALM_FINE_ENTER_DIST = 0.10               # m. 턱–컵 이 안으로 들어오면 FINE
PALM_FINE_EXIT_DIST = 0.15                # m. 이보다 멀어지면 COARSE 복귀
