...
# m/step. 사용자 지정값(08.25). 60 Hz 에서 6 m/s 이므로 팔 능력(약 1 m/s)보다 여전히
# 크다 — 즉 이 값은 팔을 굼뜨게 만들지 않고 **한 스텝 텔레포트만 3.8 배 줄인다**
# (380 → 100 mm). 구값 0.02(=1.2 m/s)는 팔 능력에 맞춘 값이었고 추종 지연을 만들었다.
# ⚠ 접촉 속도 자체를 낮추려면 팔 능력 이하로 내려야 한다(실측: 52 mm/step 이 이미 2.6배).
#   여기서는 텔레포트만 막고, 남는 낙하는 다음 판에서 이 값으로 스윕한다.
PALM_CMD_RATE_LIMIT = 0.10
