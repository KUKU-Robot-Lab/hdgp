...
목표: `openarm_tesollo_sensor_rl` 의 **왼팔 2지 그리퍼**가 cup_big 하나를 측면에서
...
| 물체 | MultiAsset 8종 + onehot | **cup_big 단일**, onehot 없음 |
...
2. **cup_big 은 원뿔형이다.** bbox 지름 90 mm 는 림의 최대치이고 몸통 하단은 62~71 mm.
   → 스케일 축소 불필요, 대신 **테이블 위 35~60 mm 에서만** 파지 가능(편측 여유 10.1 mm).
3. **jaw 수평 + 접근축 수평을 동시에 고정하면 팔이 자세를 못 낸다**(손목 j6 가 ±45°).
   파지에 필요한 건 jaw 축 수평뿐(두 접촉점이 컵 단면 지름 양끝)이므로 접근각은 풀었다.
   → 기준자세 **jaw 방위 −35° / 접근 기울기 55°** = 대각 측면 파지.
