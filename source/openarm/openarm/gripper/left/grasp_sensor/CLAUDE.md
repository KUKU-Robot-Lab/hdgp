...
`mdp.object_is_lifted` 는 **절대 z** 를 본다. 레퍼런스는 테이블 상면이 z=0 이라 `0.04` 를 쓰지만
우리 상면은 **0.215** 다. 상면을 안 더하면 컵이 테이블에 놓인 채로도 lifting 보상(weight 15)이
상시 1 이 되고 goal-tracking 게이트도 항상 열려 **학습이 통째로 망가진다**.
`P.MINIMAL_LIFT_HEIGHT` 를 쓰고, 직접 숫자를 적지 않는다.
