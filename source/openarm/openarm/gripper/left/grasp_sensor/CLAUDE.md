...
→ 두 가지를 나눠서 건다:
- **컵 자세는 게이트(AND)**: 40° 초과로 기울면 성공으로 치지 않는다 (`CUP_UPRIGHT_MIN_COS`).
- **jaw 수평은 연속 보너스**(weight 5 = lifting 15 의 1/3): 게이트로 넣으면 겨우 붙기
  시작한 파지가 한꺼번에 무너진다. 보너스면 gradient 가 생기고, **별도 term 이라
  TFEvents 에 로깅돼 학습 중 자세를 관측할 수 있다**(게이트로만 넣으면 안 보인다).
