...
class A2CBase(BaseAlgorithm):
    ...
    def __init__(self, base_name, params):
        ...
        # ★hdgp 수정(09.07): 원본은 실험명이 반드시 `<정수>_` 로 시작해야 한다(포크의 다중정책
        #   실험 규약). 우리 실험명은 `fj_b4` 처럼 트랙 접두라 int() 가 죽는다. SAPG 와 무관한
        #   규약이므로 관대하게 받는다 — 정수 접두가 없으면 0(단일 정책)으로 본다.
        _head = self.experiment_name.split('_')[0]
        self.policy_idx = int(_head) if _head.isdigit() else 0
        ...
        if self.expl_type != 'none':
            if self.expl_type.startswith('mixed_expl'):
                ...
                # ★hdgp 추가(09.07): 블록이 1개면 linspace(0.5,0,1)=[0.5] 라 계수가 균일해지고
                #   리더/팔로워가 사라진다 — SAPG 가 아니라 그냥 PPO 다. 원본은 나누어떨어지는지만
                #   보므로 num_envs == block_size 를 조용히 통과시킨다(4096/4096 이 정확히 그 경우).
                if self.num_actors // self.intr_coef_block_size < 2:
                    raise ValueError(
                        f"블록이 1개다 — num_actors({self.num_actors}) / "
                        f"expl_coef_block_size({self.intr_coef_block_size}). SAPG 는 블록이 2개 "
                        "이상이어야 탐색 램프와 리더/팔로워가 생긴다. block_size 를 줄이거나 "
                        "num_envs 를 늘려라.")
    ...
    def load_networks(self, params):
        ...
        # ★hdgp 수정(09.07): 원본은 `expl_type` 이 항상 있다고 가정한다. SAPG 를 안 쓰는
        #   설정에서는 None 이라 .startswith 가 죽는다. 다른 곳들과 같은 기본값을 준다.
        if self.config.get('expl_type', 'none').startswith('mixed_expl') and 'disjoint' in self.config.get('expl_type', 'none'):
            ...
            if self.global_rank == 0:
                ...
                if self.game_rewards.current_size > 0:
                    ...
                    if self.save_freq > 0:
                        ...
                        # ★hdgp 09.11 — 하드코딩 3 -> save_frequency (원본은 3 에포크마다 전체 상태를 썼다).
                        if epoch_num % self.save_freq == 0:
            ...
            if self.global_rank == 0:
                ...
                if self.game_rewards.current_size > 0:
                    ...
                    if self.save_freq > 0:
                        ...
                        # ★hdgp 09.11 — 하드코딩 200 -> save_frequency. 복구용 저장도
                        #   체크포인트 크기가 같아 같은 I/O 비용을 낸다(설정 하나로 묶는다).
                        if epoch_num % self.save_freq == 0:
                            torch_ext.safe_filesystem_op(os.makedirs, os.path.join(self.experiment_dir, 'last'), exist_ok=True)
