    ...
    def __init__(self, params):
        ...
        if self.expl_type != 'none':
            if self.expl_type.startswith('mixed_expl'):
                # ★hdgp 수정(09.07) — 원본 결함. 학습(a2c_common.__init__)은
                #     env_ids = arange(num_blocks).repeat_interleave(block_size)
                #     embedding = linspace(50,0,num_blocks)[env_ids]
                #   즉 **블록별 계단값**인데, 여기 원본은 `linspace(50,0,num_envs)` 로
                #   **env 별 램프**를 만든다. coef_cond 의 sigma 행과 extra_param 의
                #   임베딩은 이 값을 `== ids` 로 대조해 고르므로, 학습에 없던 값이 들어와
                #   비교가 전부 거짓이 되고 argmax 가 조용히 0 을 돌려준다 —
                #   재생이 리더가 아니라 **블록 0(가장 탐색적)** 의 sigma 로 돌아간다.
                #   학습과 같은 식으로 만든다. 블록 크기는 학습 설정에서 읽는다.
                _bs = int(config.get('expl_coef_block_size', self.env.num_envs))
                if self.env.num_envs % _bs != 0:
                    raise ValueError(
