#!/usr/bin/env python3
"""IKER unified (stage 1 + stage 2 in one policy) t2r reward round files (2026-09-18): render, ingest.
...
paths rather than an iteration directory.
    ...
    python3 scripts/iker/t2r3_reward.py render --out prompt.md [--previous compute_reward.py] \\
        [--feedback events.json] [--notes notes.md]
    ../IsaacLab/_isaac_sim/python.sh scripts/iker/t2r3_reward.py ingest \\
...
from openarm.agnostic.tasks.iker_shoe.t2r3 import prompts as P  # noqa: E402
from openarm.agnostic.tasks.iker_shoe.t2r3 import validator as V  # noqa: E402
...
def cmd_render(args: argparse.Namespace) -> int:
    ...
    cfg = PlaceRewardCfg()
    spec = P.PromptSpec(task=P.task_text(), previous_code=previous_code, feedback=feedback, user_notes=notes)
    # episode length: prompts.py's default mirrors iker_shoe_unified_env_cfg.UNIFIED_EPISODE_STEPS (a test pins them together);
    # importing the cfg here would pull isaaclab into a plain-python run of this CLI
    Path(args.out).write_text(P.render_prompt(spec, cfg=cfg), encoding="utf-8")
    print(f"[t2r3] prompt -> {args.out}")
    return 0
