...
# 홈 여유 기준. 이 팔은 이 작업영역에서 원래 한계 근처로 동작한다(파지 자세 여유도
# 0.11~0.25 수준) — 홈에만 0.20 을 요구하면 어떤 후보도 통과하지 못한다(실측).
# 파지 게이트와 같은 값으로 두고, 대신 **홈→파지 관절변위**를 최소화 기준으로 쓴다.
HOME_MARGIN_GATE = MARGIN_GATE
...
# ★가장 자연스러운 후보는 (0.094, 0, 0) = **박스 중심의 pregrasp 자세 그 자체**다.
#   액션 기준점이 어차피 per-episode pregrasp 이므로, 홈을 그 중심에 두면 Fabrics 가
#   매 에피소드 스폰 편차(±3cm/±6cm)만 따라가면 된다 — 손목 재배치가 아예 없다.
#   위/바깥으로 물린 후보들은 전부 관절 한계에 붙었다(실측 홈여유 0.000).
# ★후퇴량은 TCP 기준으로만 잡으면 부족하다. 핑거는 폭 61mm·길이 95mm 의 **부피**라
#   컵이 스폰 박스 가장자리(±3cm/±6cm)에 오면 옆구리가 겹친다 — 실측에서 리셋 직후
#   20스텝 만에 컵이 42mm 튕겨나갔다. 그래서 pregrasp(0.094)보다 더 물린 후보를 넣는다.
HOME_OFFSET_CANDIDATES = [
    (0.16, 0.00, 0.00), (0.16, 0.04, 0.02), (0.18, 0.00, 0.00),
    (0.18, 0.04, 0.04), (0.20, 0.02, 0.02), (0.20, 0.06, 0.04),
    (0.22, 0.04, 0.04), (0.14, 0.06, 0.04), (0.14, 0.00, 0.00),
    (0.094, 0.00, 0.00),
]
...
def main() -> int:
    ...
    for th, ph, sols, _mg in survivors:
        ...
        for back, lift, out in HOME_OFFSET_CANDIDATES:
            ...
            # 홈이 컵 위가 아니면 된다 — x/y 중 하나만 박스를 벗어나도 충분하고,
            # 충분히 높으면(파지 z + 8cm) 그것도 안전하다.
            outside = (abs(t_home[0] - CUP_CENTER_X) > SPAWN_HALF_X + 0.03
                       or abs(t_home[1] - CUP_CENTER_Y) > SPAWN_HALF_Y + 0.03
