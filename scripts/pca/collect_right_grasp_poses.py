#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""right grasp_v2 성공 파지 hand pose 수집기 (tesollo PCA 역추론 원료).

학습 완료 정책(rl_games LSTM)을 결정론으로 rollout 하면서, 에피소드마다
최초 성공(success_flag = in_success_region & valid) 순간의 손 20관절 자세와
물체 인덱스를 기록한다. 출력 npz 는 compute_tesollo_right_grasp_pca.py 의 입력.

실행 (IsaacLab 번들 python 필요):
  /home/user/rl_ws/IsaacLab/isaaclab.sh -p scripts/pca/collect_right_grasp_poses.py \
      --checkpoint log/rl_games/open-tesol/right/grasp-v2/lstm_test1/nn/<ckpt>.pth \
...
parser = argparse.ArgumentParser(description="right grasp_v2 성공 hand pose 수집")
...
parser.add_argument("--out", type=str, default=str(REPO / "data" / "right_grasp_poses_lstm_test1.npz"))
...
AGENT_YAML = (
    REPO / "source/openarm/openarm/tesollo/right/grasp_v2/config/agents/rl_games_ppo_lstm_cfg.yaml"
)
...
def main() -> None:
    ...
    env = gym.make("open-tesol_r_grasp_v2-lstm", cfg=env_cfg)
    ...
    np.savez_compressed(
        out,
        q_hand_right=q.astype(np.float32),
