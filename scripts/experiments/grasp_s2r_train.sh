...
case "$PRESET" in
  ...
  S2R)
    ...
    ARGS+=(env.obs_object_noise_coherent=true
           ...
           # 관절 PD 게인 — 벤더 고정값의 불확실성. **팔만** 대상이다(기본 arm).
           #   범위 0.7~2.0 은 R2S 문서의 감도 스윕 근거다 — `--kd-scale` 에서 우팔
           #   손목이 그 구간에서 주파수응답 0.666→0.498 로 완만하게 변했다.
           #   ★손은 뺀다: 09.01 튜닝 완료로 실기 JTC p 를 4.5 로 써 넣었고 실기↔sim
           #     정상오차 차이가 0.34°(손끝 1 mm 이하)로 실측됐다 — 랜덤화할 미지가 없다.
           #     게다가 sim kp 5.0 은 파지력 기준으로 정한 값이라 배율을 걸면 파지력이
           #     그만큼 깎인다.
           env.adr_joint_gain_scale_max=[0.7,2.0]
