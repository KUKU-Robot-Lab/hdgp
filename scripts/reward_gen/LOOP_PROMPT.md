...
6. 종료 조건: episode_success 최근 평균 ≥ 0.5 가 두 틱 연속 **그리고** task/nested_rate < 0.1 · bead/spill 정상
   ...
   또는 round ≥ MAX_ROUNDS(20 — 09.17 사용자 확장, 라운드 9 부터 env: 비드 30 mm 부피 DR·채움 obs·조준 전 틸트 래치).
   종료 시 사용자에게 best iter·지표·체크포인트·영상 경로를 보고한다.
   ★라운드 9 이후 피드백 표의 `task/premature_tilt_rate`·`task/tilt_far_deg`·`task/rcv_palm_speed`·`bead/fill_level` 은
   사용자 요구(접근 전 직립·리시버 정지 대기·비드 양 강건성)의 직접 지표다 — notes 에는 이 값만 사실로 적는다.
