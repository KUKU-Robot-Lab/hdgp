#!/usr/bin/env python3
"""자동 루프의 라운드 판정기 (pour_bi_rh · vision-3090 · pour_fabric_mimic) — 서버 런 상태를 읽어 "continue / advance / crashed" 를 낸다.
  ...
  advance : 현재 iter 의 events 로 `t2r.py reflect` 를 돌려 다음 iter 프롬프트를 만든다
    ...
    python3 scripts/reward_gen/t2r_round.py status  --label t2r_i00e1k --iter reward_gen/pour_bi_rh/iter_00
    python3 scripts/reward_gen/t2r_round.py advance --label t2r_i00e1k --iter reward_gen/pour_bi_rh/iter_00
...
KEY_TAGS = ("task/episode_success", "task/success_now", "task/src_grasped", "task/rcv_grasped",
            ...
            "ctrl/mimic_err_max", "ctrl/hand_dep_qd_max", "done/mimic_runaway", "done/mimic_err_runaway")
...
def tail_state(label: str) -> dict:
    log = f"~/rl_ws/hdgp/log/pour_fabric_mimic/{label}.log"
