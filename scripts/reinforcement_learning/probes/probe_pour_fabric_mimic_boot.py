"""pour_fabric_mimic 부팅·무작위 롤아웃 스모크 (Phase 1 검증 게이트).

게이트: 부팅(fabric FK 정합 ×2 팔) · N 스텝 무작위 액션에서 NaN/runaway 0 ·
hold 종료 시 비드 in_source ≥ 0.9 · 보상 로더 경로 동작.

    ./isaaclab.sh -p scripts/reinforcement_learning/probes/probe_pour_fabric_mimic_boot.py \
        --task open-rh_b_pour_fab_mimic --num_envs 8 --steps 300 [--reward_code_path f.py]
"""
...
parser.add_argument("--tilt_slot", type=int, default=5, help="pour 에서 −1 을 줄 소스 회전 슬롯(3|4|5)")
...
for t in range(args.steps):
    ...
    if t % 50 == 0:
        print(f"[step {t:4d}] inS={float(extras['bead/in_source']):.3f} "
              ...
              f"mimicErr={float(extras['ctrl/mimic_err_max']):.3f} closS={float(extras['task/src_closure']):.2f} "
              f"rew={float(rew.mean()):+.3f}", flush=True)
        summary.setdefault("mimic_err_max", 0.0)
        summary["mimic_err_max"] = max(summary["mimic_err_max"], float(extras["ctrl/mimic_err_max"]))
