...
parser.add_argument("--dump_steps", type=int, default=0, help="[P3] env0 롤아웃 이 스텝만 (obs,action,fabric_q) 덤프 후 종료.")
parser.add_argument("--dump_out", type=str, default="hdgp/docs/eval/p3_rollout.npz", help="[P3] 덤프 경로.")
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    # [P3 dump] 라이브 루프 검증 기준: env0 의 (obs, action, fabric_q, cup pose) 시퀀스.
    _dump_steps = int(getattr(args_cli, "dump_steps", 0))
    _dump = {"actor_obs": [], "action": [], "fabric_q": [],
             "arm_q": [], "finger_q": [], "cup_pos_w": [], "cup_quat_w": [],
             "left_cup_pos_w": [], "left_cup_quat_w": [], "pour_ready": []}
    _e = _raw_env
    ...
    for t in range(args_cli.eval_steps):
        with torch.inference_mode():
            ...
            if _dump_steps and t < _dump_steps:
                ao = obs["obs"] if isinstance(obs, dict) else obs
                _dump["actor_obs"].append(ao[0].detach().cpu().numpy().copy())
                _dump["action"].append(actions[0].detach().cpu().numpy().copy())
                _dump["fabric_q"].append(_e.fabric_q[0].detach().cpu().numpy().copy())
                _dump["arm_q"].append(_e.robot.data.joint_pos[0, _e.arm_dof_indices].detach().cpu().numpy().copy())
                _dump["finger_q"].append(_e.robot.data.joint_pos[0, _e.hand_dof_indices].detach().cpu().numpy().copy())
                _dump["cup_pos_w"].append((_e.cup.data.root_pos_w[0] - _e.scene.env_origins[0]).detach().cpu().numpy().copy())
                _dump["cup_quat_w"].append(_e.cup.data.root_quat_w[0].detach().cpu().numpy().copy())
                _dump["left_cup_pos_w"].append((_e.left_target_cup.data.root_pos_w[0] - _e.scene.env_origins[0]).detach().cpu().numpy().copy())
                _dump["left_cup_quat_w"].append(_e.left_target_cup.data.root_quat_w[0].detach().cpu().numpy().copy())
                _dump["pour_ready"].append(float(_e._pour_ready_latched[0].item()))
                if t == _dump_steps - 1:
                    import numpy as _np
                    _out = os.path.abspath(args_cli.dump_out)
                    os.makedirs(os.path.dirname(_out), exist_ok=True)
                    _np.savez(_out, **{k: _np.array(v) for k, v in _dump.items()})
                    print(f"[P3-dump] saved {_dump_steps} step rollout → {_out}", flush=True)
                    print(f"[P3-dump] action[0]={_np.array2string(_dump['action'][0], precision=4)}", flush=True)
